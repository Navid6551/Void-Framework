...
AddEventHandler('vrp-items:SetAmmo', function(sentammoTable)
...
function actionBarDown()
	...
	exports["vrp-ui"]:sendAppEvent("hud", {
		displayAllForce = true,
		displayAllForceVehicle = GetVehiclePedIsIn(PlayerPedId()) ~= 0,
		displayRadioChannel = true,
	})
	TriggerServerEvent("vrp-financials:cash:get", GetPlayerServerId(PlayerId()))
end
...
Citizen.CreateThread(function()
	exports["vrp-keybinds"]:registerKeyMapping("", _L("keybinds-player", "Player"), _L("keybinds-actionbar", "Action Bar"), "+actionBar", "-actionBar", "Z")
	RegisterCommand('+actionBar', actionBarDown, false)
	RegisterCommand('-actionBar', actionBarUp, false)

	exports["vrp-keybinds"]:registerKeyMapping("", _L("keybinds-player", "Player"), _L("keybinds-wink", "Wink"), "+playerWink", "-playerWink")
	RegisterCommand('+playerWink', playerWink, false)
	RegisterCommand('-playerWink', function() end, false)

	exports["vrp-keybinds"]:registerKeyMapping("", _L("keybinds-player", "Player"), _L("keybinds-passive", "Passive Mode"), "+passiveMode", "-passiveMode")
...
RPC.register('inventory:getGSR', function()
	return shotRecentlyLoopActive
end)

RegisterNetEvent('vrp-spawn:characterSpawned', function()
...
Citizen.CreateThread( function()
	...
	while true do
      ...
      if weapon == "-37975472" then
        TriggerEvent("vrp-weapons:threwSmokeGrenade")
        if lastEquippedItemToRemove then
          if exports["vrp-inventory"]:hasEnoughOfItem(lastEquippedItemToRemove,1,false) then
        ...
        lastEquippedItemToRemove = nil
			...
			if throwableWeapons[weapon] and exports["vrp-inventory"]:hasEnoughOfItem(weapon,1,false) then
			...
			if selectedWeapon ~= lastWeapon and (lastWeapon == 571920712) then
				local hasItem = exports["vrp-inventory"]:hasEnoughOfItem("571920712", 1, false, true, lastEquippedInfo)
				if not hasItem then
					lastEquippedInfo = {}
				else
		...
		if IsPedPlantingBomb(ped) then
			if exports["vrp-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
AddEventHandler("vrp-voice:focus:set", function(pState)
  focusTaken = pState
end)
...
AddEventHandler('equipWeaponID', function(hash,newInformation,sqlID,itemToRemove)
	--GiveAmmoNow()
	if not exports["vrp-propattach"]:canPullWeaponHoldingEntity() then return end
  ...
  lastEquippedItemToRemove = itemToRemove
	...
	TriggerEvent("vrp-weapons:client:weaponEquiped", hash, newInformation, sqlID, itemToRemove, armed)
	TriggerServerEvent("vrp-weapons:weaponEquiped", hash, newInformation, sqlID, itemToRemove, armed)
...
function updateAmmoTable(newammo,ammoType)
	...
	TriggerServerEvent("vrp-weapons:updateAmmo",newammo,ammoType,ammoTable)
end
...
RegisterNetEvent('vrp-item:CheckClientAmmo')
AddEventHandler('vrp-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
...
function unholster1h(weaponHash, a, info)
  ...
  lastEquippedInfo = info
	...
	local effect = exports['vrp-galleria']:GetEffect('WEAPON_PULL')
	if not effect then
		effect = {
			level = 0
		}
...
function AttachmentCheck(weaponhash)
	...
	if exports["vrp-inventory"]:hasEnoughOfItem("weapon_silencer_assault", 1, false, true) then
	...
	if exports["vrp-inventory"]:hasEnoughOfItem("weapon_silencer_pistol", 1, false, true) then
	...
	if exports["vrp-inventory"]:hasEnoughOfItem("weapon_oil_silencer", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 1532150734 )
	end

	if exports["vrp-inventory"]:hasEnoughOfItem("weapon_scope", 1, false, true) then
	...
	if exports["vrp-inventory"]:hasEnoughOfItem("weapon_uzi_extended", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xE40F1CD2 )	
	end
	if exports["vrp-inventory"]:hasEnoughOfItem("weapon_uzi_foldstock", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x8FBBD54C )		
	end
	if exports["vrp-inventory"]:hasEnoughOfItem("weapon_uzi_woodstock", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x722C6CA6 )		
	end
	if exports["vrp-inventory"]:hasEnoughOfItem("weapon_extended", 1, false, true) then
	...
	if exports["vrp-inventory"]:hasEnoughOfItem("weapon_taser_banana", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x12359AF7 )	
	end
	if exports["vrp-inventory"]:hasEnoughOfItem("weapon_pickaxe", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xE648EB21 )	
	end
	if exports["vrp-inventory"]:hasEnoughOfItem("weapon_cutlass", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x415B3660 )	
	end
	if exports["vrp-inventory"]:hasEnoughOfItem("red_saber", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xF17E0A19 )	
	end
	if exports["vrp-inventory"]:hasEnoughOfItem("orange_saber", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x5B27D3 )	
	end
	if exports["vrp-inventory"]:hasEnoughOfItem("red_t_saber", 1, false, true) then
...
RegisterNetEvent("vrp-weapons:hitPlayerWithCash", function(pTarget)
  if not lastEquippedInfo.amount then
	TriggerEvent('DoLongHudText', 'Cash could not be transferred', 2)
	return
  end

  TriggerServerEvent("vrp-weapons:processGiveCashAmount", pTarget, lastEquippedInfo.amount, lastEquippedInfo.id)
end)
