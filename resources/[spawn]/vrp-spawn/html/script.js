...
let v = new Vue({
	...
	methods: {
		...
		confirmDelete(){
			const characterFullName = `${this.currentCharData.first_name} ${this.currentCharData.last_name}`
			const characterFullNameTrimmed = `${this.currentCharData.first_name?.trim()} ${this.currentCharData.last_name?.trim()}`
...
function sendNuiMessage(data){
	$.post("https://vrp-spawn/nuiCallback", JSON.stringify(data));
}
