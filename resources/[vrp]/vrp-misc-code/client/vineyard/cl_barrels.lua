...
AddEventHandler("vrp-inventory:itemUsed", function(pItem, pData)
...
AddEventHandler("vrp-vineyard:pickupBarrel", function(p1, p2, p3)
  ...
  exports['vrp-objects']:DeleteObject(p3.meta.id)
  TriggerEvent("player:receiveItem", "vineyardbarrel", 1, false, data, data)
end)


RegisterUICallback("vrp-vineyard:changeBarrelName", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["vrp-ui"]:closeApplication("textbox")
  RPC.execute("vrp-vineyard:changeBarrelName", data.key, data.values.name)
end)

RegisterUICallback("vrp-vineyard:addGrapesToBarrel", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports["vrp-ui"]:closeApplication("textbox")
  local newData = data.key.newData
  local objectId = data.key.objectId
  local items = exports["vrp-inventory"]:getItemsOfType("vineyardgrapes", false, true)
  ...
  RPC.execute("vrp-vineyard:changeBarrelData", objectId, newData)
end)

local function getTimestamp(data)
  local remaining = math.floor((exports["vrp-misc-code"]:getCurrentTime() - data.created_at) / 1000)
...
AddEventHandler("vrp-vineyard:barrelAction", function(p1, p2, p3)
  if not isGrapeBarrel(p3) then return end
  local data = p3.meta.data.metadata
  ...
  if p1.action == "change_name" then
    exports["vrp-ui"]:openApplication("textbox", {
      callbackUrl = "vrp-vineyard:changeBarrelName",
      key = objectId,
      items = {
  ...
  if p1.action == "seal" then
    ...
    local newData = { sealed = true, created_at = exports["vrp-misc-code"]:getCurrentTime() }
    RPC.execute("vrp-vineyard:changeBarrelData", objectId, newData)
    return
  end
  ...
  if p1.action == "add_grapes" then
    ...
    if #items == 0 then return end
    exports["vrp-ui"]:openApplication("textbox", {
      callbackUrl = "vrp-vineyard:addGrapesToBarrel",
  ...
  if p1.action == "pour_bottles" then
    local hasEnough = exports["vrp-inventory"]:hasEnoughOfItem("vineyardwinebottleempty", 1, false, true)
    if not hasEnough then
      TriggerEvent("DoLongHudText", "You need empty wine bottles!", 2)
      return
    end
    TriggerEvent("doAnim","cokecut")
    local finished = exports["vrp-taskbar"]:taskBar(10000, "Bottling Wine")
    ClearPedTasks(PlayerPedId())
    if finished ~= 100 then return end
    exports['vrp-objects']:DeleteObject(objectId)
...
AddEventHandler("vrp-inventory:attachmentsToggle", function(pEnabled, pId)
...
Citizen.CreateThread(function()
  exports["vrp-interact"]:AddPeekEntryByModel({ `prop_wooden_barrel` }, {
    {
      id = "peek_prop_vineyard_barrel",
      event = "vrp-vineyard:pickupBarrel",
      icon = "shopping-basket",
      label = "Pick Up",
    },
    {
      id = "peek_prop_vineyard_barrel_details",
      event = "vrp-vineyard:barrelAction",
      icon = "info-circle",
      label = "View Label",
  ...
  exports["vrp-interact"]:AddPeekEntryByModel({ `prop_wooden_barrel` }, {
    ...
    isEnabled = function(p1, p2, p3)
      ...
      local hasEnough = exports["vrp-inventory"]:hasEnoughOfItem("vineyardbarrelheadopener", 1, false, true)
      return hasEnough
    end,
  })
end)
