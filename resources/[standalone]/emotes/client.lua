...
local COOLDOWN_EMOTES = {
    ["passout"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["chill"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["cloudgaze"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["cloudgaze2"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["crawl"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["fallover"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["fallover2"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["fallover3"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["fallover4"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["injured"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["passout"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["passout2"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["passout3"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["passout4"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["passout5"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["prone"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["shot"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["situps"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["sleep"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["slump"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["sunbathe"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["sunbathe2"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["weights"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["sit7"] = { cooldown = 5 , consumeStamina = 50.0 },
    ["getontheground"] = { cooldown = 5 , consumeStamina = 50.0 },
}
...
AddEventHandler("Animation:Set:Gait", function(pArgs)
    ...
    if setGait == "move_m@swagger" then
      if exports["vrp-inventory"]:hasEnoughOfItem("pimpcane", 1) then
        TriggerEvent("vrp-propattach:attach", "prop_cs_walking_stick")
      end
    end
end)
...
anims = {
    ...
    ["c"] = function(ped)
        if exports["vrp-flags"]:HasPedFlag(PlayerPedId(), 'isSittingOnChair') then
          TriggerEvent("vrp-emotes:sitOnChair")
        elseif exports["vrp-flags"]:HasPedFlag(PlayerPedId(), 'isPoledancing') then
    ...
    ["standguard3"] = function(ped)
        local testdic = "yoinks@idlechill"
    ...
    ["umbrella"] = function(ped)
        if obj ~= 0 then
            ...
            TriggerEvent("destroyPropPerm")
            if exports["vrp-inventory"]:hasEnoughOfItem("umbrella", 1) then

                TriggerEvent("actionbar:setEmptyHanded")

                local finished = exports["vrp-taskbar"]:taskBar(2500,"Opening Umbrella")
    ...
    ["armwindow"] = function(ped)
        local animDict = "anim@veh@lowrider@std@ds@arm@base"
        local animation = "sit_low_lowdoor"
        if IsPedArmed(ped, 7) then
            TriggerEvent("actionbar:setEmptyHanded")
        end
    ...
    ["armwindow2"] = function(ped)
        local animDict = "anim@veh@armwindow"
        local animation = "sit_low_lowdoor"
        if IsPedArmed(ped, 7) then
            TriggerEvent("actionbar:setEmptyHanded")
        end
    ...
    ["armwindow3"] = function(ped)
        local animDict = "anim@veh@armwindow2"
        local animation = "sit_low_lowdoor2"
        if IsPedArmed(ped, 7) then
            TriggerEvent("actionbar:setEmptyHanded")
        end
    ...
    ["armwindow4"] = function(ped)
        local animDict = "anim@veh@armwindow3"
        local animation = "sit_low_lowdoor3"
        if IsPedArmed(ped, 7) then
            TriggerEvent("actionbar:setEmptyHanded")
        end
    ...
    ["getontheground"] = function(ped)
        local animDict = "anim@getontheground"
        local animation = "ontheground"
        if IsPedArmed(ped, 7) then
            TriggerEvent("actionbar:setEmptyHanded")
        end
    ...
    ["standguard2"] = function(ped)
        local animDict = "yoinks@idlechill"
        local animation = "yoinks_clip"
        if IsPedArmed(ped, 7) then
            TriggerEvent("actionbar:setEmptyHanded")
        end
    ...
    ["smoke"] = function(ped)
        ...
        if exports["vrp-inventory"]:hasEnoughOfItem("ciggy", 1) then
            ClearPedTasks(PlayerPedId())
    ...
    ["cigar"] = function(ped)
        if exports["vrp-inventory"]:hasEnoughOfItem("cigar", 1) then
            local animDict = "amb@world_human_smoking@male@male_a@enter"
            local animation = "enter"
            if IsPedArmed(ped, 7) then
                TriggerEvent("actionbar:setEmptyHanded")
    ...
    -- CAUSES RANDOM CRASHES WHEN PEOPLE DO THE EMOTE
...
animsDog = {
    ...
    ["carenter"] = function(ped)
        local animDict = "creatures@rottweiler@in_vehicle@std_car"
        local currentTarget = exports['vrp-target']:GetCurrentEntity()
        ...
        local currentTargetCoords = exports['vrp-target']:GetCurrentTargetCoords()
...
function EndAnim(dict, anim)
    ...
    TaskPlayAnim(PlayerPedId(), dict, anim, 999950.0, 1.0, animlength, 48, 0, 0,
                 0, 0)
end
...
AddEventHandler("animation:PlayAnimation", function(anim)
    if imdead == 0 and not stuckincar and not IsPedGettingUp(PlayerPedId()) then
        ...
        if ped then
            ...
            -- Cooldown based emotes with option to consume stamina
            local cooldownInfo = COOLDOWN_EMOTES[animName]

            if cooldownInfo then
                local time = GetGameTimer()
                local minCD = cooldownInfo.cooldown or 10

                if antiSpamCooldown and time - antiSpamCooldown < minCD * 1000 then
                    return TriggerEvent('DoLongHudText', 'Cant do this right now..', 2)
                end

                antiSpamCooldown = time

                if cooldownInfo.consumeStamina then
                    SetPlayerStamina(PlayerId(), 100.0 - cooldownInfo.consumeStamina)
                end
...
function StressTest(animName)
    if (animName == "pushups" or animName == "situps" or animName == "chinups" or
        ...
        (#(GetEntityCoords(PlayerPedId()) - vector3(1744.53, 2646.94, 45.61)) <
            ...
            10.0) then
        local finished =
            exports["vrp-taskbar"]:taskBar(15000, "Relieving Stress")
        if finished == 100 then
