...
RegisterNetEvent('vrp-base:playerSpawned');
on('vrp-base:playerSpawned', async (broughtData) => {
...
//  $.post("http://vrp-inventory/SlotJustUsed", JSON.stringify({target: targetSlot, origin: originSlot, itemid: itemidsent }));
let recentused = [];
...
on('__cfx_nui:RepairSlot', (data, cb) => {
    ...
    emitNet('vrp-inventory:repairSlot', slot);
    cb({});
});
...
let itemListWithTax = async () => {
    const tax = 0//await RPC.execute("GetTaxLevel", "Goods");
...
on('vrp-police:cuffs:state', (pIsHandcuffed, pIsHandcuffedAndWalking) => {
...
on('inventory:open_hidden', (penis, vehicleFound) => {
    let vehId = exports['vrp-vehicles'].GetVehicleIdentifier(vehicleFound)
...
on('inventory-open-request', async () => {
    ...
    const apartmentFloor = exports["vrp-apartments"].getModule("func").getApartment()
    let targetid = 0;
    cid = exports.isPed.isPed("cid");
    ...
    const currentTarget = exports['vrp-target'].GetCurrentEntity()
    ...
    if (isInVehicle) {
        ...
        if (!IsThisModelABicycle(vehicleModel) && !IsThisModelABike(vehicleModel)) {
          if (blockedGloveblockVehicles.includes(vehicleModel)) {
            ...
            const vehId = exports['vrp-vehicles'].GetVehicleIdentifier(vehicleFound)
            if (!vehId) {
...
function openTrunk(pEntity) {
    ...
    const vehId = exports['vrp-vehicles'].GetVehicleIdentifier(pEntity)
    if (!vehId) {
        CloseGui();
...
on('vrp-inventory:openDumpster', (pParams, pEntity, pContext) => {
    openDumpster(pEntity);
});

on('vrp-inventory:openTrunk', (pParams, pEntity, pContext) => {
    openTrunk(pEntity);
});
...
function drawMarkersUI() {
    for (let Row in DrawInventories) {
        DrawMarker(
            ...
            DrawInventories[Row]?.groundZ && !Number.isNaN(DrawInventories[Row].groundZ) ? DrawInventories[Row].groundZ + 0.2 : DrawInventories[Row].position.z - 0.8,
            0,
            0,
            0,
            0,
...
RegisterNetEvent('vrp-items:SetAmmo');
on('vrp-items:SetAmmo', (sentammoTable) => {
    if (sentammoTable) {
...
on('__cfx_nui:ServerCloseInventory', (data, cb) => {
    ...
    if (data.name != 'none') {
        ...
        emit('vrp-inventory:closed', data.name);
    }
    cb({});
});
...
on('__cfx_nui:craftProgression', (data, cb) => {
    cb("ok");
    emit("vrp-inventory:craftProgression", data);
    emitNet("vrp-inventory:craftProgression", data);
});

RegisterNuiCallbackType('weaponCrafted');
on('__cfx_nui:weaponCrafted', (data, cb) => {
    cb("ok");
    emitNet('vrp-inventory:weaponCrafted', data.item);
});
...
async function CloseGui(pIsItemUsed = false) {
    ...
    //Remove blur
    // const forcedBlurred = exports['vrp-miscscripts'].IsCurrentlyBlurred()
    // if (!forcedBlurred) {
...
function hasPermissionInProperty(){
    // if(exports["vrp-housing"].isInsideProperty() && exports["vrp-housing"].hasPermissionInProperty() == true && exports["vrp-housing"].isPropertyShop()){
    //     return true
    // }
    // else
        return false 
...
function isPropertyLockdownAndIsCop(){
    // if(exports["vrp-housing"].isInsideProperty() && exports["vrp-housing"].isPropertyShop() && exports["vrp-housing"].isCurrentPropertyLockdown()){
    //     let cop = false;
    //     if (exports.isPed.isPed('myjob') == 'police' || exports.isPed.isPed('myjob') == 'judge') {
    //         cop = true;
    //     }
    //     if (cop){
    //         return true;
...
function SetCustomNuiFocus(hasKeyboard, hasMouse) {
    ...
    //   if (HasNuiFocus === true) {
    //   	emit("vrp-voice:focus:set", HasNuiFocus, hasKeyboard, hasMouse);
    //   } else {
    // 	  setTimeout(() => {if (HasNuiFocus !== true) emit("vrp-voice:focus:set", false, false, false);}, 1000)
    //   }
}
...
exports('setPlayerWeight', (cid, weight) => {
    ...
    emitNet('vrp-inventory:server:weightChange', cid, weight);
...
let tradingIn = false;
on('vrp-inventory:sionisReceipt', async ([pClass]) => {
    if (tradingIn) {
        return;
    }
    tradingIn = true;
    const [hasCraftAccess] = await RPC.execute('vrp-business:hasPermission', 'sionis', 'craft_access');
    if (!hasCraftAccess) {
        tradingIn = false;
        return;
    }
    emit('doAnim', 'cokecut');
    const finished = await new Promise((r) => exports['vrp-taskbar'].taskBar(10000, 'Gathering materials...', true, true, null, false, r));
    ClearPedTasks(PlayerPedId());
    if (finished !== 100) {
        return;
    }
    ...
    matsToTrade.forEach((mat, k) => {
        ...
        const qty = Math.min(exports['vrp-inventory'].getQuantity(mat, true), 50000);
    ...
    TriggerServerEvent('vrp-inventory:sionisTaxedMats', taxedMats);
    canOpen = true;
    tradingIn = false;
});

on('vrp-inventory:sionisReceiptTradeIn', async () => {
    const items = exports['vrp-inventory'].getItemsOfType('refinedmaterialreceipt', 1000, true);
    if (items.length === 0) {
        return;
    }
    const [hasCraftAccess] = await RPC.execute('vrp-business:hasPermission', 'sionis', 'craft_access');
...
on('vrp-inventory:sionisCollectTaxes', () => {
    // const ids = exports['vrp-config'].GetMiscConfig('sionis.pickup.ids');
    // const cid = exports['isPed'].isPed('cid');
    // if (ids.indexOf(cid) === -1) {
    //     return;
    // }
    // TriggerServerEvent('vrp-inventory:sionisCollectTaxes')
});

on('vrp-inventory:openGoldRush', async () => {
    const [hasCraftAccess] = await RPC.execute('vrp-business:hasPermission', 'gold_rush', 'craft_access');
    if (!hasCraftAccess) {
        return;
    }
...
on('vrp-inventory:itemUsed', async (pItem, pInfo, pInv, pSlot) => {
    ...
    if (!meta.id) {
        ...
        meta = await RPC.execute('vrp-inventory:getItemInformation', pInv, pSlot);
    }
    ...
    const prompt = await exports['vrp-ui'].OpenInputMenu(
        [
            {
                label: 'Action',
                name: 'action',
    ...
    if (prompt.action === 'add') {
        for (const key of validKeys) {
            const qty = Math.min(exports['vrp-inventory'].getQuantity(key, true), 50000);
            if (!qty) {
                continue;
            }
            if (!meta[key]) {
        ...
        TriggerServerEvent('vrp-inventory:matBagUpdate', meta.id, meta);
        return;
    }
...
tradingIn = false;
on('vrp-cia:depositMaterials', async () => {
    if (tradingIn) return;
    tradingIn = true;
    emit('doAnim', 'cokecut');
    const finished = await new Promise((r) => exports['vrp-taskbar'].taskBar(10000, 'Gathering materials...', true, true, null, false, r));
    ...
    const qty = Math.min(exports['vrp-inventory'].getQuantity('cia_weapon_material', true), 50000);
    if (qty < 1) return;
    emit('inventory:removeItem', 'cia_weapon_material', qty);
    TriggerServerEvent('vrp-inventory:depositCiaMaterials', qty);
});

on('vrp-cia:checkMaterials', async () => {
    if (tradingIn) return;
    TriggerServerEvent('vrp-inventory:checkCiaMaterials');
});

on('vrp-cia:getGear', async () => {
    let response = 1;
    while (response) {
        await new Promise(r => setTimeout(r, 250));
        response = await exports['vrp-ui'].OpenContextMenu([
            {
                key: { item: '-820634585' },
                title: 'Taser',
            },
        ...
        if (!response) break;
        TriggerServerEvent('vrp-inventory:ciaGetGear', response.item);
    }
});

// AddEventHandler("vrp-cia:getGear", function()
  
// end)

// AddEventHandler("vrp-cia:checkMaterials", function()
