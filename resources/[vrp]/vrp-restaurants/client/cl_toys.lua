AddEventHandler('vrp-restaurants:manageToyMenu', function(pParams, pEntity, pContext)
    ...
    local currentToys = RPC.execute('vrp-restaurants:getCustomToys', restaurant)

    local context = {
        {
            title = 'Add New Toy',
            icon = 'plus',
            action = 'vrp-restuarants:createToy',
            key = restaurant
        }
    }
            ...
            children = {
                {
                    title = 'Delete',
                    icon = 'trash',
                    action = 'vrp-restuarants:removeToy',
                    key = {
...
RegisterUICallback("vrp-restuarants:createToy", function(data, cb)
    ...
    local result = RPC.execute('vrp-restaurants:createToyItem', data.key, prompt)
    if not result then
...
RegisterUICallback("vrp-restuarants:removeToy", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    local result = RPC.execute('vrp-restaurants:removeToyItem', data.key.restaurant, data.key.id)
    if not result then
...
AddEventHandler('vrp-restaurants:getCustomToyItem', function(pParameters, pEntity, pContext)
    local restaurant = pContext.zones['restaurant_takeout'].restaurant
    local toys = RPC.execute('vrp-restaurants:getCustomToys', restaurant)
