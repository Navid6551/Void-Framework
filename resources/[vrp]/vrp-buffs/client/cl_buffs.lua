...
AddEventHandler("vrp-buffs:stamina", function(pEnabled, pPercent)
...
AddEventHandler("vrp-buffs:setJobLuckMultiplier", function(pJobLuckMultiplier)
  jobLuckMultiplier = 1 + pJobLuckMultiplier + 0.0
  TriggerServerEvent("vrp-buffs:setBuffValue", "jobluck", jobLuckMultiplier)
end)
...
AddEventHandler("vrp-buffs:setAlertLevelMultiplier", function(pAlertLevelMultiplier)
...
AddEventHandler("vrp-buffs:inDoubleBuffZone", function(pBool)
  ...
  Citizen.CreateThread(function()
    ...
    while isSitting and inRestaurantBuffZone do
      if minuteCount >= 5 then
        ...
        TriggerEvent("vrp-buffs:applyBuff", "restaurant_double", {{ buff = "double", percent = percent }})
      end
      Citizen.Wait(60000)
