...
RegisterNetEvent("vrp-signin:signOffDuty")
AddEventHandler("vrp-signin:signOffDuty", function()
  signOff()
end)

RegisterCommand('dutyon', function(p, pArgs)
  TriggerServerEvent("vrp-signin:duty", pArgs[1])
end)

RegisterUICallback("vrp-signin:handler", function(data, cb)
  local eventData = data.key
  if eventData.sign_in then
    TriggerServerEvent("vrp-signin:duty", eventData.job)
...
RegisterUICallback("vrp-signin:spawnVehicle", function(data, cb)
  local eventData = data.key
...
AddEventHandler("vrp-signin:peekAction", function(pArgs, pEntity, pContext)
  ...
  exports["vrp-ui"]:showContextMenu(MenuData[name])
end)

AddEventHandler("vrp-signin:legalAid", function(pArgs, pEntity, pContext)
  TriggerEvent("DoLongHudText", "You now appear as a legal aid.")
  RPC.execute('vrp-gov:dojApp:setStatus', "legal_aid", "Available")
end)
