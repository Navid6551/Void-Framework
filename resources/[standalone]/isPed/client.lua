...
AddEventHandler("ped:intrunk", function(trunksent)
  intrunk = trunksent
  exports['vrp-flags']:SetPedFlag(PlayerPedId(), 'isInTrunk', intrunk)
end)
...
AddEventHandler("robbing",function(passedR)
    robbing = passedR
    exports['vrp-flags']:SetPedFlag(PlayerPedId(), 'isRobbing', intrunk)
end)
...
AddEventHandler('pd:deathcheck', function()
  dead = not dead
  exports['vrp-flags']:SetPedFlag(PlayerPedId(), 'isDead', dead)
end)

-- local handcuffed = exports["isPed"]:isPed("handcuffed")
RegisterNetEvent('vrp-police:cuffs:state')
AddEventHandler('vrp-police:cuffs:state', function(handCuffed,handCuffedWalking)
	handcuffed = handCuffed
    handcuffedwalking = handCuffedWalking
    exports['vrp-flags']:SetPedFlag(PlayerPedId(), 'isCuffed', handCuffed or handCuffedWalking)
end)

RegisterNetEvent('vrp-police:cuffs:recentlyAttempted')
AddEventHandler('vrp-police:cuffs:recentlyAttempted', function(lastTimeCuffedSent)
...
AddEventHandler("vrp-jobmanager:playerBecameJob", function(jobpassed, name, notify)
	job = jobpassed
    if not job then
        job = "None"
    end
...
AddEventHandler('onResourceStart', function(resource)
    if ResourceName == resource or not cid or tonumber(cid) <= 0 then return end
