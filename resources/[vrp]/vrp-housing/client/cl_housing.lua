...
function Housing.func.enterBuilding(propertyID,enterOverride,counterPart)
    ...
    TriggerServerEvent("vrp-housing:halloween:playerEnteredProperty", propertyID)
    ...
    local isBuiltCoords,objects = exports["vrp-build"]:getModule("func").buildRoom(model,spawnBuildingLocation,false,Housing.destroyedObjects,enterOverride,true, propertyID)
    if counterPart and Housing.staticObjectRobPoints == nil then
        Housing.staticObjectRobPoints = exports["vrp-build"]:getModule("func").getRobLocationsForObjects(model,spawnBuildingLocation,Housing.housingRobTargets.static)
    ...
    if isBuiltCoords then
        ...
        if not counterPart then
            TriggerEvent("vrp-editor:buildName" ,"housing-"..propertyID,objects,offsets,isPropertyShop(propertyID))
        end
...
function placeBench(dropInventory)
    ...
    local buildingVector = exports["vrp-build"]:getModule("func").currentBuildingVector()
    local hasBenches = exports["vrp-config"]:GetMiscConfig("housing.benches")
...
AddEventHandler('housing:crafting', function()
    ...
    if not hasCraftingProg then
        local serverCode = exports["vrp-config"]:GetMiscConfig("progression.guns.inventory")
