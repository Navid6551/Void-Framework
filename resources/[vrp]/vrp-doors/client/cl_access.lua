...
AddEventHandler("vrp-jobs:jobChanged", function(currentJob, previousJob)
...
RegisterNetEvent("vrp-doors:mrpd:keyStatusUpdate")
AddEventHandler("vrp-doors:mrpd:keyStatusUpdate", function()
    clearAccessCache()
end)
...
function hasSecuredAccess(pId, pType)
    ...
    local hasMrpdKey = RPC.execute("vrp-doors:charHasMrpdKeys", characterId)
    ...
    if secured.access.item ~= nil then
        ...
        for i, v in pairs(secured.access.item) do
            if exports["vrp-inventory"]:hasEnoughOfItem(i, 1, false) then
                return true
            end
        end
    end
    ...
    local hackedAccess = RPC.execute('vrp-doors:getHackedAccess')
