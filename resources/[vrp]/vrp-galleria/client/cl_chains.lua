...
AddEventHandler("vrp-inventory:itemUsed", function(pItem, pInfo)
  ...
  if not hasCgChainOn then
    ...
    TriggerEvent("vrp-props:attachProp", model, 10706, -0.02, 0.02, -0.06, -366.0, 19.0, -163.0, true, true)
    Citizen.CreateThread(function()
      while hasCgChainOn do
        Citizen.Wait(math.random(10000, 120000))
        if info.diamond then
          local gemInfo = exports['vrp-galleria']:getGemData(info.diamond)
          TriggerServerEvent("vrp-fx:chain:blingDiamonds", GetEntityCoords(PlayerPedId()), "diamonds", gemInfo.count, 100)
        end
    ...
    Citizen.CreateThread(function()
      while hasCgChainOn do
        Citizen.Wait(math.random(10000, 120000))
        if info.ruby then
          local gemInfo = exports['vrp-galleria']:getGemData(info.ruby)
          TriggerServerEvent("vrp-fx:chain:blingDiamonds", GetEntityCoords(PlayerPedId()), "ruby", gemInfo.count, 100)
        end
      end
    end)
    Citizen.CreateThread(function()
      while hasCgChainOn do
        ...
        if info.tanzanite then
          local gemInfo = exports['vrp-galleria']:getGemData(info.tanzanite)
          TriggerServerEvent("vrp-fx:chain:blingDiamonds", GetEntityCoords(PlayerPedId()), "tanzanite", gemInfo.count, 100, 0.25)
        end
      end
    end)
  else
    ...
    TriggerEvent("vrp-props:removeProp")
  end
...
AddEventHandler('attachedItems:block', function(status)
    ...
    elseif (hadattachitem) and not status then
        hadattachitem = false
        TriggerEvent("vrp-inventory:itemUsed",storedpItem, storedpInfo)
    end
end)
...
AddEventHandler("vrp-inventory:itemUsed", function(pItem, pInfo)
  ...
  if not hasSquidMaskOn then
    hasSquidMaskOn = true
    -- SetPedComponentVariation(PlayerPedId(), 7, chainValue, 0, 0)
    TriggerEvent("vrp-props:attachProp", model.model, 31086, model.x, model.y, model.z, -13.0, -86.0, -165.0, true, true)
  else
    hasSquidMaskOn = false
    TriggerEvent("vrp-props:removeProp")
  end
...
RegisterNetEvent("vrp-inventory:itemCheck", function(item)
  local model = chainModels[item]
  if not model then return end
  local qty = exports["vrp-inventory"]:hasEnoughOfItem(item, 1)
  if qty then return end
  hasCgChainOn = false
  TriggerEvent("vrp-props:removeProp")
end)
local function hasChainAccess()
  local hasAccess = exports['vrp-business']:HasPermission('jeweled_dragon', 'craft_access')
...
AddEventHandler("vrp-clothing:openCGChainCrafting", function()
  if not hasChainAccess() then return end
  TriggerEvent("server-inventory-open", "42087", "Craft")
end)
AddEventHandler("vrp-clothing:infuseChainWithGems", function()
  if not hasChainAccess() then return end
  local cid = exports["isPed"]:isPed("cid")
  NPX.Procedures.execute("vrp-gallery:infuseCGChain", cid)
end)

-- Citizen.CreateThread(function()
--   TriggerEvent("vrp-props:attachProp", "np_squid_vip", 31086, 0.03, 0.03, 0.0, -13.0, -86.0, -165.0, true, true)
-- end)
