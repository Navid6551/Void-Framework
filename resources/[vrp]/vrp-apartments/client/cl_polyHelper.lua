...
function polyHelperEnter(apartmentLocNumber,apartmentNumber)
    ...
    listening = true
    exports["vrp-ui"]:showInteraction(getInteractionMessage(apartmentLocNumber,apartmentNumber))
...
local function showApartmentMenu(apartmentLocNumber,apartmentNumber)
    ...
    for k,v in pairs(Apart.currentRoomLocks[apartmentLocNumber]) do
        if v == false then
            unlockedApartments[#unlockedApartments + 1] = { title = _L("apartments-ui-apartment", "Apartment") .. " # "..k, action = "vrp-ui:apartmentsContext", key = {"EnterUnlocked",apartmentLocNumber,k} }
        end 
    end
  ...
  RegisterUICallback("vrp-ui:apartmentsContext", function(data, cb)
    ...
    if action == "locks" then
        ...
        exports["vrp-ui"]:showInteraction(getInteractionMessage(globalLocNumber,globalNumber))
    end
