...
RegisterNetEvent("vrp-infinity:interiors:exitInterior")
AddEventHandler("vrp-infinity:interiors:exitInterior", function(pID)
    local src = source
...
RegisterNetEvent("vrp-infinity:interiors:enteredInterior")
AddEventHandler("vrp-infinity:interiors:enteredInterior", function(pID)
    local src = source
    local state = Player(src).state
    local newWorld = GetPlayerRoutingBucket(src)
...
RPC.register("vrp-infinity:setWorld", function(src, bucket, pType, pState)
    local state = Player(src).state
    local newWorld = 0 
