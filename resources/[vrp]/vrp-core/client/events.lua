...
RegisterNetEvent('QBCore:Client:VehicleInfo', function(info)
    ...
    if GetResourceState('vrp-vehiclekeys') == 'started' then
        hasKeys = exports['vrp-vehiclekeys']:HasKeys()
    end
...
RegisterNetEvent('QBCore:Client:UseItem', function(item)
    QBCore.Debug(string.format('%s triggered QBCore:Client:UseItem by ID %s with the following data. This event is deprecated due to exploitation, and will be removed soon. Check vrp-inventory for the right use on this event.', GetInvokingResource(), GetPlayerServerId(PlayerId())))
    QBCore.Debug(item)
end)
...
AddEventHandler("onClientResourceStart", function (resourceName)
    ...
    local config = exports["vrp-config"]:GetModuleConfig("misc")
...
RegisterNetEvent("vrp-commands:meCommand")
AddEventHandler("vrp-commands:meCommand", function(user, msg, duration)
    if UseNUIRender then
        local uid = exports["vrp-scenes"]:AddPlayerDrawEntry(msg, user)

        Citizen.SetTimeout(duration or 5000, function()
            exports["vrp-scenes"]:RemoveDrawEntry(uid)
        end)
