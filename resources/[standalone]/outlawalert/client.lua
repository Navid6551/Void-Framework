...
local ticksUntilFade = 0

RegisterNetEvent('DoHudTextCoords')
AddEventHandler('DoHudTextCoords', function(text,obj)
    if HudStage > 2 then return end
    ticksUntilFade = 600
    if #text > 50 then ticksUntilFade = 700 end
    msgCount2 = msgCount2 + 0.22
    local mycount2 = msgCount2

    scary2 = ticksUntilFade - (msgCount2 * 100)
    TriggerEvent("scaryLoop2")
    local textWidth = GetDrawTextWidth(text, drawFont, 0.35) + 0.011
    while ticksUntilFade > 0 do

        ticksUntilFade = ticksUntilFade - 1

        local plyCoords2 = GetEntityCoords(obj)

        output = ticksUntilFade
        ...
        if not isInVehicle and GetFollowPedCamViewMode() == 0 then
            DrawText3DTest(plyCoords2["x"],plyCoords2["y"],plyCoords2["z"]+(mycount2/2) - 0.2, text, output, textWidth)
        elseif not isInVehicle and GetFollowPedCamViewMode() == 4 then
            DrawText3DTest(plyCoords2["x"],plyCoords2["y"],plyCoords2["z"]+(mycount2/7) - 0.1, text, output, textWidth)
        elseif GetFollowPedCamViewMode() == 4 then
            DrawText3DTest(plyCoords2["x"],plyCoords2["y"],plyCoords2["z"]+(mycount2/7) - 0.2, text, output, textWidth)
        else
            DrawText3DTest(plyCoords2["x"],plyCoords2["y"],plyCoords2["z"]+mycount2 - 1.25, text, output, textWidth)
        end
...
local setFont = function (pFont)
    local defaultFont = pFont
    ...
    local customFonts = exports["assets_fonts"]:GetAvailableFonts()
...
AddEventHandler("vrp-config:configLoaded", function (configId, config)
    ...
    local defaultFont = config["game.font"]
...
AddEventHandler("onClientResourceStart", function (resourceName)
    ...
    -- local defaultFont = exports['vrp-config']:GetMiscConfig("game.font")

    -- setFont(defaultFont)
end)

AddEventHandler("vrp-fonts:fontsLoaded", function ()
    -- local defaultFont = exports['vrp-config']:GetMiscConfig("game.font")

    -- setFont(defaultFont)
end)

function GetDrawTextWidth(text, font, scale)
...
function DrawText3DTest(x,y,z, text, alpha, width)
    local onScreen,_x,_y=World3dToScreen2d(x,y,z)

    if onScreen then
        SetTextScale(0.35, 0.35)
        SetTextFont(drawFont)
        ...
        SetTextCentre(1)
        AddTextComponentString(text)
        SetTextColour(255, 255, 255, math.min(alpha, 255))

        DrawText(_x,_y)
        DrawRect(_x + 0.0014,_y + 0.0125, width, 0.03, 11, 1, 11, math.min(alpha, 115))
    end
end
