...
RegisterNetEvent("vrp-editor:loadEditor")
AddEventHandler("vrp-editor:loadEditor", function(data)
    loadEditor(data)
end)
...
AddEventHandler('__cfx_nui:close_ui', function(data, callback)
    ...
    TriggerEvent("vrp-editor:closeEditor",CURRENT_NAME)
    closingUI()
    menuOpen = false
    clearActionObjects()
    destroyZone()
    ...
    if IsModelValid(modelHash) and LoadModel(modelHash) then
        ...
        if DoesEntityExist(entityId) then
            ...
            callback({
                exists = true,
                hash = modelHashName,
                name = GetEntityArchetypeName(entityId),
                id = entityId,
                alpha = entityAlpha,
                x = playerCoords.x,
                y = playerCoords.y,
...
AddEventHandler('__cfx_nui:select_entity', function(data, callback)
        ...
        callback({
            hit = hit,
            entity = entityHit,
            index = getCurrentEntityIndex(entityHit)
        })
end)
