...
AddEventHandler("housing:inventory", function()
    ...
    if max.canHaveInventory then
        ...
        local ownerOnlyStash = exports["vrp-config"]:GetMiscConfig("housing.stash.ownerOnly")
...
function exitingBuilding(overrideVector, skipExit)
    ...
    TriggerServerEvent("vrp-housing:halloween:playerExitProperty", id)

    if id then
        TriggerEvent("vrp-editor:destroyName","housing-"..id)
    end
    ...
    if not skipExit then
        exports["vrp-build"]:getModule("func").exitCurrentRoom(overrideVector)
    end
    Housing.currentAccess = nil
    Housing.currentlyInsideBuilding = false
end

AddEventHandler('vrp-housing:exitBuilding', exitingBuilding)
...
AddEventHandler("housing:kickFromHouse", function(propertyID)
    ...
    if id == propertyID then
        DoScreenFadeOut(1)
        exports["vrp-build"]:getModule("func").exitCurrentRoom(Housing.info[id][1])
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)
...
function Housing.func.loadInteractions(model,overWrite)
    ...
    local isBuiltCoords = exports["vrp-build"]:getModule("func").addInteractionPoints(model,interactionPoints,overWrite)
end
...
function InteractionPressed()
    if exports["vrp-objects"]:IsPlacingObject() then return end
...
function isCurrentPropertyLockdown()

    local propertyID = Housing.currentHousingInteractions.id
    if (propertyID) then
        local myjob = exports["isPed"]:isPed("myjob")
        if Housing.currentHousingLockdown == nil then return false end
        if Housing.currentHousingLockdown[propertyID] == nil then return false end
        if Housing.currentHousingLockdown[propertyID] and (myjob == "police" or myjob == "judge") then 
            return true
        end
