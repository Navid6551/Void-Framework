local function hasScale()
  return exports['vrp-inventory']:hasEnoughOfItem('qualityscales', 1, false) or
...
AddEventHandler('vrp-inventory:itemUsed', function(item, passedItemInfo, inventoryName, slot)
  if item == 'driedbud' then
    local parsedInfo = json.decode(passedItemInfo)
    local finished = exports['vrp-taskbar']:taskBar(5000, _L('weed-pack-bud', 'Packing'), false, true, false, false, nil, 5.0, PlayerPedId())
    if finished == 100 and exports['vrp-inventory']:hasEnoughOfItem(item, 1, false, true, { quality = parsedInfo.quality }) then
  ...
  if item == 'weedpackage' then
    TriggerEvent('vrp-weed:prepareBaggies', {}, PlayerPedId())
    -- local parsedInfo = json.decode(passedItemInfo)
    -- local budData = {
  ...
  if item == 'smallbud' then
    ...
    local hasPaper = exports['vrp-inventory']:hasEnoughOfItem('rollingpaper', amount, false)
    ...
    local finished = exports['vrp-taskbar']:taskBar(15000, _L('weed-roll-bud', 'Packing Joints'), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 and exports['vrp-inventory']:hasEnoughOfItem(item, 1, false, true) then
  ...
  if item == 'weedbaggie' then
    ...
    loadAnimDict(rollAnimDict)
    TaskPlayAnim(PlayerPedId(), rollAnimDict, rollAnim, 8.0, 1.0, -1, 17, 0, 0, 0, 0)
    local finished = exports['vrp-taskbar']:taskBar(5000, _L('weed-roll-joint', 'Rolling Joints'), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 and exports['vrp-inventory']:hasEnoughOfItem(item, 1, false, true) then
      rollJoints(item, amount, json.decode(passedItemInfo))
    end
