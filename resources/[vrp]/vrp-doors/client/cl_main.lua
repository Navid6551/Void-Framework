...
RegisterNetEvent('vrp-doors:initial-lock-state')
AddEventHandler('vrp-doors:initial-lock-state', function(pDoors)
...
RegisterNetEvent('vrp-doors:change-lock-state')
AddEventHandler('vrp-doors:change-lock-state', function(pDoorId, pDoorLockState, pDoorForceUnlock)
...
local function listenForKeypress()
    ...
    Citizen.CreateThread(function()
        ...
        if not hasAccess and currentDoorLockState and not isHidden then
            exports["vrp-ui"]:showInteraction('Locked', 'error')
        end
        ...
        while listening do
            ...
            if currentDoorLockState ~= newLockState then
                if #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                    ...
                    if hasAccess and not isHidden then
                        exports["vrp-ui"]:showInteraction(("[E] %s"):format(newLockState and 'Locked' or 'Unlocked'), newLockState and 'error' or 'success')
                    else
                    end
                else
                    idle = 100
            ...
            if currentDoorId ~= nil and IsControlJustReleased(0, 38) and #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                    ...
                    if hasAccess then
                        ...
                        TriggerServerEvent("vrp-doors:change-lock-state", currentDoorId, not currentDoorLockState)
                    end
            end
        ...
        exports["vrp-ui"]:hideInteraction((not hasAccess or newLockState) and 'error' or 'success')
    end)
end
...
local function printDoorData()
    ...
    TriggerServerEvent("vrp-doors:printDoorData", lastDoorData)
end

Citizen.CreateThread(function()
  if GetConvar('sv_environment', 'prod') == 'debug' then
    exports['vrp-keybinds']:registerKeyMapping('', 'zzAdmin', 'Print Door Data', '+printDoorData', '-printDoorData', 'E');
...
AddEventHandler("vrp-doors:doorKeyFob", function()
    ...
    if currentZone ~= nil and bollards[currentZone].inside then
        ...
        local doorEntity = exports['vrp-target']:GetEntityPlayerIsLookingAt(10.0, 2.0, 16)
    ...
    TriggerServerEvent('vrp-fx:sound:playGateLocking', isLocked, isSpecial)

    TriggerServerEvent("vrp-doors:change-lock-state", doorId, isBollard and (not isLocked and 6 or 0) or (not isLocked))
end)

Citizen.CreateThread(function()
    exports["vrp-polyzone"]:AddBoxZone("mrpd_bollards_01", vector3(411.66, -1027.95, 29.24), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    exports["vrp-polyzone"]:AddBoxZone("mrpd_bollards_02", vector3(411.66, -1020.09, 29.34), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    doors = DOOR_CONFIG
    TriggerServerEvent("vrp-doors:request-lock-state")
end)

RegisterNetEvent("vrp-doors:add")
AddEventHandler("vrp-doors:add", function(pArgs, pEntity, pContext)
  if GetConvar("sv_environment", "prod") == "debug" then
    TriggerServerEvent("vrp-door:add", GetEntityCoords(pEntity), GetEntityModel(pEntity))
  end
end)
...
Citizen.CreateThread(function()
    ...
    for _, door in pairs(DOOR_CONFIG) do
        TriggerEvent("vrp-cleanup:addBlacklistedProp", door.model)
    end
end)
...
--     TriggerServerEvent("vrp-doors:save-config", doorsCache)
-- end)
