...
Citizen.CreateThread(function ()
    for _, target in ipairs(Zones) do
        VPX.Zones.addBoxZone(target.id, target.zone, target.coords, target.length, target.width, target.options)
    end

    for _, target in ipairs(Targets) do
        VPX.Zones.addBoxTarget(target.id, target.event, target.coords, target.length, target.width, target.options)
    end
end)