...
-- dial from phone
RegisterUICallback("vrp-ui:callStart", function(data, cb)
  local target_number = data.number
  RPC.execute("phone:callStart", target_number)
...
AddEventHandler("phone:call911", function(args, pMakeCall, pAnon)
  ...
  if pMakeCall then
    availableNumber = RPC.execute("vrp-dispatch:get911Number")
    if not availableNumber then
      TriggerEvent("DoLongHudText", "No operators available, use /911o!", 2)
      return
    end
    RPC.execute("phone:callStart", tostring(availableNumber), '911', "911 - " .. tostring(phoneNumber), "911")
    TriggerEvent("attachItemPhone", "phone01")
  end
...
-- answer from phone
RegisterUICallback("vrp-ui:callAccept", function(data, cb)
  local call_id = data.meta.callId
  RPC.execute('phone:callAccept', call_id)
...
-- end from phone
RegisterUICallback("vrp-ui:callEnd", function(data, cb)
  local call_id = data.meta.callId
  RPC.execute('phone:callEnd', call_id)
  cb({ data = {}, meta = { ok = success, message = message }})
end)

RegisterUICallback("vrp-ui:togglePhoneNotificationSounds", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = 'done' }})
  RPC.execute("vrp-phone:silenceCallsForServerId")
end)
...
function HasValidPhoneItem()
  ...
  for _, item in ipairs(phoneItems) do
    if exports["vrp-inventory"]:hasEnoughOfItem(item, 1, false, true) then
      return true  
    end
  end
...
-- init
Citizen.CreateThread(function()
  exports["vrp-keybinds"]:registerKeyMapping("","Phone", "Call Answer", "+answerPhoneCall", "-answerPhoneCall")
  RegisterCommand('+answerPhoneCall', answerPhoneCall, false)
  RegisterCommand('-answerPhoneCall', function() end, false)
  exports["vrp-keybinds"]:registerKeyMapping("","Phone", "Call End", "+endPhoneCall", "-endPhoneCall")
