...
function OnDeath(pMinor)
    ...
    -- local newDeaths = NPX.Procedures.execute('vrp-ragdoll:addDeath', pMinor)
    -- SetDeaths(newDeaths)
end
...
AddEventHandler('vrp-spawn:characterSpawned', function()
    -- local deaths = NPX.Procedures.execute('vrp-ragdoll:fetchDeaths')
    -- SetDeaths(deaths)
end)

RegisterNetEvent('vrp-ragdoll:refreshDeaths', function(deaths)
    SetDeaths(deaths)
end)

AddEventHandler('vrp-outbreak:disableSystems', function(pDisabled)
    disableReduceFactor = pDisabled
    currentPercent = 100
end)
