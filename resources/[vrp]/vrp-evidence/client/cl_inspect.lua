...
local activeEvidence = CacheableMap(function (ctx)
    local success, evidence = RPC.execute("vrp-evidence:fetchEvidence")
...
local collectEvidence = function (pCoords)
    ...
    RPC.execute('vrp-evidence:clearEvidence', pCoords, evidence)
end

RegisterNetEvent('vrp-evidence:clearCache', function ()
...
RegisterNetEvent('vrp-evidence:startInspectView', function ()
...
RegisterNetEvent('vrp-evidence:stopInspectView', function ()
...
RegisterCommand('-collectEvidence', function ()
    ...
    local hasGatheringKit = exports['vrp-inventory']:hasEnoughOfItem('gatheringkit', 1, false)
    ...
    local finished = exports["vrp-taskbar"]:taskBar(timer,"Picking Up Evidence","What?",true)
...
Citizen.CreateThread(function ()
    exports['vrp-keybinds']:registerKeyMapping('', 'Evidence', 'Pick Up Evidence', '+collectEvidence', '-collectEvidence', 'E')
    ...
    while true do
        ...
        if canInspect and not InspectViewEnabled then
            ...
            TriggerEvent('vrp-evidence:startInspectView')
        elseif not canInspect and InspectViewEnabled then
            idle = 100

            TriggerEvent('vrp-evidence:stopInspectView')
        end
