...
RegisterNetEvent('vrp-hud:settings', function(key, value)
    if key ~= 'vrp-preferences' then
        return
    end
    ...
    exports['vrp-voice']:SetSubmixBalance('radio', radioBalance)
    exports['vrp-voice']:SetSubmixBalance('radio_medium_distance', radioBalance)
    exports['vrp-voice']:SetSubmixBalance('radio_far_distance', radioBalance)
    exports['vrp-voice']:SetSubmixBalance('atc', radioBalance)
    exports['vrp-voice']:SetSubmixBalance('drivethru', radioBalance)

    exports['vrp-voice']:SetSubmixBalance('phone', value['phone.balance'])
end)
...
RegisterNetEvent('vrp-admin:cloakStatus', function (pCloaked)
    isCloaked = pCloaked
end)
...
function CanUseFrequency(pFrequency, pNotify, jobOverWrite)
    ...
    local hasPDRadio = exports["vrp-inventory"]:hasEnoughOfItem("radio", 1, false, true)
    local hasCivRadio = exports["vrp-inventory"]:hasEnoughOfItem("civradio", 1, false, true)
...
AddEventHandler('playerSpawned', function()
    ...
    local initialHudSettings = exports["vrp-ui"]:GetPreferences()
    ...
    if initialHudSettings['phone.balance'] ~= nil then
        exports['vrp-voice']:SetSubmixBalance('phone', initialHudSettings['phone.balance'])
    end
