...
local function processStressBlock()
    TriggerEvent("vrp-buffs:applyBuff", "joints", { { buff = "stressblock", percent = 1.0 } })
end

function IsAllowedToCraft(pItemId)
    local characterId = exports["isPed"]:isPed("cid")

    -- local config = exports["vrp-config"]:GetModuleConfig("vrp-inventory:crafting:permissions")
...
AddEventHandler("vrp-katana:removeEquippedKatana", function()
...
RegisterCommand('+useKeyFob', function()
    if Throttled("vrp-doors:doorKeyFob", 1000) then return end
    TriggerEvent("vrp-doors:doorKeyFob")
end, false)
RegisterCommand('-useKeyFob', function() end, false)

Citizen.CreateThread(function()
    exports["vrp-keybinds"]:registerKeyMapping("", "Vehicle", "Door Keyfob", "+useKeyFob", "-useKeyFob", "Y")
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
        ...
        if GetEntityModel(PlayerPedId()) == `Mr_kebun` then
          if tonumber(ItemInfo.quality) > 0 then
              if not isKatanaEquipped then
                ...
                Citizen.CreateThread(function()
                    Citizen.Wait(1000)
                    TriggerServerEvent("vrp-katana:cursedKatanaEquip", GetEntityCoords(PlayerPedId()))
                    TriggerEvent("vrp-katana:cursedKatanaEquipC")
                end)
              else
                isKatanaEquipped = false
              end
    ...
    if stolenItems[itemid] and exports["vrp-npcs"]:isCloseToPawnPed() then
    ...
    if (itemid == "pdbadge") then
        RPC.execute("vrp-gov:police:showBadge", json.decode(ItemInfo.information))
    end
    ...
    if (itemid == "lsdtab" or itemid == "badlsdtab") then
        ...
        local finished = exports["vrp-taskbar"]:taskBar(3000,"Placing LSD Strip on 👅",false,false,playerVeh)
    ...
    if (itemid == "matrixredpill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["vrp-taskbar"]:taskBar(3000,"Taking the Red Pill",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",24,1200)
    ...
    if (itemid == "decryptersess" or itemid == "decrypterfv2" or itemid == "decrypterenzo") then
      if (#(GetEntityCoords(player) - vector3( 1275.49, -1710.39, 54.78)) < 3.0) then
          local finished = exports["vrp-taskbar"]:taskBar(25000,"Decrypting Data",false,false,playerVeh)
          if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "weedoz") then
      ...
      local finished = exports["vrp-taskbar"]:taskBar(5000,"Packing Q Bags",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    --     local finished = exports["vrp-taskbar"]:taskBar(1000,"Packing Joint",false,false,playerVeh)
    --     if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    --     local finished = exports["vrp-taskbar"]:taskBar(1000,"Rolling Joints",false,false,playerVeh)
    --     if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "lighter") then
        TriggerEvent("animation:PlayAnimation","lighter")
          local finished = exports["vrp-taskbar"]:taskBar(2000,"Starting Fire",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "joint" or itemid == "joint2" or itemid == "customjointitem") then
        local finished = exports["vrp-taskbar"]:taskBar(2000,"Smoking Joint",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if itemid == "weedbong" then
        ...
        if currentHits == 0 then
            ...
            local finished = exports["vrp-taskbar"]:taskBar(12000, "Hitting Bong", false, false, playerVeh)
    ...
    if (itemid == "Gruppe6Card") then
        ...
        if targetVehicle ~= 0 and GetHashKey("stockade") == GetEntityModel(targetVehicle) and countpolice > 2 then
            ...
            if aDist < 2.0 then
                ...
                local finished = exports["vrp-taskbar"]:taskBar(timeToOpen,"Unlocking Vehicle",false,false,playerVeh)
                if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                    remove = true
    ...
    if (itemid == "heavyammo") then
        local finished = exports["vrp-taskbar"]:taskBar(5000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "pistolammo") then
        local finished = exports["vrp-taskbar"]:taskBar(2500,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "flamethrowerammo") then
        local finished = exports["vrp-taskbar"]:taskBar(5000,"Reloading flamethrower",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "huntingammo") then
        local finished = exports["vrp-taskbar"]:taskBar(10000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "rpgammo") then
        local finished = exports["vrp-taskbar"]:taskBar(25000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "taserammo") then
        local finished = exports["vrp-taskbar"]:taskBar(2000,"Reloading",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "empammo") then
        local finished = exports["vrp-taskbar"]:taskBar(30000,"Recharging EMP",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "armor" or itemid == "pdarmor") then
        -- exports["rpemotes"]:EmoteCommandStart("adjusttie")
        local finished = exports["vrp-taskbar"]:taskBar(5000,"Armor",true,false,playerVeh)
    ...
    if (itemid == "cbrownie" or itemid == "cgummies") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["vrp-taskbar"]:taskBar(3000,"Consuming edibles 😉",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "gunpaintbucket") then
        if not (hasEnoughOfItem("refinedsteels", 3, false) and hasEnoughOfItem("refinedplastics", 3, false)) then
            ...
            local finished = exports["vrp-taskbar"]:taskBar(5000, "Filling Paint Gun", true, false)
    ...
    if (itemid == "bodybag") then
        local finished = exports["vrp-taskbar"]:taskBar(10000,"Opening bag",true,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
    ...
    if (itemid == "metamorphine_box") then
        local finished = exports["vrp-taskbar"]:taskBar(10000,"Unpacking box",true,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
    ...
    if (itemid == "bodygarbagebag") then
            local finished = exports["vrp-taskbar"]:taskBar(10000,"Opening trash bag",false,false,playerVeh)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                remove = true
    ...
    if (itemid == "newaccountbox") then
        local finished = exports["vrp-taskbar"]:taskBar(5000,"Opening present",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "foodsupplycrate") then
        ...
        local finished = exports["vrp-taskbar"]:taskBar(20000,"Opening the crate (ESC to Cancel)",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
    ...
    if (itemid == "fishingtacklebox") then
        local finished = exports["vrp-taskbar"]:taskBar(5000,"Opening",true,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "housesafe") then
        if not hasEnoughOfItem("heavydutydrill", 1, false) then
            ...
            local finished = exports["vrp-taskbar"]:taskBar(30000, "Cracking Safe...", true, false)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                remove = true
                TriggerServerEvent('loot:useItem', itemid)
            end
    ...
    if (itemid == "fishingchest") then
        local finished = exports["vrp-taskbar"]:taskBar(5000, "Opening Chest", true, false)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "repcrate") then
        local finished = exports["vrp-taskbar"]:taskBar(30000, "Opening...", true, false)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "babyoil") then
        local finished = exports["vrp-taskbar"]:taskBar(10000,"Applying baby oil to dome",true,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
    ...
    if (itemid == "organcooler") then
        local finished = exports["vrp-taskbar"]:taskBar(5000,"Opening cooler",true,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
    ...
    if (itemid == "Bankbox") then
        if (hasEnoughOfItem("locksystem",1,false)) then
            local finished = exports["vrp-taskbar"]:taskBar(10000,"Opening bank box.",false,false,playerVeh)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                remove = true
    ...
    if (itemid == "Securebriefcase") then
        if (hasEnoughOfItem("Bankboxkey",1,false)) then
            local finished = exports["vrp-taskbar"]:taskBar(5000,"Opening briefcase.",false,false,playerVeh)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                remove = true
    ...
    if (itemid == "Largesupplycrate") then
        if (hasEnoughOfItem("2227010557",1,false)) then
            local finished = exports["vrp-taskbar"]:taskBar(15000,"Opening supply crate.",false,false,playerVeh)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                remove = true
    ...
    if (itemid == "xmasgiftcoal") then
        local finished = exports["vrp-taskbar"]:taskBar(15000, "Opening Gift", false, false, playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "emptyballoon") then
        if (hasEnoughOfItem("nitrous",1,false)) then
            local finished = exports["vrp-taskbar"]:taskBar(5000,"Filling Balloon.",false,false,playerVeh)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                remove = true
    ...
    if itemid == "coughsyrup" or itemid == "sprunk" then
        ...
        if hasEnoughOfItem(mixWith1, 1, false) and hasEnoughOfItem(mixWith2, 1, false) then
            local finished = exports["vrp-taskbar"]:taskBar(10000, "Mixing", false, false, playerVeh)
            if finished == 100 and hasEnoughOfItem(itemid, 1, false) then
                remove = true
    ...
    if (itemid == "ollyrancher") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["vrp-taskbar"]:taskBar(3000,"Eating Olly Rancher.",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
        end
    end
    ...
    if (itemid == "Peecup_empty") then
        ...
        local finished = exports["vrp-taskbar"]:taskBar(10000, "Peeing in cup.", false, false, playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            local cid = exports["isPed"]:isPed("cid")
            TriggerServerEvent("vrp-drugeffects:useEmptyPeeCup", cid)
        end
        else
    ...
    if (itemid == "megaphone") then
      TriggerEvent("vrp-usableprops:megaphone")
    end

    if (itemid == "nitrous") then
        local currentVehicle = GetVehiclePedIsIn(player, false)
        local isElectric = exports['vrp-vehicles']:IsVehicleElectric(currentVehicle)
        print(isElectric)
        if not isElectric then
            ...
            if status == "Empty" then
                ...
                elseif not IsToggleModOn(currentVehicle, 18) then
                    ...
                    finished = exports["vrp-taskbar"]:taskBar(20000,"Nitrous")
    ...
    if (itemid == "ciggy") then
        local finished = exports["vrp-taskbar"]:taskBar(1000,"Lighting Up",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            Wait(300)
    ...
    if (itemid == "oxygentank" or itemid == "oxygentanknavy") then
        local finished = exports["vrp-taskbar"]:taskBar(30000,"Oxygen Tank",true,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "weightlosspill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["vrp-taskbar"]:taskBar(3000,"Losing weight! 💊",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerEvent("animation:PlayAnimation","cancel")
            Citizen.Wait(100)
    ...
    if (itemid == "Desomorphine") then
        ...
        if success then
            local cid = exports["isPed"]:isPed("cid")
            TriggerServerEvent("vrp-drugeffects:drugsTaken", cid, "desomorphine")
    ...
    if (itemid == "adderall_pill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["vrp-taskbar"]:taskBar(3000,"Gotta focus!",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            local drugEffect = true
    ...
    if (itemid == "test_pill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["vrp-taskbar"]:taskBar(3000,"Taking testosterone.",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
        end
    end
    ...
    if (itemid == "cokeline") then
        ...
        if itemInfo and itemInfo.quality > 0 then
            ...
            if currentUses == 0 then
                ...
                local finished = exports["vrp-ui"]:taskBarSkill(3000, math.random(15, 20))
                if (finished == 100 and hasEnoughOfItem(itemid, 1, false)) then
                    ...
                    TriggerEvent("Evidence:StateSet",2,1200)
                    TriggerEvent("Evidence:StateSet",6,1200)
                    NPX.Events.emitNet('vrp-fx:sound:playSoundFromSelf', 'SNIFFA', 'DLC_NIKEZ_GENERAL', 'GENERAL_GENERAL');
                    TaskItem("anim@amb@nightclub@peds@", "missfbi3_party_snort_coke_b_male3", 49, 2500, "Coke Gaming", fireEvent, false, itemid, playerVeh)
                    local cid = exports["isPed"]:isPed("cid")
                    TriggerServerEvent("vrp-drugeffects:drugsTaken", cid, "cocaine")
                    if isCocainium then
                        TriggerServerEvent("vrp-drugeffects:drugsTaken", cid, "trace radiation")
                    end
                end
            end
        end
    ...
    if (itemid == "crackpipe") then
        ...
        if itemInfo and itemInfo.quality > 0 then
            ...
            if currentUses == 0 then
                ...
                local finished = exports["vrp-ui"]:taskBarSkill(2400, math.random(12, 18))
                if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
                    currentUses = currentUses - 1
                    TriggerEvent("inventory:updateItem", itemid, slot, json.encode({ uses = currentUses }))
                    ...
                    local cid = exports["isPed"]:isPed("cid")
                    TriggerServerEvent("vrp-drugeffects:drugsTaken", cid, "crack")
                end
            end
        end    
    end
            ...
            if #currentPurities == 0 then
                ...
                if (finished == 100 and hasEnoughOfItem(itemid, 1, false)) then
                    ...
                    if success then
                        ...
                        local cid = exports["isPed"]:isPed("cid")
                        TriggerServerEvent("vrp-drugeffects:drugsTaken", cid, "meth")
                    end
    ...
    if string.match(itemid, "^experiment_%d+$") then
        TriggerEvent("animation:PlayAnimation", "pill")
        local finished = exports["vrp-taskbar"]:taskBar(3000, "Downing content of bottle. 💊", false, false, playerVeh)
        if finished == 100 and hasEnoughOfItem(itemid, 1, false) then
    ...
    if itemid == "pillbox" then
        local finished = exports["vrp-taskbar"]:taskBar(5000, "Opening pill box", false, false, playerVeh)
        if finished == 100 and hasEnoughOfItem(itemid, 1, false) then
            for i = 1, 9 do
    ...
    if itemid == "boggsproteinbar" then
        ...
        if success then
            TriggerEvent("vrp-buffs:applyBuff", "boggs", { { buff = "bikeStats", percent = 1.0, timeOverride = 60 * 1000 } })
    ...
    if itemid == "kdragonwater" then
        ...
        AttachPropAndPlayAnimation(
          ...
          6000,
          "Drink",
          "vrp-katana:transformKFromDrink",
          true,
          itemid,
          playerVeh
        )
    ...
    if itemid == "ketamine" then
        local finished = exports["vrp-ui"]:taskBarSkill(2000, math.random(10, 15))
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            TriggerEvent("attachItemObjectnoanim","drugpackage01")
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",6,1200)
    ...
    if (itemid == "shitlockpick") then
        ...
        local finished = exports["vrp-ui"]:taskBarSkill(1000, math.random(7, 12))
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            TriggerEvent("vrp-police:cuffs:uncuff")
        end
        lockpicking = false
        remove = true
    end
    ...
    if (itemid == "harness") then
        ...
        finished = exports["vrp-taskbar"]:taskBar(20000,"Harness")
    ...
    if itemid == "roostertea" then
        AttachPropAndPlayAnimation("amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49,15000,"Drink","vrp-roostersrest:drinkTea",true,itemid,playerVeh)
    end
    ...
    if itemid == "franksflute" then
      TriggerServerEvent("vrp-inventory:franksFlute", GetEntityCoords(PlayerPedId()))
    ...
    if itemid == "tattooremover" then
        local target = exports['vrp-target']:GetCurrentEntity()
        --local target = PlayerPedId()
        if IsEntityAPed(target) and IsPedAPlayer(target) then
            TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_WELDING", 0, true)
            local finished = exports["vrp-taskbar"]:taskBar(60000, "Removing tattoos")
    ...
    if itemid == "buddhaletter" then
        if GetEntityModel(PlayerPedId()) == `ig_buddha` or GetEntityModel(PlayerPedId()) == `g_m_m_buddha` then
            local finished = exports["vrp-taskbar"]:taskBar(5000,"Opening letter.",false,false,playerVeh)
            if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
        ...
        if hasEnoughOfItem("buddhablade",1,false) and hasEnoughOfItem("buddhaguard",1,false) and hasEnoughOfItem("buddhahilt",1,false) then
            if hasEnoughOfItem("buddhamedalion",1,false) then
                ...
                if #(playerPos - vector3(-172.54, 319.13, 87.35)) < 10 then
                    local finished = exports["vrp-taskbar"]:taskBar(30000,"Putting sword together.",false,false,playerVeh)
                    if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if itemid == "buddhashovel" then
        ...
        if #(playerPos - vector3(-1911.78, 1388.1, 219.35)) < 10 then
            if BuddhaIsItemFound1 == false then
                ...
                local finished = exports["vrp-taskbar"]:taskBar(15000, "Digging for something.", false, true, false, false, nil, 5.0, PlayerPedId())
                ClearPedTasks(PlayerPedId())
                if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    TriggerEvent("vrp-inventory:itemUsed", itemid, passedItemInfo, inventoryName, slot)
    TriggerServerEvent("vrp-inventory:itemUsed", itemid, passedItemInfo, inventoryName, slot)
...
AddEventHandler("vrp-inventory:attachPropPlayAnim", function(pType)
    ...
    TriggerEvent("vrp-buffs:itemUsedSuccess", success)
end)

RegisterNetEvent('randPickupAnim')
AddEventHandler('randPickupAnim', function()
    local ped = PlayerPedId()
    local _, isAnimalModel = pcall(function () return exports["vrp-misc-code"]:isAnimalModel(GetEntityModel(ped)) end)
...
RegisterNetEvent('vrp-business:repairItem')
AddEventHandler('vrp-business:repairItem', function()

    local info = GetItemInfo(1)
    local Quality = info.quality
    local QualityCheck = (Quality < 80 and Quality or false)
    local PercentRepair = 80 - Quality
    local itemListInfo = json.decode(exports["vrp-inventory"]:itemListInfo(info.item_id))
    local allowed = true
...
RegisterNetEvent('vrp-business:examineItem')
AddEventHandler('vrp-business:examineItem', function()
...
function TaskItem(dictionary,animation,typeAnim,timer,message,func,remove,itemid,playerVeh,itemreturn,itemreturnid,quality)
    ...
    if timer > 0 then
        local finished = exports["vrp-taskbar"]:taskBar(timer,message,true,false,playerVeh)
...
function checkForItems()
    ...
    for _, item in ipairs(items) do
        ...
        if hasItem and not PreviousItemCheck[item] then
            ...
            TriggerEvent('vrp-inventory:itemCheck', item, true, quantity)
        elseif not hasItem and PreviousItemCheck[item] then
            PreviousItemCheck[item] = false
            TriggerEvent('vrp-inventory:itemCheck', item, false, quantity)
        end
    end
end
...
AddEventHandler('inv:lockPick', function(isForced, inventoryName, slot, itemType)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 and (IsPedAPlayer(driverPed) or not IsPedDeadOrDying(driverPed)) then
            ...
            exports['vrp-vehicles']:LockpickVehicleDoor(targetVehicle, itemType, isForced, function(result)
                p:resolve(result)
            end)
            ...
            Citizen.SetTimeout(math.random(120, 7200) * 1000, function ()
                TriggerServerEvent('vrp:vehicles:hotPlate', plate, '10-60')
            end)

            if result.success and not isForced then
                exports['vrp-flags']:SetVehicleFlag(targetVehicle, 'isStolenVehicle', true)
            end

        lockpicking = false
    else
        if targetVehicle ~= 0 then
            if exports["vrp-vehicles"]:HasVehicleKey(targetVehicle) then lockpicking = false return end
            TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3.0, 'lockpick', 0.4)
            local triggerAlarm = GetVehicleDoorLockStatus(targetVehicle) > 1
            if triggerAlarm then
                SetVehicleAlarm(targetVehicle, true)
            ...
            exports['vrp-vehicles']:HotwireVehicle(targetVehicle, itemType, isForced, function(result)
                p:resolve(result)
            end)
...
function getInformationFromFirstItem(itemToFind)
  local foundItem = exports["vrp-inventory"]:GetInfoForFirstItemOfName(itemToFind)
...
RegisterUICallback("vrp-ui:inv:moneycaseAction", function(data, cb)
	cb({ data = {}, meta = { ok = true, message = "done" } })
  exports['vrp-ui']:closeApplication('textbox')
  RPC.execute("vrp-inventory:doMoneycaseAction", data)
end)

AddEventHandler("vrp-inventory:openGolfStore", function(name, info)
...
function openBurner(info)
    ...
    exports["vrp-ui"]:openApplication("burner", info)
end
...
AddEventHandler("ammonium_bicarbonate:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["vrp-taskbar"]:taskBar(5000 * quantity, "Breaking down into baggies...", false, false)
...
AddEventHandler("crackpipe:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["vrp-taskbar"]:taskBar(20000, "Packing crack pipe", false, false)
    if finished ~= 100 then return end
...
AddEventHandler("cokeline:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["vrp-taskbar"]:taskBar(15000, "Filling with " .. cConfig.fillString, false, false)
    if finished ~= 100 then return end
...
AddEventHandler("methpipe:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["vrp-taskbar"]:taskBar(10000, "Packing meth pipe", false, false)
    if finished ~= 100 then return end
...
AddEventHandler("weedbong:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["vrp-taskbar"]:taskBar(20000, "Packing Bong", false, false)
    if finished ~= 100 then return end
...
function notepadUsed()
    exports["vrp-ui"]:openApplication("notepad", {
        canSave = true
    })
    TriggerEvent("animation:PlayAnimation", "notepad")
end

RegisterUICallback('vrp-ui:cancelNotepadEmote', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })
    TriggerEvent("animation:PlayAnimation", "cancel")
end)

RegisterUICallback('vrp-ui:createNotepadNote', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })

    exports["vrp-ui"]:closeApplication("notepad")
...
function applyPaint(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo, minRng, maxRng)
    ...
    local prompt = exports['vrp-ui']:OpenInputMenu(elements, function(values)
    ...
    local finished = exports["vrp-taskbar"]:taskBar(15000, "Painting gun", false, false)
    if finished ~= 100 then return end
