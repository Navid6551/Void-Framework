...
QBCore.Commands.Add('ooc', Lang:t('command.ooc.help'), {}, false, function(source, args)
    ...
    local firstnamooc = char.firstname
    local lastnameooc = char.lastname
    TriggerClientEvent("chatMessage", -1, 'OOC '.. firstnamooc.. " " ..lastnameooc .. ' ['.. source .. '] ', 2, message, "ooc")
end, 'user')

QBCore.Commands.Add('whisper', "Ooc chat in range", {}, false, function(source, args)
    local message = table.concat(args, ' ')
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    for _, v in pairs(Players) do
        if #(playerCoords - GetEntityCoords(GetPlayerPed(v))) < 20.0 then
            TriggerClientEvent("chatMessage", v, 'WHISPER '.. firstnamooc.. " " ..lastnameooc .. ' ['.. source .. '] ', 2, message, "ooc")
        end
    end
end, 'user')
...
QBCore.Commands.Add('me', Lang:t('command.me.help'), { { name = Lang:t('command.me.params.message.name'), help = Lang:t('command.me.params.message.help') } }, false, function(source, args)
    ...
    TriggerClientEvent('vrp-commands:meCommand', -1, source, msg)
end, 'user')
