...
local InteractStartCorner = {
  ...
  options = {
    ...
    isEnabled = function(pEntity, pContext)
      local hasWeed = exports['vrp-inventory']:hasEnoughOfItem('weedbaggie', 1, false, true)
      local isCorneringAlready = exports['vrp-meth']:IsCornering()
...
local InteractCornerVehicle = {
  ...
  options = {
    distance = { radius = 5.0 },
    isEnabled = function(pEntity, pContext)
      local hasWeed = exports['vrp-inventory']:hasEnoughOfItem('weedpackage', 1, false, true)
      return hasWeed
    end,
  },
}
...
Citizen.CreateThread(function()
  exports['vrp-interact']:AddPeekEntryByEntityType(InteractCorner.group, InteractCorner.data, InteractCorner.options)
  exports['vrp-interact']:AddPeekEntryByEntityType(InteractCornerVehicle.group, InteractCornerVehicle.data, InteractCornerVehicle.options)
  exports['vrp-interact']:AddPeekEntryByEntityType(InteractStartCorner.group, InteractStartCorner.data, InteractStartCorner.options)
  exports['vrp-interact']:AddPeekEntryByEntityType(InteractStopCorner.group, InteractStopCorner.data, InteractStopCorner.options)
end)

AddEventHandler('vrp-weed:startCornering', function(pContext, pEntity)
  ...
  local canCorner, message, engMessage = RPC.execute('vrp-weed:startCorner', CurrentCornerCoords)
  ...
  Citizen.CreateThread(function()
    ...
    while CurrentlyCornering do
      ...
      if not foundPed then
        ...
        RPC.execute('vrp-weed:cornerPed', coords, NetworkGetNetworkIdFromEntity(foundPed),
...
AddEventHandler('vrp-weed:cornerSale', function(pContext, pEntity)
  if not DoesEntityExist(pEntity) == 1 or IsPedDeadOrDying(pEntity) then
    return
  end

  local hasBaggies = exports['vrp-inventory']:hasEnoughOfItem('weedbaggie', 1, true)
  ...
  RPC.execute('vrp-weed:cornerSyncHandoff', CurrentCornerCoords, NetworkGetNetworkIdFromEntity(pEntity))
  ...
  local didSale = RPC.execute('vrp-weed:cornerSale', CurrentCornerCoords, NetworkGetNetworkIdFromEntity(pEntity), CurrentCornerZone)
  if didSale then
...
AddEventHandler('vrp-weed:prepareBaggies', function(pContext, pEntity)
  if CornerConfig.RequireScales then
    local hasScale = exports['vrp-inventory']:hasEnoughOfItem('qualityscales', 1, false) or
                         exports['vrp-inventory']:hasEnoughOfItem('smallscales', 1, false)
  ...
  local hasBaggies = exports['vrp-inventory']:hasEnoughOfItem('emptybaggies', CornerConfig.BaggiesPerBrick / 4, false)
  if not hasBaggies then
  ...
  local finished =
      exports['vrp-taskbar']:taskBar(30000, _L('weed-corner-prepare', 'Preparing'), false, true, false, false, nil, 5.0, pEntity)
  if finished == 100 then
    local weedpackages = exports['vrp-inventory']:getItemsOfType('weedpackage', 1, true)
    if #weedpackages == 0 then
      return
    end
    -- local package = weedpackages[1]
    -- local info = json.decode(package.information)
    RPC.execute('vrp-weed:prepareBaggies')
  end
end)

AddEventHandler('vrp-weed:stopCornering', function(pContext, pEntity)
  TriggerEvent('DoLongHudText', _L('weed-corner-not-selling', 'No longer selling...'))
  stopCornering()
end)

RegisterNetEvent('vrp-weed:addCorneredPed')
AddEventHandler('vrp-weed:addCorneredPed', function(pPed)
  local ped = NetworkGetEntityFromNetworkId(pPed)
  ActivePeds[ped] = false
end)

RegisterNetEvent('vrp-weed:cornerSyncHandoff')
AddEventHandler('vrp-weed:cornerSyncHandoff', function(pPed)
  local ped = NetworkGetEntityFromNetworkId(pPed)
...
RegisterNetEvent('vrp-weed:cornerPed')
AddEventHandler('vrp-weed:cornerPed', function(pPed, pCornerCoords, pVehicle)
  local ped = NetworkGetEntityFromNetworkId(pPed)
  ActivePeds[ped] = true
  UsedPeds[ped] = true
  if NetworkHasControlOfEntity(ped) then
...
RegisterNetEvent('vrp-weed:cleanCornerPeds')
AddEventHandler('vrp-weed:cleanCornerPeds', function()
  UsedPeds = {}
end)
...
function stopCornering()
  ...
  RPC.execute('vrp-weed:stopCorner')
end
