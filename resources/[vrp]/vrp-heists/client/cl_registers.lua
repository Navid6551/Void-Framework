...
local function loopSkill(count)
  ...
  while loopCount < count do
    ...
    local finished = exports["vrp-ui"]:taskBarSkill(math.random(1400, 5000), math.random(7, 12))
...
local function breakInToRegister()
  ...
  skilling = false
  RPC.execute("complete:job", math.random(100, 150), 'h')
end

AddEventHandler("vrp-heists:breakInToRegister", function(pArgs, pEntity)
...
Citizen.CreateThread(function()
  exports["vrp-interact"]:AddPeekEntryByModel({ 303280717 }, {{
    event = "vrp-heists:breakInToRegister",
    id = "vrp-heists:breakInToRegister",
    icon = "hammer",
    label = "Smash!",
    parameters = {},
  }}, {
