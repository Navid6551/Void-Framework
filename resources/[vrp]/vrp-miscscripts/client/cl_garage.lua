...
local function attemptGarageAction(pAction)
  if pAction == 'enter' then
    ...
    if not exports['vrp-business']:IsEmployedAt('ron_corp') then return end
  end

  local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
  local netId = vehicle ~= 0 and NetworkGetNetworkIdFromEntity(vehicle) or nil

  VPX.Procedures.execute('vrp-miscscripts:rgarage:attemptGarage', netId, pAction)
end
...
AddEventHandler('vrp-polyzone:enter', function(pZone, pData)
  ...
  if pZone == 'garage_display' and pData.id == 'business_ron_garage' then
    insideGarage = true

    startGarageExitListener()
  end

  if pZone == 'ron_garage_entrance' then
    if insideGarageEntrance then return end

    insideGarageEntrance = true
  
    Citizen.CreateThread(function()
      while insideGarageEntrance do
        if IsControlJustReleased(0, 38) then
          attemptGarageAction('enter')
        end
...
AddEventHandler('vrp-polyzone:exit', function(pZone, pData)
  ...
  if pZone == 'ron_garage_entrance' then
    insideGarageEntrance = false
  end
end)

VPX.Events.onNet('vrp-miscscripts:rgarage:enterGarage', function(pIsDriver)
  enterGarage(pIsDriver)
end)

VPX.Events.onNet('vrp-miscscripts:rgarage:exitGarage', function(pIsDriver)
  exitGarage(pIsDriver)
end)

Citizen.CreateThread(function()
  exports['vrp-polyzone']:AddBoxZone('ron_garage_entrance', vector3(-856.79, -243.91, 39.15), 6.6, 4.4, {
    heading=26,
    minZ = 38.15,
    maxZ = 42.15,
