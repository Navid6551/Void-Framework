...
local rentalLocations = {
  ["vespucci_rentals"] = { name = "Vespucci Rentals", coords = vector3(117.84, -1079.95, 29.23), heading = 355.92, vehicles = vehicleList },
  ["paleto_rentals"] = { name = "Paleto Rentals", coords = vector3(-247.94, 6205.98, 30.82), heading = 139.67, vehicles = vehicleList },
  ["escalera_rentals"] = { name = "Escalera Rentals", coords = vector3(-906.03,-2333.41,6.59), heading = 239.0, vehicles = vehicleList },
  ["cayo_perico"] = {
    name = "Cayo Rentals",
    coords = vector3(4933.99, -5154.84, 0.47),
    heading = 67.6,
    vehicles = {
      { name = "Toro", model = "toro2", price = 25000 },
      { name = "Dinghy", model = "dinghy2", price = 10000 },
      { name = "Seashark", model = "seashark3", price = 5000 }
    }
  }
}
...
local function showVehicleMenu()
  ...
  data[#data + 1] = {
    ...
    title = 'Current Available Rentals',
    description = '',
    action = '',
    key = {},
    titleRight = '',
  ...
  for _, vehicle in pairs(rentalShop.vehicles) do
    data[#data + 1] = {
      title = vehicle.name,
      ...
      children = {
          {
            icon = 'check-square',
            title = _L('boatshop-confirm-purchase', 'Confirm Purchase'),
            action = 'vrp-ui:rentalPurchase',
            key = vehicle.model,
          },
          {
            icon = 'window-close',
            title = _L('boatshop-cancel-purchase', 'Cancel Purchase'),
            action = 'vrp-rentals:ui:cancelPurchase',
            key = {},
          },
      },
    }
...
RegisterUICallback("vrp-ui:rentalPurchase", function(data, cb)
  local rentalShop = getClosestRentalShop()
  if rentalShop == nil then return end

  local finished = exports["vrp-taskbar"]:taskBar(10000, "Renting", false, true, false, false, nil, 2.5, PlayerPedId())
  ...
  local d = nil
  for _, v in pairs(rentalShop.vehicles) do
  ...
  local rentalInfo = RPC.execute("vrp:vehicles:rentalSpawn", model, coords, rentalShop.heading)
  ...
  exports['vrp-flags']:SetVehicleFlag(veh, 'isRentalVehicle', true)
  ...
  TriggerEvent('player:receiveItem', 'rentalpapers', 1, false, metaData, metaData)
...
RegisterNetEvent("vrp-npcs:ped:vehiclekeeper")
AddEventHandler("vrp-npcs:ped:vehiclekeeper", function(pArgs, pEntity, pEntityFlags, pEntityCoords)
  local npcHashes = {
    [GetHashKey("npc_veh_rental")] = true,
    [GetHashKey("npc_veh_rental2")] = true,
    [GetHashKey("npc_veh_rental_cayo_boats")] = true,
  }
...
AddEventHandler("vrp-inventory:itemUsed", function(item, info)
  if item == "rentalpapers" then
      ...
      local vin = exports["vrp-vehicles"]:GetVehicleIdentifier(ent)
...
RegisterUICallback('vrp-rentals:ui:cancelPurchase', function(data, cb)
  cb({ data = {}, meta = { ok = false, message = nil } })
end)
