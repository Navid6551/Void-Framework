...
AddEventHandler('vrp-housing:hotreload', function()
    defaultStart()
end)

Citizen.CreateThread(function()
    GetPerms = VPX.Utils.cache(function ()
        local isDev = RPC.execute("vrp:admin:isAdmin",'dev')

        if not isDev then return false, nil end
...
Citizen.CreateThread(function()
    ...
    exports["vrp-keybinds"]:registerKeyMapping("","Housing", "Interact", "+propertyInteract", "-propertyInteract", "E")
...
RegisterUICallback("vrp-housing:handler", function(data, cb)
...
AddEventHandler("property:menuAction", function(pData)
    ...
    if isComplete and dist <= 3.0 then
        ...
        elseif action == "forfeit" then
            ...
            exports["vrp-ui"]:showContextMenu(MenuData["property_check"])
        end
    else
...
function hasPermissionInProperty()
    ...
    if exports['vrp-business']:HasPermission(Housing.currentHousingInteractions.buisness, 'craft_access') then
        return true
    end
end

function isPropertyRaidable()
    if not Housing.currentlyInsideBuilding then
        return false
    end
    ...
    if (not isPropertyShop(propertyID)) then
        return false
    end

    if isCurrentPropertyLockdown() then
        return true
    end
end
...
function hasRealtorAccess(checkHire, orDev)
    local hasAccess = false

    if orDev then
        if GetPerms.get()[1] then
            return true;
        end
    end

    if checkHire then
        local realtor = exports['vrp-business']:HasPermission('smol_dick_realtors', 'hire')
        local statecontracting = exports['vrp-business']:HasPermission('statecontracting', 'hire')

        hasAccess = realtor or statecontracting
    else
        local realtor = exports["vrp-business"]:IsEmployedAt("smol_dick_realtors")
        local statecontracting = exports["vrp-business"]:IsEmployedAt("statecontracting")
