...
function GetPedContext(pEntity, pContext)
    ...
    if pContext.flags['isJobEmployer'] then
        pContext.job = exports['vrp-jobs']:GetNPCJobData(npcId)
    end
end
...
function GetEntityContext(pEntity, pEntityType, pEntityModel)
    ...
    if context.type == 1 then
        context.flags = exports['vrp-flags']:GetPedFlags(pEntity)
    ...
    elseif context.type == 2 then
        context.meta = exports['vrp-vehicles']:GetVehicleMetadata(pEntity)
        context.flags = exports['vrp-flags']:GetVehicleFlags(pEntity)
    elseif context.type == 3 then
        context.meta = exports['vrp-objects']:GetObjectByEntity(pEntity)
        context.flags = exports['vrp-flags']:GetObjectFlags(pEntity)
    end
...
function isCloseToBoot(pEntity, pPlayerPed, pDistance, pModel)
    ...
    return #(engineCoords - playerCoords) <= pDistance
end exports('isCloseToBoot', isCloseToBoot)
...
function hasKeys(pEntity)
    ...
    local hasKeys = exports['vrp-vehicles']:HasVehicleKey(pEntity)
...
function hasJobPermission(pJob, pPermission)
  ...
  local hasJobPermission = function()
    if pPermission == "employee" then
      ...
      return RPC.execute('vrp-business:hasPermission', pJob, pPermission)
    end
  end
...
RegisterNetEvent("vrp-interact:bustVehicleOwnerShipCache", function(pEntity)
...
AddEventHandler("vrp-fishing:currentlyRentedBoat", function(handle)
    currentlyRentedBoat = handle
end)

AddEventHandler("vrp-interact:openFridge", function()
  TriggerEvent("server-inventory-open", "31", "Craft");
end)

AddEventHandler("vrp-interact:grabDrink", function(args, entity, context)
  TriggerEvent("server-inventory-open", "1", context.zones["bar:grabDrink"].name)
end)

RegisterNetEvent('vrp-interact:setNetVehicleBypassed', function(netId, bypass)
