...
Citizen.CreateThread(function()
  exports["vrp-keybinds"]:registerKeyMapping("", "Player", "Drop Prop", "+propAttachDrop", "-propAttachDrop")
...
RegisterNetEvent('vrp-propattach:attach')
AddEventHandler('vrp-propattach:attach', function(pPropItem, pTargetFunction)
...
AddEventHandler('attachcarryObject', function(attachModelSent, boneNumberSent, x, y, z, xR, yR, zR, pRotOrder)
	local rotationOrder = pRotOrder or 2
	...
	AttachEntityToEntity(carryObject, PlayerPedId(), bone, x, y, z, xR, yR, zR, 1, 1, true, 0, rotationOrder, 1)
end)
...
AddEventHandler('animation:carry', function(item,isInventory)
	...
	if not carryingObject then
		...
		lastObjectHealth = 0
		TriggerEvent("attachcarryObject",attachPropList[item]["model"], attachPropList[item]["bone"], attachPropList[item]["x"], attachPropList[item]["y"], attachPropList[item]["z"], attachPropList[item]["xR"], attachPropList[item]["yR"], attachPropList[item]["zR"], attachPropList[item]["rotOrder"])
	else
		holding = "none"
		removeAttachedcarryObject()
		carryingObject = false
