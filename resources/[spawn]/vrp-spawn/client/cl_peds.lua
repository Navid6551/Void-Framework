...
Citizen.CreateThread(function()
    GetCharacterData = VPX.Utils.cache(function ()
        local result = RPC.execute('vrp-character:fetchCharacterData')
    ...
    GetClothingData = VPX.Utils.cache(function (ctx, pCharacterIds)
        local result = RPC.execute('vrp-clothing:fetchClothingData', pCharacterIds)
        if not result then return false, nil end
...
function Login.CreatePeds(pIsReset)
    ...
    VPX.Streaming.loadModel(selectModelMale)
    VPX.Streaming.loadModel(selectModelFemale)
    ...
    local selectedCid
    -- if GetResourceState("vrp-queue") == "started" then
    --     selectedCid = RPC.execute("vrp-queue:getAllowedCharacters")
    -- end
    ...
    for idx = 1, math.min(8, getCharLimit()) do
        ...
        if not isBlocked then
            print(cModelHash)
            VPX.Streaming.loadModel(cModelHash)
            ...
            if character ~= nil then
                ...
                if isMpModel then
                    exports['vrp-clothing']:LoadPedDefaults(newPed)
                end
                exports['vrp-clothing']:ApplyPedClothing(
                    newPed,
                ...
                if isMpModel then
                    ...
                    exports['vrp-clothing']:ApplyPedData(
                        newPed,
                        headData.headblend,
                        headData.features,
                        headData.overlays,
                    ...
                    exports['vrp-clothing']:ApplyDecorations(newPed, decorations)
                end
            end
