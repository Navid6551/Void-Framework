...
function checkForAlternative(pItem)
	if pItem and exports["vrp-inventory"]:getQuantity(pItem["alternativeItem"]) > 0 then
...
AddEventHandler("AttachWeapons", function()
	...
	DeleteAttached()
	for i = 1, #w do
		if exports["vrp-inventory"]:getQuantity(w[i]["id"]) > 0 then
			...
			elseif w[i]["type"] == 4 and not sheathed then
				TriggerEvent("vrp-inventory:attachmentsToggle", true, w[i]["id"])
				sheathed = true
...
function DeleteAttached()
	TriggerEvent("vrp-inventory:attachmentsToggle", false)
	for i = 1, #ag do
		DeleteEntity(ag[i])
	end
	for i = 1, #ad do
