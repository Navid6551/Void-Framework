...
local supportedModels = {
    ...
    [`f14d3`] = {
        hasFlares = false,
        overrideFlareModRequirement = false,
        hasReticle = false,
        allowPDFlares = false
    },
    [`hydra2`] = {
...
local function canDropBombs(pModelInfo)
    if DoesEntityExist(currentVehicle) and IsEntityAVehicle(currentVehicle) and not IsEntityDead(currentVehicle) then
        if pModelInfo.hasBombs and GetVehicleMod(currentVehicle, 9) > -1 then
            ...
            if isPolice then return false end
            return true
        end
    end
    return false
...
local function canShootFlares(pModelInfo)
    if DoesEntityExist(currentVehicle) and IsEntityAVehicle(currentVehicle) and not IsEntityDead(currentVehicle) then
        if (pModelInfo.hasFlares and GetVehicleMod(currentVehicle, 1) == 1) or (pModelInfo.hasFlares and pModelInfo.overrideFlareModRequirement) then
            return true
        end
    end
    return false
...
local function runAirplane()
    ...
    CreateThread(function()
        ...
        local canUseFlares = canShootFlares(modelInfo)
        local canUseBombs = canDropBombs(modelInfo)

        while isRunning do
            if IsControlJustReleased(0, 117) and canUseFlares then
            ...
            if IsControlPressed(0, 118) and canUseBombs then
                local toggle = false
                local timer = GetGameTimer()
...
local function isPermitted(pModelInfo)
    local isDev = RPC.execute("vrp-misc-code:isAllowedAirCraft")
    local isSupportedPDCraft = isPolice and ((supportedModels[pModelInfo] and supportedModels[pModelInfo].allowPDFlares) or false)
    return (isDev or isSupportedPDCraft) or false
end

RegisterNetEvent("vrp-jobmanager:playerBecameJob")
AddEventHandler("vrp-jobmanager:playerBecameJob", function(pJob)
    isPolice = false
    if pJob == "police" then isPolice = true end
end)
...
AddEventHandler('baseevents:enteredVehicle', function(pVehicle, pSeat, pName, pClass, pModel)
    if ((pClass == 15 or pClass == 16) and supportedModels[pModel]) then
        if supportedModels[pModel].hasReticle then
            TriggerEvent("vrp-vehicle:showReticle", true)
        end
        if not isPermitted(pModel) then return end
...
AddEventHandler('baseevents:vehicleHotreload', function(pVehicle, pSeat, pEngineOn)
    ...
    if ((vehicleClass == 15 or vehicleClass == 16) and supportedModels[vehicleModel]) then
        if supportedModels[vehicleModel].hasReticle then
            TriggerEvent("vrp-vehicle:showReticle", true)
        end
        if not isPermitted(vehicleModel) then return end
        currentVehicle = pVehicle
...
AddEventHandler('baseevents:leftVehicle', function(pVehicle, pSeat, pName, pClass)
    ...
    currentModel = nil
    isRunning = false
    TriggerEvent("vrp-vehicle:showReticle", false)
