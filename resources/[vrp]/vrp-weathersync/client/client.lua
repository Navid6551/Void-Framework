...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    disable = false
    TriggerServerEvent('vrp-weathersync:server:RequestStateSync')
end)

RegisterNetEvent('vrp-weathersync:client:EnableSync', function()
    disable = false
    TriggerServerEvent('vrp-weathersync:server:RequestStateSync')
end)

RegisterNetEvent('vrp-weathersync:client:DisableSync', function()
    disable = true
    SetRainLevel(0.0)
...
RegisterNetEvent('vrp-weathersync:client:SyncWeather', function(NewWeather, newblackout)
    CurrentWeather = NewWeather
    blackout = newblackout
end)

RegisterNetEvent('vrp-weathersync:client:SyncTime', function(base, offset, freeze)
    freezeTime = freeze
    timeOffset = offset
    baseTime = base
end)
    ...
    while true do
        if not disable then
            ...
            if Config.RealTimeSync then
                ...
                TriggerEvent('timeheader', hour, second)
            end
