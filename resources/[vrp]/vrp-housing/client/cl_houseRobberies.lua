...
Citizen.CreateThread(function()
    exports["vrp-interact"]:AddPeekEntryByModel({ `prop_ld_int_safe_01` }, {
        {
...
function attemptToLockPickHouse(skipPicking)
    ...
    if not skipPicking then
        if Housing.housingBeingRobbedClient[propertyID].hasBeenRobbed == false then
            ...
            for i=1,2 do
                if not failed then
                    local finished = exports["vrp-ui"]:taskBarSkill(1650,  math.random(10, 14))
            ...
            Citizen.CreateThread(function()
                ...
                local baseProg = exports["vrp-progression"]:GetProgression("streetrep") or 0
    ...
    if pickedToOpen and not skipPicking then
        TriggerEvent("alert:houseRobbery", false)
    end
...
AddEventHandler("housing:exitFrontDoor", function()
    local spawnInApartmentsOnly = (exports["vrp-config"]:GetMiscConfig("spawn.apartments.only") or false)
    ...
    Housing.currentlyRobInside = false
    DoScreenFadeOut(1)
    exports["vrp-build"]:getModule("func").exitCurrentRoom(Housing.info[Housing.currentHousingInteractions.id][1])
    exitingBuilding()
    DoScreenFadeIn(1900)
end)
...
function displayPickup(force,itemName)
    ...
    if Housing.currentlyDisplayingPickup == false and not force then
        ...
        if type(itemName) == "string" then
            exports["vrp-ui"]:showInteraction("[E] Take Object")
        else
            exports["vrp-ui"]:showInteraction("[E] Search location")
        end
    end
...
function interactRob()
    ...
    if type(item) == "string" then
        local weight = exports["vrp-inventory"]:getCurrentWeight()
        local maxWeight = exports["vrp-inventory"]:getPlayerMaxWeight()
    ...
    if type(item) == "string" then
        ...
        local finished = exports["vrp-taskbar"]:taskBar(timer,"Securing Item")
        ClearPedTasks(PlayerPedId())
        if finished ~= 100 then
            return
        end
        ...
        TriggerEvent("animation:PlayAnimation","search")
        local finished = exports["vrp-taskbar"]:taskBar(15000,"Searching Location")
        ClearPedTasks(PlayerPedId())
        if finished ~= 100 then
            return
        end
    ...
    if finished then
        if type(item) == "string" then
            ...
            if complete then 
                ...
                TriggerServerEvent("vrp-placeables:chance", "house-robbery")
            end
            ...
            for k,v in pairs(itemTable) do
                ...
                TriggerServerEvent("vrp-gallery:generateGem", "robbery")
...
function alarm(clearAlarm)
    print("alarm(" .. tostring(clearAlarm) .. "), Housing.alarm=" .. tostring(Housing.alarm)) -- TEMP DEBUG

    if Housing.alarm then 
        local alarmOffset = exports["vrp-build"]:getModule("func").getAlarm()
...
RegisterNetEvent("vrp-housing:alarmFail")
AddEventHandler("vrp-housing:alarmFail", function()
    print("alarmFail", insideGame) -- TEMP DEBUG
...
RegisterNetEvent("vrp-housing:alarmEnter")
AddEventHandler("vrp-housing:alarmEnter", function()
    if not Housing.alarm then return end
    insideGame = true
    for i=1,2 do
        if not failedAlarm then
            local finished = exports["vrp-ui"]:taskBarSkill(3200,  math.random(10, 14))
            if finished ~= 100 then
                failedAlarm = true
            end
        end
    ...
    print("alarmEnter", failedAlarm) -- TEMP DEBUG
...
AddEventHandler("property:updateAlarms", function(alarmPass)
    print("updateAlarms", alarmPass) -- TEMP DEBUG
...
AddEventHandler("housing:takeSafe", function(pData, pEntity)
    ...
    local finished = exports['vrp-taskbar']:taskBar(20000, "Picking up safe", true, true, nil, false);
