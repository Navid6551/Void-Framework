...
function manageFood(restaurant, foodType)
  ...
  exports['vrp-ui']:openApplication('textbox', {
    callbackUrl = 'vrp-ui:restaurants:createFood',
...
RegisterUICallback("vrp-ui:restaurants:createFood", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  exports['vrp-ui']:closeApplication('textbox')
  RPC.execute('vrp-restaurants:createFoodItem', data)
end)

RegisterUICallback("vrp-ui:restaurants:createMenuItem", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  Wait(100)
  manageFood(data.key.restaurant, data.key.foodType)
end)

RegisterUICallback("vrp-ui:restaurants:manageFoodItem", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  Wait(100)
  if data.key.action == "delete" then
    RPC.execute("vrp-restaurants:deleteFoodItem", data.key.id, data.key.restaurant)
    return
  end
end)

AddEventHandler("vrp-restaurants:manageFoodMenu", function(pParams, p2, pContext)
  local restaurant = pParams.isEditorPeek and exports["vrp-housing"]:getCurrentPropertyID() or pContext.zones["restaurant_manage_food_menu"].id
  ...
  local hasCraftAccess = exports["vrp-business"]:HasPermission(restaurant, "craft_access")
      ...
      children = {
        {
          ...
          action = "vrp-ui:restaurants:createMenuItem",
        },
        {
          i18nTitle = true,
          title = "Side Dish",
      ...
      children = {
        {
          ...
          action = "vrp-ui:restaurants:manageFoodItem",
        },
      },
    }
  end
  ...
  exports["vrp-ui"]:showContextMenu(contextMenu)
end)

AddEventHandler("vrp-restaurants:manageDailyMenu", function(pParams, p2, pContext)
  local restaurant = pParams.isEditorPeek and exports["vrp-housing"]:getCurrentPropertyID() or pContext.zones["restaurant_manage_food_menu"].id
  ...
  RPC.execute("vrp-restaurants:setMenu", restaurant, dishes)
end)
