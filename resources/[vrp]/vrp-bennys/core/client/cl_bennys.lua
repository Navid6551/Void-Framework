...
local bennysLocations = {
    ...
    ["bennyspillbox"] = {
        pos = vector3(317.29,-1425.59,29.92),
        heading = 225.14,
        access = 'police'
    },
    -- ["bennyspaleto"] = {
    ...
    ["bennyssandyems"] = {
        pos = vector3(1653.18,3679.7,35.1),
        heading = 299.46,
        access = 'police'
    },
    ["bennyspdm"] = {
...
local function saveVehicle()
    ...
    if isAtEmployedBennys and not isAtPreviewBennys then
        ...
        TriggerEvent("vrp-jobs:bennys:order", veh, changedStuff)
...
function AttemptPurchase(purchaseType, upgradeLevel)
    ...
    if bennys.access and (bennys.access == "police" or bennys.access == "ems") and purchaseType == "repair" then
        return RPC.execute("vrp-bennys:jobRepair")
    end
    ...
    if (bennys.access == "police" or bennys.access == "ems") then
        local free = exports['vrp-config']:GetMiscConfig('bennys.pd.free')

        pd = free
    end
    TriggerServerEvent("vrp-bennys:attemptPurchase", cheap, purchaseType, upgradeLevel, pd)
...
function RepairVehicle()
    Citizen.CreateThread(function ()
        ...
        if (missingEngineHealth > 50) then
            local finished = exports["vrp-taskbar"]:taskBar(5000 + (missingEngineHealth / 50), "Repairing engine...", true, nil, nil, nil, nil, 6.0)
        ...
        if missingBodyHealth > 50 then
            local finished = exports["vrp-taskbar"]:taskBar(5000 + (missingBodyHealth / 50), "Repairing body...", true, nil, nil, nil, nil, 6.0)
...
function PreviewMod(categoryID, modID)
    ...
    if categoryID == 39 or categoryID == 4 then
        if GetResourceState("vrp-vehiclesync-state") == "started" then
            exports['vrp-vehiclesync-state']:SetEngineSound(plyVeh, true)
        elseif GetResourceState("vrp-vehiclesync") == "started" then
            exports['vrp-vehiclesync']:SetEngineSound(plyVeh, true)
        end
    end
end
...
function ExitBennys()
    ...
    if isAtEmployedBennys and not isAtPreviewBennys then
        ...
        if GetResourceState("vrp-vehiclesync-state") == "started" then
            exports['vrp-vehiclesync-state']:SetEngineSound(plyVeh)
        elseif GetResourceState("vrp-vehiclesync") == "started" then
            exports['vrp-vehiclesync']:SetEngineSound(plyVeh)
        end
    ...
    TriggerServerEvent("vrp-bennys:removeFromInUse", currentBennys)
    if devmode then
      TriggerEvent("vehicle:leftBennys", nil, nil)
    elseif not isAtPreviewBennys then
      exports["vrp-ui"]:showInteraction("Bennys")
...
local function finishEnterLocation()
    ...
    isPlyInBennys = true
    TriggerServerEvent("vrp-bennys:addToInUse", currentBennys)
    disableControls()
    TriggerEvent("inmenu", isPlyInBennys)
end
...
function enterLocation(pBennys, needsRepair)
    ...
    if isAtEmployedBennys then
        ...
        VehicleClass = exports["vrp-vehicles"]:GetVehicleRatingClass(plyVeh)
    end
    ...
    isVinScratched = exports['vrp-vehicles']:IsVinScratched(plyVeh)
...
--#[Event Handlers]#--
RegisterNetEvent("vrp-bennys:purchaseSuccessful")
AddEventHandler("vrp-bennys:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
end)

RegisterNetEvent("vrp-bennys:purchaseFailed")
AddEventHandler("vrp-bennys:purchaseFailed", function()
...
AddEventHandler("bennys:enter", function()
    ...
    local isAllowed, message  = RPC.execute("vrp-bennys:isAllowedToUse", currentBennys, NetworkGetNetworkIdFromEntity(plyVeh))
    ...
    if bennysAccess then
        ...
        jobAccess = exports["vrp-business"]:IsEmployedAt(bennysAccess)

        if (isAtPreviewBennys and previewPermission[currentBennys] and isTestDrive) then
            jobAccess = exports["vrp-business"]:HasPermission(bennysAccess, previewPermission[currentBennys])
        end
...
Citizen.CreateThread(function()
    Wait(5000)
    local catalog = {}--RPC.execute('vrp-jobs:bennys:getPartsCatalog')

    local modifiers ={}-- RPC.execute('vrp-jobs:bennys:getPriceModifiers')

    local taxLevel = 5--RPC.execute("GetTaxLevel", 2)
...
function GetPartPrice(pType, pPart, pIndex, pRating)
    ...
    return 0--math.floor(indexData.price * modifier)
end
...
RegisterNetEvent('vrp-admin:bennys', function()
    devmode = true
