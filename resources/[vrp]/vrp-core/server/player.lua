    ...
    if source and source ~= '' then
        if citizenid then
            ...
            if PlayerData and license == PlayerData.license then
                ...
                PlayerData.charinfo = info
                TriggerClientEvent('isPedUpdate', source, info.firstname, info.lastname, info.phone, PlayerData.citizenid)
                QBCore.Player.CheckPlayerData(source, PlayerData)
            else
                DropPlayer(source, Lang:t('info.exploit_dropped'))
                TriggerEvent('vrp-log:server:CreateLog', 'anticheat', 'Anti-Cheat', 'white', GetPlayerName(source) .. ' Has Been Dropped For Character Joining Exploit', false)
            end
        else
...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    -- if GetResourceState('vrp-inventory') ~= 'missing' then
    --     PlayerData.items = exports['vrp-inventory']:LoadInventory(PlayerData.source, PlayerData.citizenid)
    -- end
...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    function self.Functions.AddMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            ...
            if amount > 100000 then
                TriggerEvent('vrp-log:server:CreateLog', 'playermoney', 'AddMoney', 'lightgreen', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') added, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason, true)
            else
                TriggerEvent('vrp-log:server:CreateLog', 'playermoney', 'AddMoney', 'lightgreen', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') added, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason)
            end
    ...
    function self.Functions.RemoveMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            self.Functions.UpdatePlayerData()
            if amount > 100000 then
                TriggerEvent('vrp-log:server:CreateLog', 'playermoney', 'RemoveMoney', 'red', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') removed, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason, true)
            else
                TriggerEvent('vrp-log:server:CreateLog', 'playermoney', 'RemoveMoney', 'red', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') removed, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason)
            end
            TriggerClientEvent('hud:client:OnMoneyChange', self.PlayerData.source, moneytype, amount, true)
            if moneytype == 'bank' then
                TriggerClientEvent('vrp-phone:client:RemoveBankMoney', self.PlayerData.source, amount)
            end
    ...
    function self.Functions.SetMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            self.Functions.UpdatePlayerData()
            TriggerEvent('vrp-log:server:CreateLog', 'playermoney', 'SetMoney', 'green', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') set, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason)
...
function QBCore.Player.Save(source)
    ...
    if PlayerData then
        ...
        -- if GetResourceState('vrp-inventory') ~= 'missing' then exports['vrp-inventory']:SaveInventory(source) end
...
function QBCore.Player.SaveOffline(PlayerData)
    if PlayerData then
        ...
        -- if GetResourceState('vrp-inventory') ~= 'missing' then exports['vrp-inventory']:SaveInventory(PlayerData, true) end
...
function QBCore.Player.DeleteCharacter(source, citizenid)
    ...
    if license == result then
        ...
        MySQL.transaction(queries, function(result2)
            if result2 then
                TriggerEvent('vrp-log:server:CreateLog', 'joinleave', 'Character Deleted', 'red', '**' .. GetPlayerName(source) .. '** ' .. license .. ' deleted **' .. citizenid .. '**..')
            end
        end)
    else
        DropPlayer(source, Lang:t('info.exploit_dropped'))
        TriggerEvent('vrp-log:server:CreateLog', 'anticheat', 'Anti-Cheat', 'white', GetPlayerName(source) .. ' Has Been Dropped For Character Deletion Exploit', true)
    end
end
...
function QBCore.Player.ForceDeleteCharacter(citizenid)
    ...
    if result then
        ...
        MySQL.transaction(queries, function(result2)
            if result2 then
                TriggerEvent('vrp-log:server:CreateLog', 'joinleave', 'Character Force Deleted', 'red', 'Character **' .. citizenid .. '** got deleted')
            end
        end)
    end
end
...
function QBCore.Player.SaveInventory(source)
    -- if GetResourceState('vrp-inventory') == 'missing' then return end
    -- exports['vrp-inventory']:SaveInventory(source, false)
end

function QBCore.Player.SaveOfflineInventory(PlayerData)
    -- if GetResourceState('vrp-inventory') == 'missing' then return end
    -- exports['vrp-inventory']:SaveInventory(PlayerData, true)
end

function QBCore.Player.GetTotalWeight(items)
    -- if GetResourceState('vrp-inventory') == 'missing' then return end
    -- return exports['vrp-inventory']:GetTotalWeight(items)
end

function QBCore.Player.GetSlotsByItem(items, itemName)
    -- if GetResourceState('vrp-inventory') == 'missing' then return end
    -- return exports['vrp-inventory']:GetSlotsByItem(items, itemName)
end

function QBCore.Player.GetFirstSlotByItem(items, itemName)
    -- if GetResourceState('vrp-inventory') == 'missing' then return end
    -- return exports['vrp-inventory']:GetFirstSlotByItem(items, itemName)
end

-- Util Functions

function QBCore.Player.CreateCitizenId()
    local CitizenId = math.random(1, 10)..math.random(1, 10) .. math.random(1, 10) .. math.random(1, 10).. math.random(1, 10)
...
end exports('CreateCitizenId', QBCore.Player.CreateCitizenId)

function QBCore.Functions.CreateAccountNumber()
    local AccountNumber = math.random(1, 9) .. math.random(1111, 9999) .. math.random(1111, 9999) .. math.random(11, 99).. math.random(11, 99)
...
end exports('CreateAccountNumber', QBCore.Functions.CreateAccountNumber)
