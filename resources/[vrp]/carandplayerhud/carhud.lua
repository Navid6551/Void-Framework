...
toggleFaceWear = function(pWearType, pShouldRemove)
  ...
  if pWearType == 1 and not pShouldRemove and removedHatManually then
    local hasHat = exports["vrp-inventory"]:hasEnoughOfItem("hat", 1, false, true, {hat = facialWear[PropIndex+1]["Prop"]})
    if not hasHat then
  ...
  if pWearType == 4 and not pShouldRemove and removedMaskManually then
    local hasMask = exports["vrp-inventory"]:hasEnoughOfItem("mask", 1, false, true, {mask = facialWear[pWearType]["Prop"]})
    if not hasMask then
    ...
    if pWearType ~= 5 then
      ...
      if pWearType == 4 then
        ...
        if currentDrawable ~= -1 then
          ...
          local hasPropItem = exports["vrp-inventory"]:hasEnoughOfItem("mask", 1, false, true, itemMeta)
          if not hasPropItem then
            TriggerEvent("player:receiveItem", "mask", 1, false, itemMeta)
            exports['vrp-clothing']:SaveCurrentClothing(PlayerPedId())
          end
        ...
        if pWearType ~= 2 then
          ...
          if pWearType == 1 and currentProp ~= -1 then
            ...
            local hasPropItem = exports["vrp-inventory"]:hasEnoughOfItem("hat", 1, false, true, itemMeta)
            if not hasPropItem then
...
AddEventHandler("vrp-jobmanager:playerBecameJob", function(job, name)
  ...
  if DoesEntityExist(objFound) then
    -- TriggerEvent("DoLongHudText","Press [E] to leave the bed at any time.",1)
      ...
      if not disableKeys then
        showInteraction = true
        exports["vrp-ui"]:showInteraction("[E] Leave Bed; [F] Dismiss")
      end
...
Citizen.CreateThread(function()
  while true do
    local isSitting = exports["vrp-flags"]:HasPedFlag(PlayerPedId(), "isSittingOnChair")
...
RegisterNetEvent('vrp-voice:focus:set')
AddEventHandler('vrp-voice:focus:set', function(hasFocus, hasKeyboard, hasMouse)
  ...
  while true do
    ...
    if counter == 0 then
      ...
      if valueChanged then
        -- TriggerEvent("vrp-ui:hud:values", currentValues)
...
AddEventHandler("buffs:triggerBuff", function(item)
  ...
  TriggerEvent("vrp-buffs:addUIBuff", { buffedHunger = true, buffedThirst = true })
  Citizen.CreateThread(function()
    Citizen.Wait(60000 * 60 * 6) -- 6hr
    buffStartTime = 0
    TriggerEvent("vrp-buffs:addUIBuff", { buffedHunger = false, buffedThirst = false })
  end)
end)
...
AddEventHandler("buffs:triggerBuff", function(item)
  ...
  TriggerEvent("vrp-buffs:addUIBuff", { buffedHealth = true, buffedArmor = true })
  Citizen.CreateThread(function()
    Citizen.Wait(60000 * 60 * 6) -- 3hr
    citrineStartTime = 0
    TriggerEvent("vrp-buffs:addUIBuff", { buffedHealth = false, buffedArmor = false })
  end)
end)
Citizen.CreateThread(function()
  while true do
...
AddEventHandler("vrp-inventory:itemUsed", function(item)
  ...
  local finished = exports["vrp-taskbar"]:taskBar(4000, "Using Chapstick")
...
AddEventHandler("shoes:steal", function(pArgs, pEntity)
  ...
  Citizen.Wait(1600)
  ClearPedTasks(PlayerPedId())
  TriggerServerEvent("vrp-clothing:removeShoes", GetPlayerServerId(NetworkGetPlayerIndexFromPed(pEntity)))
...
AddEventHandler("baseevents:enteredVehicle", function(pVehicle)
  ...
  local hasLicense = exports["vrp-gov"]:HasLicense("pilot_license")
  if hasLicense then
    return
  end
    ...
    while BLOCKING_LOOP and not dToggle do
      if IsEntityInWater(pVehicle) then
        if GetPedInVehicleSeat(pVehicle, -1) == PlayerPedId() then
          TriggerServerEvent("vrp:vehicles:reduceFuel", NetworkGetNetworkIdFromEntity(pVehicle), 1)
        end
      end
      Wait(4000)
    end
