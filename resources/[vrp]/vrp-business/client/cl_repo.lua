...
RegisterUICallback("vrp-ui:repoSeizeVehicle", function(data, cb)
    local loan = data.key
...
RegisterUICallback("vrp-ui:repoSeizeProperty", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })
end)

RegisterUICallback("vrp-ui:repoFetchAssetLocation", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })

    local info = data.key

    local location = RPC.execute('vrp-business:repo:fetchAssetLocation', info.type, info.data)
...
RegisterUICallback("vrp-ui:repoRecoverAsset", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })
    ...
    if info.type == 'vehicle' then
        local garage = exports['vrp-vehicles']:GetCurrentGarage()

        if garage ~= location.garage then
            return TriggerEvent('DoLongHudText', 'This is not the correct garage.', 2)
        end

        local isOnSpot, parkingSpot = table.unpack(exports['vrp-vehicles']:FindParkingSpot(PlayerPedId(), true, 4.5))
...
RegisterUICallback("vrp-ui:repoGenerateOrder", function(data, cb)
    ...
    RPC.execute('vrp-business:repoGenerateOrder', info.plate, info.vin, prompt.reason, info.loanId)
end)
...
AddEventHandler("vrp-polyzone:enter", function(name, data)
    if name ~= "vrp-jobs:impound:dropOff" then return end
...
function RepoOrderMenu(pType, pData)
    ...
    local elements = {
        ...
        },
        {
            title = 'Locate Asset',
            description = '',
            action = "vrp-ui:repoFetchAssetLocation",
    ...
    if pType == 'vehicle' then
        if InDropOffLocation then 
            ...
            elements[#elements+1] = {
                title = 'Recover Asset',
                description = '',
                action = "vrp-ui:repoRecoverAsset",
                disabled = garage == nil,
                key = { type = pType, data = pData },
            }
        end
    ...
    exports['vrp-ui']:showContextMenu(elements)
end
        ...
        local element = {
            ...
            children = {
                ...
                },
                {
                    ...
                    description = '',
                    action = "vrp-ui:repoGenerateOrder",
