...
RegisterNetEvent('hadcocainium')
AddEventHandler('hadcocainium', function()
  if drugEffectTime > 0 then return end
  drugEffectTime = 60

  TriggerEvent("client:newStress", false, math.random(500, 1000))
  
  local ped = PlayerPedId()
  local pId = PlayerId()
  local loops = 0

  SetRunSprintMultiplierForPlayer(pId, 1.275)
  SetSwimMultiplierForPlayer(pId, 1.175)
  ...
  while drugEffectTime > 0 do
    ...
    if loops == 10 then
      TriggerEvent("fx:run", "cocaine", math.random(10, 30), 0.0, false, false)
    end
    loops = loops + 1
    Citizen.Wait(1000)
    RestorePlayerStamina(pId, 1.0)
...
AddEventHandler('inventory:ketamine', function(pPurity)
  ...
  TriggerServerEvent("vrp-drugeffects:forceStress", 1, 3 * 60 * 1000)
  ...
  ketaActive = false
  TriggerEvent("fx:run", "cocaine", 10, 0.0, false, false)
  TriggerServerEvent("vrp-drugeffects:forceStress", 5000, 10 * 60 * 1000)
end)
...
AddEventHandler('inventory:heroin', function(pPurity)
  ...
  TriggerEvent("fx:run", "heroin", 7 - 3 * (100-pPurity), -1, false, false)
  ...
  TriggerEvent("healed:useOxy", false)
  TriggerServerEvent("vrp-drugeffects:forceStress", 1, length)

  local purityRng = (200 - math.floor(pPurity)) * 2
  while drugRunning do
    Citizen.Wait(0)
    local rng = math.random(purityRng)
    local hit = rng == 69
    if pPurity > 99 then
      hit = rng > 69 and rng < 74
    end
    if hit then
      -- fast armor regen
      local armor = GetPedArmour(PlayerPedId())
...
-- adderall Effects
RegisterNetEvent('hadadderall')
AddEventHandler('hadadderall', function()
  if drugEffectTime > 0 then return end
  drugEffectTime = 30

  TriggerServerEvent("server:alterStress", false, 3000)
  TriggerEvent("fx:run", "adderall", 300, 0.0, false, false)
...
RegisterNetEvent('hadsteroids')
AddEventHandler('hadsteroids', function()
  if drugEffectTime > 0 then return end
  drugEffectTime = 60

  TriggerEvent("fx:run", "steroids", 60, 0.0, false, false)
  ...
  while drugEffectTime > 0 do
    ...
    loops = loops + 1
    drugEffectTime = drugEffectTime - 1
    SetWeaponDamageModifierThisFrame(`WEAPON_UNARMED`, 10.0)
