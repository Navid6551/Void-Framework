RegisterUICallback("vrp-ui:getCrypto", function(data, cb)
...
RegisterUICallback("vrp-ui:cryptoExchange", function(data, cb)
...
function processXCoinRedeem(cid)
  local hasItem = exports["vrp-inventory"]:hasEnoughOfItem("inkedmoneybag", 1, false, true)
  if not hasItem then
    TriggerEvent("DoLongHudText", "You don't have what they're looking for", 2)
    return
  end
  local canRedeem = RPC.execute("vrp-phone:xcoin:canRedeem", cid)
...
AddEventHandler("vrp-phone:getXCoin", function()
...
RegisterUICallback("vrp-ui:cryptoPurchase", function(data, cb)
...
RegisterUICallback("vrp-ui:cryptoSell", function(data, cb)
  local character_id, bank_id, crypto_id, amount = data.character.id, data.character.bank_account_id, data.id, data.amount
