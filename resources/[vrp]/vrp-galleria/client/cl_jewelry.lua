...
AddEventHandler('vrp-inventory:closed', function(pName)
    ...
    if found then
        TriggerServerEvent('vrp-gallery:fetchJewelry')
    end
    ...
    if found then
        TriggerServerEvent('vrp-gallery:scrapJewelry', pName)
    end
end)

AddEventHandler('vrp-gallery:jewelryAction', function(pParameters, pEntity, pContext)
    ...
    context[#context+1] = {
        title = 'Scrap Jewelry',
        icon = 'hammer',
        action = 'vrp-gallery:jewelryMenuAction',
        key = { scrap = true }
    }
    context[#context+1] = {
...
RegisterUICallback("vrp-gallery:jewelryMenuAction", function(data, cb)
    ...
    if data.identifyAll then
        ...
        Citizen.CreateThread(function()
            exports['vrp-taskbar']:taskBar(length, 'Identifying Jewelry')
        end)
        for _,item in ipairs(items) do
            local info = json.decode(item.information)
            if info.identified then
            ...
            NPX.Procedures.execute('vrp-gallery:identifyJewelry', item)
            Wait(1000)
            ::continue::
        end
    end
    ...
    if data.identifyItem then
        ...
        local result = NPX.Procedures.execute('vrp-gallery:identifyJewelry', item)
        if not result then
            return
        end
    ...
    if data.repairItem then
        ...
        local hasParts = exports['vrp-inventory']:hasEnoughOfItem('jewelry_part', cost, false, true)
    ...
    if data.craft then
        ...
        TriggerServerEvent('vrp-gallery:craftJewelryItem', type, cost)
    end
end)
...
function GetJewelryInventoryItems(pType)
    ...
    if pType == 'jewelry' then
        local rings = exports['vrp-inventory']:getItemsOfType('jewelry_ring', 10, false)
        local necklaces = exports['vrp-inventory']:getItemsOfType('jewelry_necklace', 10, false)
    ...
    if pType == 'relics' then
        local relics = exports['vrp-inventory']:getItemsOfType('jewelry_relic', 10, false)
...
RegisterNetEvent('vrp-gallery:applyEffects', function(pEffects)
    appliedEffects = {}
    ...
    for _,effect in pairs(pEffects) do
        ...
        if effect.id == 'LUCKY_BONUS' then
            appliedLucky = true
            TriggerEvent("vrp-buffs:applyBuff", "jewelry", { { buff = "jobluck", percent = (effect.level / 100), timeOverride = 12 * 60 * 60 * 1000 } })
        end
    end

    if not appliedLucky then
        TriggerEvent("vrp-buffs:applyBuff", "jewelry", { { buff = "jobluck", percent = 0.0, timeOverride = 1 } })
    end
end)
...
AddEventHandler('vrp-gallery:jewelry:armorRegen', function(pLevel)
...
AddEventHandler('vrp-gallery:jewelry:healthRegen', function(pLevel)
...
AddEventHandler('vrp-gallery:jewelry:stressReduce', function(pLevel)
    TriggerEvent("client:newStress", false, pLevel, true)
end)

AddEventHandler('vrp-gallery:jewelry:jailTimeReduce', function(pLevel)
    if not isPrisoner then return end
    TriggerServerEvent('vrp-jail:reducePrisonTime', pLevel)
end)

AddEventHandler("vrp-jail:playerJailed", function(pMonths)
    if pMonths < 120 then
        isPrisoner = true
    end
end)
