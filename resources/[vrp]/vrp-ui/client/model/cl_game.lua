...
AddEventHandler('vrp-config:configReady', function()
    loadPublicData()
end)

AddEventHandler("vrp-config:configLoaded", function (configId, config)
    if configId == 'vrp-police' then
        StunDuration = config["stunDuration"] or 5000
    end
end)

function loadPublicData()
    local showWatermark = exports['vrp-config']:GetMiscConfig('hud.watermark')
    local showroomPurchase = exports['vrp-config']:GetMiscConfig('showrooms.catalog.purchase')
    local skipMdwProfileCheck = exports['vrp-config']:GetMiscConfig('mdw.profiles.skip')
    local serverCode = 'wl'
...
function sendCharacterData()
    Citizen.CreateThread(function()
        ...
        print(characterId, firstName, lastName, phoneNumber)
        if not characterId then return end
        local hasBankAccount, bankAccountId = RPC.execute("GetDefaultBankAccount", characterId, true)
        local hasEmailAddress, emailAddress = RPC.execute("vrp-phone:getEmailAddress")
        local character = {
            id = characterId,
        ...
        TriggerEvent('vrp-ui:phoneReady')
        TriggerServerEvent('vrp-ui:phoneReady')
    end)
end

AddEventHandler('vrp-ui:updateCharacterData', function()
    sendCharacterData()
end)
...
RegisterCommand('charspawn', function()
    TriggerEvent('vrp-spawn:characterSpawned')
end)
...
AddEventHandler("vrp-ui:openUrl", function(pUrl)
    SendUIMessage({ source = "vrp-nui", app = "main", action = "openUrl", url = pUrl })
end)

-- AddEventHandler("vrp-ui:hud:values", function(values)
--   -- sendAppEvent("hud", {
--   --     food = values["hunger"],
