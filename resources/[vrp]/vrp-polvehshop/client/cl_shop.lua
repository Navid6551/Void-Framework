...
local function showPolcarMenu()
  ...
  for _, vehicle in pairs(polcars) do
      ...
      data[#data + 1] = {
          ...
          key = vehicle.model,
          children = {
              { title = "Confirm Purchase", action = "vrp-ui:polcarsPurchase", key = vehicle.model },
          },
      }
  end
...
RegisterUICallback("vrp-ui:polcarsPurchase", function(data, cb)
  local zoneName, zone = getZone()
  if not zone then return end
  data.model = data.key
...
AddEventHandler("vrp-polyzone:enter", function(zone)
  ...
  exports["vrp-ui"]:showInteraction(("[E] %s"):format(zones[zone].label))
  listenForKeypress()
end)
