...
local cmdsSet = {}
local vehBypass = {}
RegisterNUICallback("vrp-ui:hudSetPreferences", function(data, cb)
    ...
    if data["hud.presets"] then
      ...
      for _, p in pairs(data["hud.presets"]) do
        ...
        if not cmdsSet[idx] then
          ...
          exports["vrp-keybinds"]:registerKeyMapping("", "HUD", "Enable " .. s, "+hud_enable_" .. s, "-hud_enable_" .. s)
          RegisterCommand('+hud_enable_' .. s, function()
            exports["vrp-ui"]:sendAppEvent("preferences", eventOptions)
          end, false)
    ...
    TriggerEvent("vrp-preferences:setPreferences", data)
end)
...
AddEventHandler("vrp-ui:watch", function()
...
AddEventHandler('vrp-inventory:itemCheck', function (item, hasItem, quantity)
...
AddEventHandler("vrp:voice:proximity", function(proximity)
    sendAppEvent("hud", {
        voiceRange = proximity,
    })
end)

AddEventHandler("vrp:voice:transmissionStarted", function(data)
    sendAppEvent("hud", {
...
AddEventHandler("vrp:voice:transmissionFinished", function()
    sendAppEvent("hud", {
        voiceActive = false,
...
RegisterNetEvent("vrp-admin:currentDebug")
AddEventHandler("vrp-admin:currentDebug", function(debugToggle)
...
AddEventHandler('vrp-vehicles:pursuitMode', function (pEnabled, pMode, pModes)
    inPursuitVehicle = pEnabled

    if pMode == nil or pModes == nil then currentPursuitMode = 0 return end
...
AddEventHandler('vrp-vehicles:autopilot:status', function (pStatus)
    isAutopilotEnabled = pStatus == 'started'
end)

AddEventHandler('vrp-jail:attachedCollar', function (pAttached)
...
RegisterNetEvent('vrp-boosting:client:setBoostCompletions')
AddEventHandler('vrp-boosting:client:setBoostCompletions', function (pAmount)
...
RegisterNetEvent('vrp-weapons:client:setWeaponFireRate')
AddEventHandler('vrp-weapons:client:setWeaponFireRate', function (pAmount)
    weaponFireRate = pAmount
    setHudValue("weaponFireRate", weaponFireRate)
end)

RegisterNetEvent('vrp-weapons:client:showWeaponFireRate')
AddEventHandler('vrp-weapons:client:showWeaponFireRate', function (pToggle)
    showWeaponFireRate = pToggle
    setHudValue("showWeaponFireRate", pToggle)
end)

RegisterNetEvent('vrp-ui:hud:forceShowMinimap', function(pForce)
...
AddEventHandler('vrp-drifting:showDriftMode', function (pToggle)
    showDriftMode = pToggle
    setHudValue("showDriftMode", pToggle)
end)

AddEventHandler('vrp-drifting:toggleDriftMode', function (pToggle)
    driftMode = pToggle
...
AddEventHandler("vrp-drifting:remainingTireThread", function (pThread)
...
function roundedRadar()
    ...
    Citizen.CreateThread(function()
        ...
        -- for k,v in pairs(exports["vrp-base"]:getModule("Blips")) do
...
function getFuel()
    fuel = exports["vrp-vehicles"]:CurrentFuel() or 0
end
...
function generateSpeedo()
    ...
    SetVehicleAudioBodyDamageFactor(veh, 0.0)
    ...
    Citizen.CreateThread(function()
        while engineOn do
            if GetVehicleEngineHealth(veh) < 400.0 then
                ...
                SetVehicleAudioEngineDamageFactor(veh, 1.0)
            else
                engineDamageShow = false
                SetVehicleAudioEngineDamageFactor(veh, 0.0)
            end
            ...
            if GetPedInVehicleSeat(veh, -1) == GetPed() then
                harnessDurability = exports["vrp-vehicles"]:GetVehicleMetadata(veh, 'harness')
            end

            local degradation = exports["vrp-vehicles"]:GetVehicleDegradation(veh)
            if not degradation then degradation = {} end
            for _,damage in pairs(degradation) do
                if damage < 75.0 then
                    partsDamageShow = true
                    SetVehicleAudioBodyDamageFactor(veh, 1.0)
                    break
                end
            end
...
AddEventHandler('vrp-ui:setVehicleBypassed', function(veh, bypass)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isPMA and not pauseActive then
            ...
            TriggerEvent('vrp-ui:pauseMenuActive', pauseActive)
...
AddEventHandler('vrp-ui:setGaze', function (pEntity, pDelay)
    GazeEntity = pEntity
    GazeDelay = pDelay
end)
...
Citizen.CreateThread(function()
  while true do
    ...
    if toggleParachuteOff and pState == -1 then
      ...
      exports["vrp-ui"]:sendAppEvent("hud", { hasParachute = false })
...
AddEventHandler("hud:equipParachute", function()
  hasParachute = true
  exports["vrp-ui"]:sendAppEvent("hud", { hasParachute = true })
end)

AddEventHandler("vrp-island:onIsland", function(pState)
    onIsland = pState
end)

AddEventHandler("vrp-vehicle:showReticle", function(pState)
    showReticle = pState
end)
