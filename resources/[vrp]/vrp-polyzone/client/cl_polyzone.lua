...
local function addToComboZone(zone)
    if comboZone ~= nil then
        ...
        comboZone = ComboZone:Create({ zone }, { name = "vrp-polyzone" })
        comboZone:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
            if leftZones ~= nil then
              for i = 1, #leftZones do
                ...
                TriggerEvent("vrp-polyzone:exit", leftZone.name, leftZone.data)
              end
            end
            if enteredZones ~= nil then
              for i = 1, #enteredZones do
                ...
                TriggerEvent("vrp-polyzone:enter", enteredZone.name, enteredZone.data, enteredZone.center)
              end
            end
        end, 500)
    end
...
RegisterNetEvent("vrp-polyzone:createCircleZone")
AddEventHandler("vrp-polyzone:createCircleZone", function(name, ...)
  addCircleZone(name, ...)
end)
...
if GetConvar("sv_environment", "prod") == "debug" then
  RegisterCommand("vrp-polyzone:debug", function (src, args)
    toggleDebug(not debugEnabled)
  end)

  RegisterCommand("vrp-polyzone:insideZones", function ()
