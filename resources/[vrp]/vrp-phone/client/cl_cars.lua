...
RegisterUICallback("vrp-ui:getCars", function(data, cb)
  local data = RPC.execute("vrp:vehicles:getPlayerVehiclesWithCoordinates", data.character.id)
...
RegisterUICallback("vrp-ui:carActionTrack", function(data, cb)
...
RegisterUICallback("vrp-ui:carActionSpawn", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' }})
  ...
  local success, message = table.unpack(RPC.execute('vrp:vehicles:respawnVehicle', vehicle_plate))
...
RegisterUICallback("vrp-ui:carActionSell", function(data, cb)
  local vehicleCoords = data.car.location
  if not vehicleCoords then return end
  TriggerEvent('vrp:vehicles:sellPhone', data.car.vin, data.stateId, data.price + 0.0)
  cb({ data = {}, meta = { ok = true, message = '' } })
end)
