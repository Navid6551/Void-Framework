...
local meleeEffects = {
  ...
  ["weapon_banana"] = "knockdownlowhp",
  ["weapon_firework"] = "knockdown",
}
...
CreateThread(function()
  ...
  while true do
    ...
    if RecentlyRiding then
      local BikeArmor = exports["vrp-inventory"]:hasEnoughOfItem("bikearmor", 1, false,true)
      if BikeArmor then
...
function ProcessHitByCash(pAttacker)
  TriggerServerEvent("vrp-weapons:attackedByCash", pAttacker)
end
...
AddEventHandler("DamageEvents:EntityDamaged", function(victim, attacker, pWeapon, isMelee)
  ...
  -- print(pWeapon)
  if ( (pWeapon == CrashHash or pWeapon == RamHash or pWeapon == FallHash) and exports["vrp-inventory"]:hasEnoughOfItem("bikearmor", 1, false, true) and not DegApplied and RecentlyRiding) then
    DegApplied = true
