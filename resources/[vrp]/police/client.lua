...
RegisterNetEvent("vrp-config:configLoaded", function (configId, config)
  if configId ~= "vrp-police" then return end
...
AddEventHandler("onClientResourceStart", function (resourceName)
  ...
  local config = exports["vrp-config"]:GetModuleConfig("vrp-police")
...
function plyTackel()
	...
	if GetLastInputMethod(2) then
		...
		if not isInVeh and GetEntitySpeed(PlayerPedId()) > 2.5 then
			TriggerEvent("vrp-police:tackle")
		end
	end
end

Citizen.CreateThread(function()
  exports["vrp-keybinds"]:registerKeyMapping("", "Vehicle", "Cruise Control", "+vehCruise", "-vehCruise", "X")
  RegisterCommand('+vehCruise', vehCruise, false)
  RegisterCommand('-vehCruise', function() end, false)

  exports["vrp-keybinds"]:registerKeyMapping("", "Player", "Tackle", "+plyTackel", "-plyTackel")
	...
	if not inmenus and isCop and not IsPauseMenuActive() then
		...
		if isInVeh then
			...
			if not IsControlPressed(0,19) then
				TriggerEvent("vrp-police:cuffs:attempt",false)
			end
		end
	end
end
...
function policeEscort()
	if not inmenus and (isMedic or isDoctor or isCop) and not IsPauseMenuActive() then
		...
		if isInVeh and isCop then
			...
			TriggerEvent("vrp-police:drag:attempt") 
		end
	end
end
...
function policeSeat()
	if not inmenus and (isMedic or isCop) and not IsPauseMenuActive() then
		TriggerEvent("vrp-police:vehicle:seat")
	end
end
...
function policeSoft()
	if not inmenus and isCop and not IsPauseMenuActive() then
		local isInVeh = IsPedInAnyVehicle(PlayerPedId(), false)
		if not isInVeh then
			TriggerEvent("vrp-police:cuffs:attemptSoft")
		end
	end
end
...
Citizen.CreateThread( function()
	...
	exports["vrp-keybinds"]:registerKeyMapping("", "Gov", "Cuff / Radar Front", "+policeCuff", "-policeCuff", "UP")

	RegisterCommand('+medicRevive', medicRevive, false)
	RegisterCommand('-medicRevive', function() end, false)
	exports["vrp-keybinds"]:registerKeyMapping("", "Gov", "EMS Revive", "+medicRevive", "-medicRevive")

	RegisterCommand('+emsHeal', emsHeal, false)
	RegisterCommand('-emsHeal', function() end, false)
	exports["vrp-keybinds"]:registerKeyMapping("", "Gov", "EMS Heal", "+emsHeal", "-emsHeal")

	RegisterCommand('+policeEscort', policeEscort, false)
	RegisterCommand('-policeEscort', function() end, false)
	exports["vrp-keybinds"]:registerKeyMapping("", "Gov", "Escort / Speedo", "+policeEscort", "-policeEscort", "LEFT")

	RegisterCommand('+policeSeat', policeSeat, false)
	RegisterCommand('-policeSeat', function() end, false)
	exports["vrp-keybinds"]:registerKeyMapping("", "Gov", "Force into Vehicle", "+policeSeat", "-policeSeat", "RIGHT")

	RegisterCommand('+policeUnCuff', policeUnCuff, false)
	RegisterCommand('-policeUnCuff', function() end, false)
	exports["vrp-keybinds"]:registerKeyMapping("", "Gov", "UnCuff / Radar Rear", "+policeUnCuff", "-policeUnCuff", "DOWN")

	RegisterCommand('+policeSoft', policeSoft, false)
	RegisterCommand('-policeSoft', function() end, false)
	exports["vrp-keybinds"]:registerKeyMapping("", "Gov", "Soft Cuff", "+policeSoft", "-policeSoft")
end)
...
RegisterNetEvent("vrp-signin:signoff")
AddEventHandler("vrp-signin:signoff", function()
	isDoctor = false
	isTher = false
end)
...
AddEventHandler('vrp-police:disableRobbing', function(pState)
...
AddEventHandler("police:rob", function(pArgs, pEntity)
  ...
  local finished = exports["vrp-taskbar"]:taskBar(15000,"Robbing",false,true,nil,false,nil,5,pEntity)
...
AddEventHandler( 'KneelHU', function()
	...
	if ( DoesEntityExist( player ) and not IsEntityDead( player )) then
		...
		local finished = exports["vrp-taskbar"]:taskBar(2500,"Surrendering")
	end
end )
...
AddEventHandler('police:woxy', function()
	...
	if vehFront > 0 then
		...
		local finished = exports["vrp-taskbar"]:taskBar(4000,"Grabbing Scuba Gear")
	  	if finished == 100 then
	  		loadAnimDict('anim@narcotics@trash')
...
AddEventHandler('police:remmaskAccepted', function()
	TriggerEvent('vrp-clothing:faceWear', "mask", false)
	TriggerEvent('vrp-clothing:faceWear', "hat", false)
	TriggerEvent('vrp-clothing:faceWear', "glasses", false)
end)
...
AddEventHandler('police:gsr', function(pArgs, pEntity)
	...
	local finished = exports["vrp-taskbar"]:taskBar(15000, "GSR Testing")
	if finished == 100 then
...
AddEventHandler('police:payFines', function(amount, fromUnbill)
	local effect = exports['vrp-galleria']:GetEffect('FINE_REDUCE')
	if not effect then
		effect = {
			level = 0
		}
	...
	local characterEffect = exports['vrp-character']:GetFactor('FINE_AMOUNT') or 1.0
...
AddEventHandler('requestWounds', function(pArgs, pEntity)
	local targetPed = nil
	if not pEntity then
		targetPed = exports['vrp-target']:GetCurrentEntity()
	else
		targetPed = pEntity
	end 
...
Citizen.CreateThread(function()
  exports["vrp-keybinds"]:registerKeyMapping("", "Gov", "(EMS) Examine Target", "+examineTarget", "-examineTarget")
end)
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			...
			if myjob ~= "ems" and myjob ~= "doctor" and myjob ~= "doc" then
				local bandages = exports["vrp-inventory"]:getQuantity("bandage")
				if bandages == 0 then
					return
				else
...
AddEventHandler("ems:stomachpump", function()
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			local finished = exports["vrp-taskbar"]:taskBar(10000,"Inserting stomach pump 🤢", false, true)
...
AddEventHandler("ems:bloodtest", function()
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			local finished = exports["vrp-taskbar"]:taskBar(10000,"Taking blood test", false, true)
			if finished == 100 then
...
AddEventHandler('FlipVehicle', function(pDummy, pEntity)
  ...
  local finished = exports["vrp-taskbar"]:taskBar(30000, "Flipping Vehicle Over", true, true, nil, false, nil, 3.5, pEntity)
  ClearPedTasks(PlayerPedId())
...
AddEventHandler('animation:runtextanim', function(anim)
	local handCuffed = exports["isPed"]:isPed("handcuffed") or exports["isPed"]:isPed("beingcuffed")
	if not handCuffed and not IsPedRagdoll(PlayerPedId()) and not IsPedBeingStunned(PlayerPedId(), 0) then
	...
	Citizen.CreateThread(function()
		...
		if IsThisModelABoat(hash) then
			...
			if not nearDock then
				local finished = exports["vrp-taskbar"]:taskBar(45000, "Requesting Vehicle...", true)
				if finished ~= 100 then return end
			end
		end
...
AddEventHandler("police:chatCommand", function(args, cmd, isVehicleCmd,isEmsWhiteListed)
	...
	if cmd == "whitelist" then
		if rankService >= 10 then
			...
			TriggerServerEvent("police:whitelist",arg2,arg,arg3)
		else
	...
	if cmd == "rank" then
		if rankService >= 10 then
			...
			if not arg then errorMsg("Invalid argument") return end
			if not arg2 then errorMsg("Invalid Second argument") return end
			TriggerServerEvent("police:rank", arg2, arg, arg3)
		else
			errorMsg("You do not have the rank to do this action")
		end
	end
...
Citizen.CreateThread(function()
	exports["vrp-polyzone"]:AddBoxZone("wheelchair_store", vector3(-803.48, -1248.7, 7.34), 1.6, 1.0, {
		heading=320,
		minZ=6.14,
		maxZ=8.74,
		data = {
			id = "wheelchair_store_1",
		},
	})

	exports["vrp-polyzone"]:AddBoxZone("wheelchair_store", vector3(344.42, -589.63, 43.28), 1.5, 1.5, {
		heading=340,
		minZ=40.48,
		maxZ=44.48,
		data = {
			id = "wheelchair_store_2"
		}
	})

	exports["vrp-polyzone"]:AddBoxZone("wheelchair_store", vector3(343.08, -1414.06, 32.5), 1.8, 3.4, {
		heading=320,
		minZ=29.7,
		maxZ=33.7,
		data = {
			id = "wheelchair_store_3"
		}
	})
	

  exports["vrp-polyzone"]:AddBoxZone("police_station", vector3(461.91, -993.59, 30.69), 55.8, 54.8, {
		heading=0,
		minZ = 23.49,
		maxZ = 32.29,
		data = {
  ...
  exports["vrp-polyzone"]:AddBoxZone("police_station", vector3(-1084.8, -836.75, 13.52), 25.6, 11.2, {
		heading=309,
		minZ=12.32,
		maxZ=17.72,
		data = {
  ...
  exports["vrp-polyzone"]:AddBoxZone("police_station", vector3(1834.68, 3687.33, 42.97), 28.0, 65.4, {
		heading=30,
		minZ=33.17,
		maxZ=41.37,
		data = {
  ...
  exports["vrp-polyzone"]:AddBoxZone("police_station", vector3(-443.63, 6003.52, 31.49), 26.6, 37.6, {
		heading=316,
		minZ=22.34,
		maxZ=44.34,
		data = {
  ...
  exports["vrp-polyzone"]:AddBoxZone("police_station", vector3(382.87, 796.83, 187.46), 8.0, 11.8, {
		heading=0,
		minZ=186.46,
		maxZ=193.26,
		data = {
	...
	exports["vrp-polyzone"]:AddBoxZone("police_station", vector3(375.22, -1608.44, 30.04), 38.0, 61.2, {
		heading=320,
		minZ=27.84,
		maxZ=41.24,
		data = {
	...
	exports["vrp-polyzone"]:AddBoxZone("police_station", vector3(844.08, -1295.15, 38.62), 36.4, 32.2, {
		heading=0,
		minZ=24.82,
		maxZ=34.02,
		data = {
  ...
  exports["vrp-polyzone"]:AddBoxZone("possession_pickup", vector3(472.98, -1007.43, 26.27), 1.2, 2.6, {
	heading=0,
	minZ=25.27,
	maxZ=27.47,
	data = {
  ...
  exports["vrp-polyzone"]:AddBoxZone("possession_pickup", vector3(-1088.5, -811.11, 19.3), 1.2, 2.6, {
	heading=40,
	minZ=18.3,
	maxZ=20.5,
	data = {
  ...
  exports["vrp-polyzone"]:AddBoxZone("possession_pickup", vector3(384.99, 799.86, 187.46), 1.2, 2.6, {
	heading=0,
	minZ=186.46,
	maxZ=188.66,
	data = {
  ...
  exports["vrp-polyzone"]:AddBoxZone("possession_pickup", vector3(1835.42, 3678.33, 34.2), 2, 2, {
	heading=30,
	minZ=32.4,
	maxZ=36.4,
	data = {
  ...
  exports["vrp-polyzone"]:AddBoxZone("possession_pickup", vector3(-446.11, 6013.73, 32.29), 1.6, 1.2, {
		heading=316,
		minZ=31.29,
		maxZ=33.49,
		data = {
  ...
  exports["vrp-polyzone"]:AddBoxZone("possession_pickup", vector3(1840.42, 2579.44, 46.01), 2.2, 2.6, {
	heading=0,
	minZ=45.01,
	maxZ=47.61,
	data = {
    ...
    local data = {
        {
            ...
            key = true,
            children = {
				{ title = "Confirm Purchase", action = "vrp-ui:wheelchairPurchase", key = true },
			},
        },
    }
...
RegisterUICallback("vrp-ui:wheelchairPurchase", function(data, cb)

	local finished = exports["vrp-taskbar"]:taskBar(10000, "Purchasing...", true)
	if finished ~= 100 then
...
RegisterUICallback("vrp-ui:police:flagPlate", function(data, cb)
	cb({ data = {}, meta = { ok = true, message = "done" } })
...
Citizen.CreateThread(function()
  ...
  local afkAutoKick = exports["vrp-config"]:GetMiscConfig("afk.kick.auto")
  while true do
    Citizen.Wait(60000)
    ...
    if _myJob == "unemployed" then -- 1hr
      ...
      if afkCount == 30 then
        ...
        TriggerServerEvent("vrp-bugs:playerLogAction", {
