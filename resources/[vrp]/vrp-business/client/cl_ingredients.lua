...
Citizen.CreateThread(function()
  ...
  for idx, npc in ipairs(NPC_DATA) do
    ...
    local _npc = exports['vrp-npcs']:RegisterNPC(data, 'vrp-biz:ingr_' .. npc.id)
    exports['vrp-npcs']:EnableNPC(_npc.id)
    ids[#ids + 1] = _npc.id
  end

  exports['vrp-interact']:AddPeekEntryByFlag({ 'isNPC' }, {
    { id = 'biz_open_ingredients', label = 'View Stock', icon = 'list-alt', event = 'vrp-business:ingredients:open' },
  }, { distance = { radius = 2.5 }, npcIds = ids })
end)

AddEventHandler('vrp-business:ingredients:open', function(pArgs, pEntity, pContext)
  -- local passed = false
  ...
  local _context = RPC.execute('vrp-business:ingredients:getShop', npcType)
  exports['vrp-ui']:showContextMenu(_context)
end)

RegisterUICallback('vrp-business:ingredients:collectIngredients', function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  RPC.execute('vrp-business:ingredients:collectionHandler', data)
end)

RegisterUICallback('vrp-business:ingredients:purchaseIngredients', function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  RPC.execute('vrp-business:ingredients:purchaseHandler', data)
end)
