...
AddEventHandler('vrp-restaurants:getTakeoutItem', function(pParameters, pEntity, pContext)
    ...
    RPC.execute('vrp-restaurants:getTakeoutBox', pContext.zones['restaurant_takeout'])
end)

AddEventHandler('vrp-restaurants:openFridge', function(pParams, pEntity, pContext)
    local id = pParams.isEditorPeek and exports["vrp-housing"]:getCurrentPropertyID() or pContext.zones['restaurant_fridge'].id
...
AddEventHandler("vrp-inventory:itemUsed", function(item, info)
    ...
    if toyItems[item] then
        local finished = exports["vrp-taskbar"]:taskBar(1000, _L("restaurant-opening", "Opening"))
...
AddEventHandler('vrp-restaurants:shelfPrompt', function(pParams, pEntity, pContext)
    local id = pParams.isEditorPeek and exports["vrp-housing"]:getCurrentPropertyID() or pContext.zones['restaurant_shelf'].id
...
local function logToyGeneration(pRestaurant, pAmount)
    ...
    if generatedToys[pRestaurant][pId].amount then
        ...
        if newlyMadeToys >= 10 then
            TriggerServerEvent("vrp-restaurants:reportToysMade", pRestaurant, newlyMadeToys, generatedToys[pRestaurant][pId].amount)
            generatedToys[pRestaurant][pId].lastReportedAmount = generatedToys[pRestaurant][pId].amount
        end
    end
end

AddEventHandler('vrp-restaurants:getToyItem', function(pParameters, pEntity, pContext)
    local data = pContext.zones['restaurant_takeout']
...
AddEventHandler('vrp-restaurants:viewSafeCash', function(pParameters, pEntity, pContext)
    local business

    if pParameters and pParameters.isEditorPeek then
        business = exports["vrp-housing"]:getCurrentPropertyID()
    else
        business = pContext.meta.data.metadata.business
    end

    local context = RPC.execute('vrp-restaurants:getSafeCash', business)
    if not context then
        return
    end
...
AddEventHandler('vrp-restaurants:takeSafeCash', function(pParameters, pEntity, pContext)
    local business
    ...
    RPC.execute('vrp-restaurants:takeSafeCash', business, isNotRestaurant)
end)

AddEventHandler('vrp-restaurants:tradeInCash',  function(pParameters, pEntity, pContext)
    local item = exports['vrp-inventory']:getItemsOfType('envelope', 1, true, {
        cashEnvelope = true,
    })[1]
    if not item then
        return
    ...
    RPC.execute('vrp-restaurants:depositCash', info)
end)

AddEventHandler('vrp-restaurants:crackSafe', function(pParameters, pEntity, pContext)
    ...
    local finished = exports['vrp-taskbar']:taskBar(30000, _L("restaurant-crack-safe", "Cracking"), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute('vrp-restaurants:crackSafe', business)
    end
end)

AddEventHandler('vrp-restaurants:setWorkHours', function(pParameters, pEntity, pContext)
    ...
    RPC.execute('vrp-restaurants:setWorkHours', biz, prompt.open, prompt.close)
end)

AddEventHandler('vrp-restaurants:viewWorkHours', function(pParameters, pEntity, pContext)
    local biz = pContext.zones['restaurant_sign_on'].biz
    local context = RPC.execute('vrp-restaurants:getWorkHours', biz)
    exports['vrp-ui']:showContextMenu(context)
end)
