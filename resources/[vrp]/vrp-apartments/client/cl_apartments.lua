...
RegisterNetEvent('vrp-binds:keyEvent')
AddEventHandler('vrp-binds:keyEvent', function(name,onDown)
...
function Apart.processBuildType(numMultiplier,roomType,isSpawn,spawnOutSide)
    ...
    local isBuiltCoords = exports["vrp-build"]:getModule("func").buildRoom(name,numMultiplier,false)
    ...
    if isBuiltCoords then
        ...
        if isSpawn then
            ...
            TriggerEvent("vrp-spawn:characterSpawned", cid)
        else
...
function Apart.leaveApartment()
    local spawnInApartmentsOnly = false
    ...
    if Apart.exitPoint[Apart.currentRoomType] == nil then
        exports["vrp-build"]:getModule("func").exitCurrentRoom(Apart.Locations[Apart.currentRoomType][Apart.currentRoomNumber])
    else
        if type(Apart.exitPoint[Apart.currentRoomType]) == typeVector then
            exports["vrp-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType])
        else
            local rnd = math.random(1,#Apart.exitPoint[Apart.currentRoomType])
            exports["vrp-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType][rnd])
        end
    end
...
function Apart.OpenStash()
    ...
    TriggerEvent("server-inventory-open", "1", "motel-"..Apart.currentRoomType.."-"..Apart.currentHotelInformation.citizenid)
...
function garageToWorld()
    ...
    exports["vrp-build"]:getModule("func").exitCurrentRoom(vector3(4.67, -724.85, 32.18))
    Apart.currentRoomNumber = Apart.defaultInfo[2]
    Apart.currentRoomType = Apart.defaultInfo[1]
    Apart.currentHotelInformation = nil
    Apart.currentGarageNumber = 0
...
function Apart.WakeFromBed()
    local bedOffset = exports["vrp-build"]:getModule("func").getCurrentBed()
...
local outfitPlaces = {
  ...
  vector3(-1504.72,841.71,181.62), --Parsons
  ...
  vector3(-1789.08,461.31,94.0), -- clean manor basement
  ...
  vector3(-1558.92, -624.33, 23.28), -- the pit stop
  vector3(-521.97, 516.29, 112.59), -- mushkin motors
  vector3(-1722.56,156.39,64.38), -- temp pride 1
  vector3(-1738.39,179.19,64.38), -- temp pride 2
}

AddEventHandler("vrp-apartments:addOutfitSpot", function(pCoords)
...
function nearClothing()
    local isNear = exports["vrp-build"]:getModule("func").isNearCurrentInteract(1,5.5)
    if isNear then
        return true
    end
...
AddEventHandler('apartment:removeFromBuilding', function(roomNumber,buildType)
    if Apart.currentRoomType == buildType and Apart.currentRoomNumber == roomNumber then
        if exports["vrp-build"]:getModule("func").isInBuilding() then
            Apart.leaveApartment()
        end
    end
end)
...
RegisterUICallback("vrp-apartments:handler", function(data, cb)
...
AddEventHandler('apartments:menuAction', function(action)
    ...
    elseif action == "forfeit" then
        exports["vrp-ui"]:showContextMenu(MenuData["apartment_check"])
    end
...
function Apart.func.upgradeApartment(apartmentTargetType)
    ...
    local hasBankAccount = true

    if hasBankAccount then

        local success, message = RPC.execute("DoTransaction", apartmentInfo[apartmentTargetType].apartmentPrice)
...
RegisterNetEvent("vrp-spawn:characterSpawned", function(cid)
    TriggerServerEvent("vrp-spawn:characterSpawnedServer", cid)
end)
