...
AddEventHandler("mumbleConnected", function()
    ...
    TriggerEvent('vrp:voice:state', true)
end)

AddEventHandler("mumbleDisconnected", function()
    print("Mumble: Disconnected")
    TriggerEvent('vrp:voice:state', false)
end)

AddEventHandler('vrp:voice:state', function (state)
    VoiceEnabled = state

    TriggerServerEvent("vrp:voice:connection:state", state)
...
function AddPlayerToTargetList(serverID, context, transmit)
    if not Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("vrp:voice:transmission:state", serverID, context, true)
        end
...
function RemovePlayerFromTargetList(serverID, context, transmit, refresh)
    if Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("vrp:voice:transmission:state", serverID, context, false)
        end
...
function AddGroupToTargetList(group, context, pIsInVehicle)
    ...
    TriggerServerEvent("vrp:voice:transmission:state", group, context, true, nil, pIsInVehicle)
end
...
function RemoveGroupFromTargetList(group, context, transmit)
    ...
    if transmit then
        TriggerServerEvent("vrp:voice:transmission:state", group, context, false)
    end
end
...
function SetVoiceProximity(proximity)
    ...
    TriggerEvent("vrp:voice:proximity", proximity)
...
RegisterNetEvent("vrp:voice:transmission:state")
AddEventHandler("vrp:voice:transmission:state", function(serverID, context, transmitting, effect, isSourceInVehicle)
    ...
    local effect = exports['vrp-galleria']:GetEffect('RADIO_STRENGTH')
...
RegisterNetEvent('vrp:voice:targets:player:add')
AddEventHandler('vrp:voice:targets:player:add', AddPlayerToTargetList)

RegisterNetEvent('vrp:voice:targets:player:remove')
AddEventHandler('vrp:voice:targets:player:remove', RemovePlayerFromTargetList)

RegisterNetEvent('vrp:voice:refresh')
AddEventHandler('vrp:voice:refresh', RefreshConnection)

RegisterNetEvent('vrp:voice:endpoint:override')
AddEventHandler('vrp:voice:endpoint:override', OverrideEndpoint)

RegisterNetEvent('vrp:voice:proximity:override')
AddEventHandler('vrp:voice:proximity:override', function(pId, pMode, pRange, pPriority)
...
AddEventHandler('vrp-spawn:characterSpawned', RefreshConnection)
...
RegisterNetEvent('vrp-voice:setTransmissionDisabled', function (pType, pDisabled)
...
Citizen.CreateThread(function()
    exports["vrp-keybinds"]:registerKeyMapping("", "Voice", "Proximity / Range Toggle", "+cycleProximity", "-cycleProximity", "GRAVE")
    RegisterCommand('+cycleProximity', CycleVoiceProximity, false)
    RegisterCommand('-cycleProximity', function() end, false)

    exports["vrp-keybinds"]:registerKeyMapping("", "Voice", "Reset Mumble VOIP", "+mumble", "-mumble", "")
    ...
    TriggerEvent("vrp:voice:ready")

    -- SetSettings(exports["vrp-base"]:getModule("SettingsData"):getSettingsTable())
    SetSettings({
        ["tokovoip"] = {
          ["stereoAudio"] = true,
          ["localClickOn"] = true,
...
RegisterNetEvent('vrp-voice:setVoiceProximity')
AddEventHandler('vrp-voice:setVoiceProximity', function(pProximity)
...
Citizen.CreateThread(function ()
    ...
    while true do
        ...
        if (isTalking and not isTalkingToggle) or IsTalkingOnRadio and not isTalkingOnRadio then
            ...
            TriggerEvent("vrp:voice:transmissionStarted", { phone = true, radio = IsTalkingOnRadio })
        elseif (not isTalking and isTalkingToggle) or  isTalkingOnRadio and not IsTalkingOnRadio then
            ...
            TriggerEvent("vrp:voice:transmissionFinished", { phone = false, radio = IsTalkingOnRadio })
        end
