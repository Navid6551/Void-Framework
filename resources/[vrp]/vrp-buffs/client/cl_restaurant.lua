  ...
  ["resfooditem"] = {
    ...
    restaurantBaseBuff = {
      ["burger_shot"] = function(info)
        ...
        TriggerEvent("vrp-buffs:applyBuff", "burger_shot", { { buff = "stressblock", percent = 1.0, timeOverride = 180 * 60000 } })
        TriggerEvent("hadsugar")
        return processInfo(info)
      end,
      ["rooster"] = function(info)
        TriggerEvent("vrp-buffs:applyBuff", "rooster", { { buff = "jobluck", percent = 1.0 } })
        return processInfo(info)
      end,
      ["uwu_cafe"] = function(info)
        TriggerEvent("vrp-buffs:applyBuff", "uwu_cafe", { { buff = "strength", percent = 0.5, timeOverride = 60000 * 60 } })
        return processInfo(info)
      end,
      ["maldinis"] = function(info)
        TriggerEvent("vrp-buffs:applyBuff", "maldinis", { { buff = "fullness", percent = 1.0 } })
        return processInfo(info)
      end,
    },
  },
...
AddEventHandler("vrp-buffs:itemUsedSuccess", function(pSuccess)
...
AddEventHandler("vrp-inventory:itemUsed", function(pItem, pInfo)
  ...
  itemUseSuccess = nil
  TriggerEvent("vrp-inventory:attachPropPlayAnim", config.attachItem)
  ...
  TriggerEvent("vrp-buffs:applyBuff", pItem, buffs)
  if config.remove then
    if info._remove_id then
