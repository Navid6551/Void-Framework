...
function currentApartment()
    local cb = exports["vrp-apartments"]:getModule("func").currentApartment()
    return cb
end

function apartmentTable()
    local apartmentInfo = exports["vrp-apartments"]:getModule("info")
    return apartmentInfo
end

function upgradeApartment(apartmentTargetType)
    local isComplete,info = exports["vrp-apartments"]:getModule("func").upgradeApartment(apartmentTargetType)
...
function setGps(housingName)
    ...
    if propertyID == 0 and housingName ~= nil and housingName ~= "" then
        exports["vrp-apartments"]:getModule("func").gpsApartment(housingName)
    end
    if propertyID == 0 then return false,"failed to find property" end
...
function sellProperty(housingName, pAbandon)
    local propertyID = Housing.func.getPropertyIdFromName(housingName)
    if propertyID == 0 then return false,"failed to find property" end
    ...
    local currentKeys,currentOwned = RPC.execute("sellProperty",propertyID,propertyZone, pAbandon)
...
function housingSelfRemoveKey(housingName)
    ...
    local response = exports['vrp-ui']:OpenInputMenu(items)
    exports['vrp-ui']:closeApplication('textbox')
    exports['vrp-selector']:deselect()
    textBoxOpen = false
...
function unlock(propertyID)
    ...
    local jobAccess = hasRealtorAccess(false, true)
...
function enterEdit(housingName)
    ...
    if playerInRangeOfProperty(propertyID) then 
        ...
        local publicCrafting = exports["vrp-config"]:GetMiscConfig("housing.crafting.public")
...
function setGarage()
    ...
    if exports["vrp-vehicles"]:IsOnParkingSpot(GetVehiclePedIsIn(PlayerPedId(), false), true, 5.0) then TriggerEvent("DoLongHudText","To close to another garage.",2) return false end
...
function setCharChanger()
    ...
    if Housing.currentlyInsideBuilding then
        ...
        local buildingVector = exports["vrp-build"]:getModule("func").currentBuildingVector()
...
function switchBenchInventory(isBench)
    ...
    if isBench then
        local hasCraftingProg = exports["vrp-config"]:GetMiscConfig("progression.guns")
        ...
        if invVector ~= vector3(0.0,0.0,0.0) and invVector ~= nil then
            ...
            exports["vrp-ui"]:closeApplication("phone")
            exports["vrp-ui"]:showContextMenu(MenuData["crafting_check"])
        else
            placeBench(false)
        end
    else
        ...
        if benchVector ~= vector3(0.0,0.0,0.0) and benchVector ~= nil then
            exports["vrp-ui"]:closeApplication("phone")
            exports["vrp-ui"]:showContextMenu(MenuData["inventory_check"])
        else
            setInventory()
        end
    end
...
RegisterNetEvent("vrp:vehicles:hasHouseGarageAccess")
AddEventHandler("vrp:vehicles:hasHouseGarageAccess", function(garageId, resolve)
    local canUse = false
    local propertyID = 0
...
RegisterUICallback("vrp-ui:housingAddAccess", function(data, cb)
...
RegisterUICallback("vrp-ui:housingRemoveAccess", function(data, cb)
    local success, message = removeAccess(data.name,data.state_id)
    cb({ data = message, meta = { ok = success, message = (not success and message or 'done') } })
end)

RegisterUICallback("vrp-ui:housingEditCurrent", function(data, cb)
    local success, message = furnitureTrigger(data.name)
    cb({ data = message, meta = { ok = success, message = (not success and message or 'done') } })
end)

RegisterUICallback("vrp-ui:getCurrentApartment", function(data, cb)
    local message = exports["vrp-apartments"]:getModule("func").currentApartment()
    cb({ data = message, meta = { ok = true, message = (not success and message or 'done') } })
end)
RegisterUICallback("vrp-ui:getApartmentTypes", function(data, cb)
    local message = RPC.execute("getApartmentInformation")
    cb({ data = message, meta = { ok = true, message = (not success and message or 'done') } })
end)
RegisterUICallback("vrp-ui:upgradeApartmentType", function(data, cb)
    local success, message = exports["vrp-apartments"]:getModule("func").upgradeApartment(data.type)
    cb({ data = message, meta = { ok = success, message = (not success and message or 'done') } })
end)

RegisterUICallback("vrp-ui:getProperties", function(data, cb)
    local properties = combineIntoOneTable()
    cb({ data = properties, meta = { ok = true, message = "done" } })
end)

RegisterUICallback("vrp-ui:housingSetGPS", function(data, cb)
    setGps(data.name)
    cb({ data = {},  meta = { ok = true, message = "done" } })
end)

RegisterUICallback("vrp-ui:housingCheckCurrentLocation", function(data, cb)
    local success, message = currentLocation()
    cb({ data = message,  meta = { ok = success, message = (not success and message or 'done') } })
end)
RegisterUICallback("vrp-ui:housingCurrentLocationPurchase", function(data, cb)
    local success, message = buyProperty(data.name)
    cb({ data = message,  meta = { ok = success, message = (not success and message or 'done') } })
end)
RegisterUICallback("vrp-ui:housingSellProperty", function(data, cb)
    local success, message = sellProperty(data.name)
    cb({ data = message,  meta = { ok = success, message = (not success and message or 'done') } })
end)
RegisterUICallback("vrp-ui:housingAbandonProperty", function(data, cb)
    local success, message = sellProperty(data.name, true)
    cb({ data = message,  meta = { ok = success, message = (not success and message or 'done') } })
end)
RegisterUICallback("vrp-ui:housingEditProperty", function(data, cb)
    local success, message = enterEdit(data.name)
    cb({ data = message,  meta = { ok = success, message = (not success and message or 'done') } })
end)
RegisterUICallback("vrp-ui:housingEditPropertyStop", function(data, cb)
    exitEdit(true)
    cb({ data = {},  meta = { ok = true, message = "done" } })
end)
RegisterUICallback("vrp-ui:housingEditPropertyConfig", function(data, cb)
...
RegisterUICallback("vrp-ui:housingToggleLock", function(data, cb)
    local propertyID = Housing.func.getPropertyIdFromName(data.name)
    if propertyID == 0 then return false,"failed to find property" end
...
function housingRealtorBuyBack(housingName, price, sellerName)
    ...
    if price == nil or price <= 0 then
        return false, "Cannot sell for a negative amount"
    end

    local passed, msg = RPC.execute("housing:SendBuyBackRequest", propertyID, propertyZone, price, sellerName)

    return passed, msg
end

RegisterUICallback("vrp-ui:housingRealtorBuyBack", function(data, cb)
    local success, message = housingRealtorBuyBack(data.name, data.amount, data.character.first_name .. ' ' .. data.character.last_name)
    cb({ data = message,  meta = { ok = success, message = (not success and message or 'done') } })
end)
...
AddEventHandler("housing:reciveSellRequest", function(propertyID, senderName, cost, requestID)
    ...
    local accepted = exports['vrp-phone']:DoPhoneConfirmation("Housing", 'Buy For $'..cost, "horse-head");
...
RegisterNetEvent("housing:receiveBuyBackRequest", function(propertyID, senderName, price, requestID)
    if not playerInRangeOfProperty(propertyID) then 
        TriggerEvent("DoLongHudText", senderName .. " tried to buy your property but you are not near it.",2)
        return
    end

    local accepted = exports['vrp-phone']:DoPhoneConfirmation("Sell your property?", '$' .. price, "horse-head");
    RPC.execute("housing:buyBackRequestResponse", requestID , accepted)
end)

RegisterUICallback("vrp-ui:housingSellCurrentPropertyToPlayer", function(data, cb)
...
RegisterUICallback("vrp-ui:housingMakeLocation", function(data, cb)
    exports["vrp-ui"]:closeApplication("phone")
    openTextbox()
    cb({ data = {},  meta = { ok = success, message = message } })
end)
...
RegisterUICallback("vrp-ui:housingRemoveLocation", function(data, cb)
    exports["vrp-ui"]:closeApplication("phone")
    openRemoveTextbox()
    cb({ data = {},  meta = { ok = success, message = message } })
end)

RegisterUICallback("vrp-ui:attachBuisness", function(data, cb)
    exports["vrp-ui"]:closeApplication("phone")
    openAttatchTextbox()
    cb({ data = {},  meta = { ok = success, message = message } })
end)

function housingNameOrIDOrNearestToPropertyID(housingNameOrID)
   
    local propertyID = 0
    if housingNameOrID ~= nil then
        local tonum = tonumber(housingNameOrID)
        if tonum then
            propertyID = tonum
        else
            propertyID = Housing.func.getPropertyIdFromName(housingNameOrID)
        end
    end

    if propertyID == 0 then 
        local isComplete, propertyID, dist, zone = Housing.func.findClosestProperty()
        if (isComplete and propertyID) then
            return propertyID
        else
            TriggerEvent('DoLongHudText', 'Failed to find property', 2)
            print('(HOUSING) Error: failed to find property')
            return nil
        end
    end
...
RegisterNetEvent("housing:unlockcommand", function(housingNameOrID)
    local propertyID = housingNameOrIDOrNearestToPropertyID(housingNameOrID)
...
RegisterNetEvent("housing:lockcommand", function(housingNameOrID)
    local propertyID = housingNameOrIDOrNearestToPropertyID(housingNameOrID)
    if propertyID == nil then 
        return
    end
