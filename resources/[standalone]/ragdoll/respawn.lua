...
local InjuryList = {
    ...
    [`WEAPON_BANANA`] = { "WEAPON_BANANA", "Banana", minor = true },
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsEntityDead(player) then
            -- if exports['vrp-admin']:IsInGodMode() then
            ...
            local isHardcore = exports['vrp-character']:GetCharacterType() == 'hardcore'
            if isHardcore then
                local score = 40
                if isMinor then
                    score = 10
                ...
                exports['vrp-character']:AddHCScore(score)

                local icu = exports['vrp-character']:RollICU()
                Wait(1000)
                if icu then
            ...
            if GetZoneAtCoords(GetEntityCoords(player)) == 1321 then -- cayo
                allowLocalEMS = true
            end
...
AddEventHandler('doTimer', function()
    ...
    deathText = exports["vrp-i18n"]:GetStringSwap(deathText, i18nMap)
    respawnText = exports["vrp-i18n"]:GetStringSwap(respawnText, i18nMap)
...
AddEventHandler('disableAllActions', function()
    if not disablingloop then
        ...
        while imDead == 1 do
            ped = PlayerPedId()
            local isCarried = false--exports['vrp-police']:IsCarried()
            Citizen.Wait(200)
...
function getDeadAnimTreeAndAnim()
    ...
    local _, isAnimalModel = pcall(function () return exports["vrp-misc-code"]:isAnimalModel(model) end)
    if not isAnimalModel then return defaultAnimTree, defaultAnim end

    local _, animTree = pcall(function () return exports["vrp-misc-code"]:getAnimalModelDict(model) end)
    local _, anim = pcall(function () return exports["vrp-misc-code"]:getAnimalModelAnim(model, "dead") end)
...
function IsDeadAnimPlaying()
    if IsEntityPlayingAnim(PlayerPedId(), currentAnimTree, currentAnim, 1) or IsEntityPlayingAnim(PlayerPedId(), minorDict, minorAnim, 1) then
        return true
    else
        return false
    end
...
AddEventHandler('trycpr', function()
    ...
    curDist3 = #(GetEntityCoords(PlayerPedId(), 0) - vector3(5265.69, -5428.14, 64.75))
    if curDist < 5 or curDist2 < 5 or curDist3 < 10 then
        local penis = 0
        while penis < 10 do
            penis = penis + 1
            local finished = exports["vrp-ui"]:taskBarSkill(math.random(2000, 6000), math.random(5, 10))
...
--     Citizen.CreateThread(function()
--         Citizen.Wait(4000)
--         TriggerEvent("vrp-police:drag:stopForced")
--         TriggerEvent("vrp-police:cuffs:reset")
--     end)
-- end
...
function releaseBody(dropItems)
    ...
    local doctors = 0--RPC.execute('vrp-jobmanager:jobCount', 'doctor')
    if not doctors or doctors <= 0 then
        ...
        deathText = exports["vrp-i18n"]:GetStringSwap(deathText, i18nMap)
        respawnText = exports["vrp-i18n"]:GetStringSwap(deathText, i18nMap)
    end
    local coords,hosp = getClosestHospital()
    SetEntityCoords(player, coords)
    Wait(2000)
    TriggerEvent("bed:checkin", doctors and doctors > 0)
    TriggerServerEvent('vrp-healthcare:playerRespawn', hosp)
...
function getClosestHospital()
    local hospitals = {
        ['Central Hospital'] = vector3(341.25, -1397.43, 32.51),
        ['Sandy Hospital'] = vector3(1680.29, 3652.3, 35.44),
    }
    local plyCoords = GetEntityCoords(PlayerPedId())
    local min = 9999
    local foundName = 'Central Hospital'
