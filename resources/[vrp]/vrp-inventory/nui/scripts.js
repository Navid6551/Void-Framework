...
$(document).ready(function () {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://vrp-inventory/UpdateSettings',
            JSON.stringify({
    ...
    window.addEventListener('message', function (event) {
        ...
        if (item.response == 'openGui') {
            ...
            isCopAndLockdown = item.isCopAndLockdown
...
function UpdateQuality(data, penis) {
    ...
    let htmlstring = DOMPurify.sanitize(
        '<div ' +
        itemMaxed +
        " style='width:" +
        qualityHeight +
        ...
        +
        `<img src='${DOMPurify.sanitize(image)}` +
        "' data-info='" +
        escape(info) +
        "' data-inventory='" +
        inventoryNumber +
...
function ToggleBar(toggle, boundItems, boundItemsAmmo) {
    if (toggle) {
        ...
        $('#ActionBar').css({ display: "flex" }).fadeIn(50);
    } else {
        $('#ActionBar').fadeOut(50);
    }
}
...
function invStack(
    ...
    $.post('https://vrp-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://vrp-inventory/move', JSON.stringify(arr));
}
...
function CreateEmptySecondarySlot(slotLimit) {
    ...
    let htmlstring = '';
    for (i = 1; i < slotLimit + 1; i++) {
        htmlstring += "<div id='secondaryslot" + i + "' class='item " + classColorName + "' > </div>";
    }
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://vrp-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
            ...
            if (itemList[itemid] !== undefined) {
                ...
                if ((TargetInventoryName == 'Shop' || isPlayerShop) && !main) {
                    ...
                    htmlstring = DOMPurify.sanitize(
                        ...
                        + `</div> <img src='${DOMPurify.sanitize(image)}` +
                        "' data-inventory='" +
                        inventoryNumber +
                        "' data-creationDate='" +
                        creationDate +
                ...
                if (TargetInventoryName.indexOf('Craft') > -1 && !main) {
                    if (sqlInventory[i - 1] === undefined || sqlInventory[i - 1].amount === undefined) {
                        itemcount = inventory[i].itemcount ?? 1;
                    } else {
                        itemcount = sqlInventory[i - 1].amount;
                    }
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            // $.post('https://vrp-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://vrp-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://vrp-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://vrp-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://vrp-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://vrp-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function InsertItem(targetSlot, originSlot, inv1, inv2, itemid1, iteminfo1, itemid2, iteminfo2) {
    ...
    $.post(
        'https://vrp-inventory/insert-item',
        JSON.stringify({
            originInventory,
            targetInventory,
            originSlot,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://vrp-inventory/ServerCloseInventory',
        JSON.stringify({
...
function CheckCraftFail(itemid, moveAmount) {
    ...
    for (let ArrayX = 0; ArrayX < requirementsArray.length; ArrayX++) {
        ...
        if (!itemcheck && !weightcheck) {
            ...
            $.post('https://vrp-inventory/craftProgression', JSON.stringify({
            ...
            if (itemList[itemid].weapon) {
              $.post('https://vrp-inventory/weaponCrafted', JSON.stringify({
                "item": itemid,
              }));
            }
...
function AttemptDropInFilledSlot(slot) {
    ...
    if (
        ...
        (inventoryDropName === 'wrapsecondary' && isOwnerAndShop(TargetInventoryName,false) && !isCopAndLockdown)
    ...
    if (TargetInventoryName.startsWith('mailtemp')) {
        let isIllegal = false;
        let sqlInventory = JSON.parse(MyInventory);
        for (let i = 0; i < parseInt(MyItemCount); i++) {
            if (sqlInventory[i].item_id !== itemidsent && sqlInventory[i].item_id !== itemid2) continue;
            isIllegal = (itemList[itemidsent].illegal || itemList[itemid2].illegal) ?? false;
        }
        if (isIllegal) {
            result = _i18n("Can't place this here.");
        }
    ...
    if ((result == 'Success' || result == _i18n('Success')) && (result2 == 'Success' || result2 == _i18n('Success'))) {
        ...
        if (stacking) {
                ...
                if(isOwnerAndShop(TargetInventoryName,false) || !TargetInventoryName.includes('player-Shop')) {
                    if (purchaseCost > userCash && purchaseCost !== 0) {
                        ...
                        if (currentInventory == 2 && inventoryDropName == 'wrapmain') {
                            ...
                            InventoryLog(`${_i18n('Purchase Cost')}: $${purchaseCost} - ${_n('you have')} $${userCash}`);
                            purchase = true;
                        }
                    }
                } else {
                    if (isOwnerAndShop(TargetInventoryName,true) && !isCopAndLockdown){
            ...
            if ((isOwnerAndShop(TargetInventoryName,false) && !isCopAndLockdown) && inventoryReturnItemDropName == 'wrapsecondary'){
                result = 'You can not swap items into the shop!';
                EndDragError(slot);
                InventoryLog('Error: ' + result2 + ' | ' + result);
            }

            if (inventoryDropName == 'Shop' || inventoryDropName.indexOf('Craft') > -1 || (!StoreOwner && PlayerStore) || (isOwnerAndShop(TargetInventoryName,false) && !isCopAndLockdown)) {
            ...
            } else if (itemid1 === 'repairhammer_dev') {
                $.post(
                    'https://vrp-inventory/RepairSlot',
                    JSON.stringify({
                        slot: parseInt(slot.replace(/\D/g, '')),
                    }),
                );
            } else {
                SwapStacks(
...
function AttemptDropInEmptySlot(slot, isDropped, half) {
    ...
    if (TargetInventoryName.startsWith('mailtemp')) {
        ...
        for (let i = 0; i < parseInt(MyItemCount); i++) {
            if (sqlInventory[i].item_id !== itemidsent) continue;
            isIllegal = itemList[sqlInventory[i].item_id].illegal ?? false;
        }
        if (isIllegal) {
            result = _i18n("Can't place this here.");
        }
    ...
    if (result == 'Success' || result == _i18n('Success')) {
        ...
        //InventoryLog(item.dataset.fwewef + " | " + purchaseCost + " | " + moveAmount);
        if ((TargetInventoryName == 'Shop' || (!StoreOwner && PlayerStore) || TargetInventoryName.includes('player-Shop') ) && inventoryReturnItemDropName !== inventoryDropName) {
            if((isOwnerAndShop(TargetInventoryName,false) && !isCopAndLockdown) || !TargetInventoryName.includes('player-Shop') ) {
                //InventoryLog("eh: PURCHASE")
                if (purchaseCost > userCash && purchaseCost !== 0) {
...
function useitem() {
    ...
    if (currentInventory == 1) {
        ...
        $.post('https://vrp-inventory/invuse', JSON.stringify(arr));
...
function setPriceForItem(item,targetName, itemData) {
    if(priceEdit == ''){
        ...
        if(price >= 1){
            ...
            $.post('https://vrp-inventory/invSetPrice', JSON.stringify(information));
