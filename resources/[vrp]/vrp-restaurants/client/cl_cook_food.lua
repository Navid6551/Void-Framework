...
FoodItems = CacheableMap(function (ctx, pRestaurant)
  local items = RPC.execute("vrp-restaurants:getFoodItems", pRestaurant)
...
AvailableMainDishes = CacheableMap(function (ctx, pRestaurant)
  local items = RPC.execute("vrp-restaurants:fetchMenu", pRestaurant)
  if not items then return false, nil end
...
AddEventHandler("vrp-restaurants:createFoodItem", function(pParams, p2, pContext)
  local data = pParams.isEditorPeek and pParams or pContext.zones["restaurant_create_food_item"]
  if pParams.isEditorPeek then
    data.restaurant = exports["vrp-housing"]:getCurrentPropertyID()
  end
  ...
  for _, item in pairs(foodItems) do
    ...
    contextMenu[#contextMenu + 1] = {
      ...
      icon = "utensils",
      key = item,
      action = "vrp-ui:restaurant:cookFoodItem",
...
RegisterUICallback("vrp-ui:restaurant:cookFoodItem", function(data, cb)
  ...
  while true do
    if math.random() < 0.05 then
      local finished = exports["vrp-ui"]:taskBarSkill(math.random(2000, 8000), math.random(7, 12))
    ...
    for _, item in pairs(items) do
      for _, ingred in pairs(uniqueIngredients) do
        if hasCategory(item.__og_food_cats and item.__og_food_cats or item.foodCategory, ingred) then
          ...
          local qty = exports["vrp-inventory"]:getQuantity(item.itemName, true)
    ...
    local success = exports["vrp-taskbar"]:taskBar(5000, "Preparing...", true, false, false, false, nil, 0.5, PlayerPedId())
    if success ~= 100 then break end
    local metaDatas = {}
    for ingred, items in pairs(currentIngredientCounts) do
      local processedCount = 0
      for itemName, count in pairs(items) do
        local itemData = exports["vrp-inventory"]:getItemsOfType(itemName, count, true)
...
RegisterNetEvent("vrp-restaurants:changedFoodItems", function(pRestaurant)
  resetFoodItems(pRestaurant)
end)

RegisterNetEvent("vrp-restaurants:changedMenuItems", function(pRestaurant)
