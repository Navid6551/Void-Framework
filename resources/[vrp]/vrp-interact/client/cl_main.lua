...
function RefreshPeekList()
    ...
    Citizen.SetTimeout(250, function()
        ...
        exports["vrp-ui"]:sendAppEvent('eye', {
            action = "refresh",
            payload = entries or {}
        })
    end)
...
function StartPeekin()
    ...
    NearZones = {}
    nearbyZones = exports["vrp-polytarget"]:GetZones(GetEntityCoords(PlayerPedId())) or {}
    ...
    Citizen.CreateThread(function()
        ...
        while IsPeeking do
            ...
            if IsPeakActive and (IsControlJustReleased(0, 24) or IsDisabledControlJustReleased(0, 24)) then
                ...
                exports["vrp-ui"]:openApplication("eye", {
                    action = "interact",
    ...
    exports["vrp-ui"]:sendAppEvent('eye', { action = "peek", payload = { display = true, active = false } })
end
...
AddEventHandler("vrp:target:changed", function(pTarget, pEntityType, pEntityOffset)
...
AddEventHandler('vrp-polyzone:enter', function(zoneName, zoneData, zoneCenter)
...
AddEventHandler('vrp-polyzone:exit', function(zoneName)
    if not PeekEntries['polytarget'][zoneName] then return end
...
RegisterUICallback("vrp-ui:targetSelectOption", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })

    IsPeeking = false

    exports["vrp-ui"]:closeApplication("eye")
...
AddEventHandler('vrp-ui:phoneReady', function()
    RefreshPeekList()
end)
...
Citizen.CreateThread(function()
    exports["vrp-keybinds"]:registerKeyMapping("", "Player", "Peek at Target", "+targetInteract", "-targetInteract", "LMENU")
end)
...
AddEventHandler("vrp-preferences:setPreferences", function(data)
