...
function Spawn.obtainWorldSpawnPos(spawnName)
    ...
    local sanguine = Spawn.getSanguineSpawn()
    ...
    if sanguine and sanguine.info == spawnName then 
        found = sanguine.pos
    end
...
function getCharLimit()
    -- local userData = exports["vrp-base"]:getModule("LocalPlayer")

    -- local characterLimit = exports["vrp-config"]:GetModuleConfig("main").characterLimit

    -- local customSlots = userData:getVar("customCharacterSlots")

    return 5--customSlots ~= 0 and customSlots or characterLimit
end
