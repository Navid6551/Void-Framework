...
local function openToolbox()
  ...
  if not activeTools or #activeTools == 0 then
    activeTools = RPC.execute('vrp-business:sc:getToolbox')
    for _, tool in ipairs(activeTools) do
      tool.callback = 'vrp-business:sc:toolCallback:' .. tool.id
      RegisterUICallback(tool.callback, function(data, cb)
        cb({ data = {}, meta = { ok = true, message = 'done' } })
        Wait(1)
        local success = RPC.execute('vrp-business:sc:useTool', data.key, selection)
        if not success then
...
AddEventHandler('vrp-inventory:itemUsed', function(pItemId, pInfo)
  ...
  while deviceActive do
    ...
    if IsControlJustPressed(0, 25) then
      exports['vrp-selector']:startSelecting(-1, player, function(_entity, type, model)
        return false
      end)
      aiming = true
    end
    ...
    if IsDisabledControlJustReleased(0, 25) and not selection then
      exports['vrp-selector']:stopSelecting()
    ...
    if aiming and IsDisabledControlJustPressed(0, 24) then
      selection = exports['vrp-selector']:stopSelecting()
...
AddEventHandler('vrp-ui:application-closed', function(pName, pData)
...
RegisterNetEvent('vrp-business:sc:openTextbox', function(pId, pData)
  ...
  local response = exports['vrp-ui']:OpenInputMenu(pData)
  exports['vrp-ui']:closeApplication('textbox')
  exports['vrp-selector']:deselect()
  textBoxOpen = false
  TriggerServerEvent('vrp-business:sc:textboxResponse', pId, response)
end)

RegisterNetEvent('vrp-business:sc:openTextpopup', function(pData)
  exports['vrp-ui']:openApplication('textpopup', pData)
end)

RegisterNetEvent('vrp-business:sc:drawHousing', function()
  ...
  local zones = exports['vrp-housing']:retrieveHousingZonesConfig()
...
RegisterNetEvent('vrp-business:sc:openStreetMode', function()
  if streetModeEnabled then
    local success = RPC.execute('vrp-business:sc:addStreetHouses', streetModeData)
    if not success then
  ...
  while streetModeEnabled do
    ...
    if IsDisabledControlJustPressed(0, 24) then
      local streetSelection = exports['vrp-selector']:stopSelecting()
