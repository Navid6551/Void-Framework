...
Citizen.CreateThread(function()
    ...
    -- Register Keybinds
    exports["vrp-keybinds"]:registerKeyMapping("", "Weapons", "Switch Weapon Mode", "+switchWeaponMode", "-switchWeaponMode")
...
function SwitchWeaponMode()
	...
	if ForceWeaponMode[pWeaponHash] ~= nil then
		if SELECTED_FIRE_RATE == ForceWeaponMode[pWeaponHash] then
			...
			TriggerEvent("vrp-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
		end
		return
	end
...
RegisterNetEvent("vrp-weapons:client:weaponEquiped")
AddEventHandler("vrp-weapons:client:weaponEquiped", function(pWeaponHash, newInformation, sqlID, itemToRemove, pArmed)
	if ForceWeaponMode[pWeaponHash] ~= nil then
		SELECTED_FIRE_RATE = ForceWeaponMode[pWeaponHash]
		TriggerEvent("vrp-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
	end
	...
	TriggerEvent("vrp-weapons:client:showWeaponFireRate", pArmed)
	TriggerEvent("vrp-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
end)
