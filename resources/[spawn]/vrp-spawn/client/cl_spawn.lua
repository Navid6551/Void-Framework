...
function Login.ShowMenu(toggle)
	...
	local lifersDisabled = false--exports["vrp-config"]:GetMiscConfig("characters.lifers.disabled")
	local hardcoreDisabled = false--exports["vrp-config"]:GetMiscConfig("characters.hardcore.disabled")
	if Login.Open then
		SendNUIMessage({
			showAnim = true,
		...
		Wait(5000)
		local canCreate = RPC.execute('vrp-spawn:canCreateHardcoreCharacter')
...
RegisterNUICallback("nuiCallback", function(data, callback)
	...
	if data.action == "deleteCharacter" then
		...
		-- local events = exports["vrp-base"]:getModule("Events")
		-- events:Trigger("vrp-base:deleteCharacter", data.actionData, function(deleted)
		-- 	Login.actionsBlocked = false
		-- 	GetCharacterData.reset()
		-- 	GetClothingData.reset()
		-- 	Login.CreatePeds(true)
	...
	if data.action == "newCharacter" then
		...
		local result = RPC.execute('vrp-character:createCharacter', chardata)
