...
Citizen.CreateThread(function()
    ActiveAreaEvents = RPC.execute('vrp:sync:getActiveEvents') or {}
    while true do
        local idle = 1000
...
RegisterNetEvent('vrp:sync:active:area:start')
AddEventHandler('vrp:sync:active:area:start', function (data)
    ActiveAreaEvents[#ActiveAreaEvents + 1] = data
end)

RegisterNetEvent('vrp:sync:active:area:stop')
AddEventHandler('vrp:sync:active:area:stop', function (pEventId)
