Spawn.defaultSpawns = {
	...
	[10] =  { ["pos"] = vector4(374.2235, -1537.648, 29.29157, 139.534), ['info'] = 'Cody Sharp Memorial Park Bus Stop'}
}
...
AddEventHandler('spawn:clientSpawnData', function(spawnData)
	...
	if spawnData.overwrites ~= nil then
		...
		elseif spawnData.overwrites == "new" then
			Spawn.isNew = true
			Spawn.selectedSpawn(' Apartments 3')
		end
		return
	end

	local spawnInApartmentsOnly = false--(exports["vrp-config"]:GetMiscConfig("spawn.apartments.only") or false)
	...
	if spawnData.icuRooms and #spawnData.icuRooms > 0 then
		-- local roomData = exports['vrp-healthcare']:getICURoomSpawn(spawnData.icuRooms[1])
	...
	Spawn.businessSpawns = {}--RPC.execute('vrp-spawn:getBusinessSpawns', cid)
	...
	currentSpawns[#currentSpawns + 1] = Spawn.getSanguineSpawn()
...
function Spawn.getSanguineSpawn()
	local spawn = nil

	-- local sanguine = exports['vrp-business']:HasPermission('sanguine_isle', 'property_keys')
	-- if sanguine then
	-- 	spawn = { ["pos"] = vector4(4424.56, -4452.36, 7.57, 266.08), ['info'] = 'Sanguine Isle'}
	-- end
...
function Spawn.overwriteSpawn(overwrite, data)
	...
	if overwrite == "icu" then
		pos = vector4(318.02, -1358.95, 71.77, 203.16)
		SetEntityCoords(PlayerPedId(), data.coords.x, data.coords.y, data.coords.z)
		-- RPC.execute('vrp-healthcare:icu:rpc:spawn', cid)
	end
	...
	if overwrite == "jail" then
		TriggerEvent("vrp-jail:beginJail", false)
	end
