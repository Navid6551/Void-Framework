...
RegisterUICallback("vrp-ui:activateSelfieMode", function(data, cb)
...
RegisterUICallback("vrp-ui:phone:hasNotificationChanged", function(data, cb)
...
AddEventHandler('vrp-inventory:itemCheck', function(itemId, hasItem)
  ...
  for _,item in ipairs(phoneItems) do 
    hasPhone = exports['vrp-inventory']:hasEnoughOfItem(item, 1, false, true) or hasPhone
  end
  ...
  exports["vrp-ui"]:sendAppEvent("phone", { action = "phone-state-update", hasPhone = hasPhone })
end)
