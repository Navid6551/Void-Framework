...
function spawn(carsToSpawn)
  for i = 1, #carsToSpawn do
    ...
    if not carSpawns[i] or not spawnedVehicles[i] or carSpawns[i].model ~= car.model then
      ...
      if preset then
        exports["vrp-vehicles"]:SetVehicleAppearance(veh, preset.appearance and preset.appearance or {})
        exports["vrp-vehicles"]:SetVehicleMods(veh, preset.mods and preset.mods or {})

        if car.fitment and car.fitment.w_width then
          DecorSetBool(veh, "vrp-wheelfitment_applied", true)
          DecorSetFloat(veh, "vrp-wheelfitment_w_width", preset.fitment.w_width)
          DecorSetFloat(veh, "vrp-wheelfitment_w_fl", preset.fitment.w_fl)
          DecorSetFloat(veh, "vrp-wheelfitment_w_fr", preset.fitment.w_fr)
          DecorSetFloat(veh, "vrp-wheelfitment_w_rl", preset.fitment.w_rl)
          DecorSetFloat(veh, "vrp-wheelfitment_w_rr", preset.fitment.w_rr)
        end
      end
...
RegisterNetEvent("showroom:setFitment", function(pNetId, pFitment)
  ...
  DecorSetFloat(vehicle, "vrp-wheelfitment_w_width", pFitment.w_width)
  DecorSetFloat(vehicle, "vrp-wheelfitment_w_fl", pFitment.w_fl)
  DecorSetFloat(vehicle, "vrp-wheelfitment_w_fr", pFitment.w_fr)
  DecorSetFloat(vehicle, "vrp-wheelfitment_w_rl", pFitment.w_rl)
  DecorSetFloat(vehicle, "vrp-wheelfitment_w_rr", pFitment.w_rr)
end)
