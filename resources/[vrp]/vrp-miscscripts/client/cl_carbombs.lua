...
local function explodeVehicle(pEntity)
  exports['vrp-sync']:SyncedExecution('NetworkExplodeVehicle', pEntity, 1, 0, 0)
  TriggerServerEvent('vrp-miscscripts:carbombs:removeBomb', NetworkGetNetworkIdFromEntity(pEntity))
end
...
local function checkForCarBomb(pEntity) 
  ...
  local progress = exports['vrp-taskbar']:taskBar(25000, 'Searching for car bomb...', true)
  ClearPedTasks(PlayerPedId())

  if progress ~= 100 then return end

  local metaData = exports['vrp-vehicles']:GetVehicleMetadata(pEntity)
  local carBombMeta = metaData.carBombData or false
  local remoteBomb = VPX.Procedures.execute('vrp-usableprops:hasPhoneBomb', NetworkGetNetworkIdFromEntity(pEntity))

  if remoteBomb and remoteBomb.planted then
    local number = remoteBomb.bomb.number
    TriggerServerEvent('vrp-usableprops:defusePhoneBomb', number)
    TriggerEvent('vrp-police:client:showTextPopup', { show = true, text = 'Cell Phone Found, Number: ' .. number })
  ...
  if carBombMeta and carBombMeta.hasCarBomb then
    TriggerServerEvent('vrp-miscscripts:carbombs:foundBomb', NetworkGetNetworkIdFromEntity(pEntity), carBombMeta)
...
AddEventHandler('vrp-inventory:itemUsed', function (name, info)
  ...
  local elements = {
    { name = 'minSpeed', label = 'Min speed to stay above', icon = 'time', _type = 'number' },
    { name = 'ticksBeforeExplode', label = 'Seconds before explosion if below min speed', icon = 'time', _type = 'number' },
    { name = 'ticksForRemoval', label = 'Bomb time before turning off', icon = 'time', _type = 'number' },
    ...
    { name = "timeToComplete", label = "Time To Complete puzzle (15-30 Seconds)", icon = "time", _type = "number" },
  }
  ...
  local progress = exports['vrp-taskbar']:taskBar(25000, 'Planting car bomb...', true)
  ...
  local added = VPX.Procedures.execute('vrp-miscscripts:carbombs:addCarBomb', netId, minSpeed, ticksBeforeExplode, ticksForRemoval, gridSize, coloredSquares, timeToComplete)
  TriggerEvent('inventory:removeItem', 'car_bomb', 1)
  
  return TriggerEvent('DoLongHudText', 'Successfully added car bomb to vehicle', 1)
end)

AddEventHandler('vrp-miscscripts:carBombs:removeBomb', function (pData, pEntity)
  local metaData = exports['vrp-vehicles']:GetVehicleMetadata(pEntity)
  local carBombMeta = metaData.carBombData or false
  ...
  if carBombMeta and carBombMeta.hasCarBomb then
    ...
    exports['vrp-ui']:openApplication('memorygame', {
      gameFinishedEndpoint = 'vrp-miscscripts:carBombs:completeHacking',
...
RegisterUICallback('vrp-miscscripts:carBombs:completeHacking', function(data, cb)
  ...
  TriggerServerEvent('vrp-miscscripts:carbombs:removeBomb', netId)
