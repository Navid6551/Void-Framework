VPX = VPX or {}
VPX.BlipManager = VPX.BlipManager or {}
VPX.Blips = VPX.Blips or {}

function VPX.BlipManager.CreateBlip(self, id, data)
    ...
    VPX.Blips[id] = {blip = blip, data = data}
end

function VPX.BlipManager.RemoveBlip(self, id)
    local blip = VPX.Blips[id]
    if blip then RemoveBlip(blip.blip) end
    VPX.Blips[id] = nil
end

function VPX.BlipManager.HideBlip(self, id, toggle)
    local blip = VPX.Blips[id]
...
function VPX.BlipManager.GetBlip(self, id)
    local blip = VPX.Blips[id]
