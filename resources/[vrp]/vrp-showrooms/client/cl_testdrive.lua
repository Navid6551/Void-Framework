...
local function savePreset()
  ...
  local fitment = {}
  if DecorExistOn(vehicle, "vrp-wheelfitment_applied") then
    fitment = {
        w_width = DecorGetFloat(vehicle, "vrp-wheelfitment_w_width"),
        w_fl = DecorGetFloat(vehicle, "vrp-wheelfitment_w_fl"),
        w_fr = DecorGetFloat(vehicle, "vrp-wheelfitment_w_fr"),
        w_rl = DecorGetFloat(vehicle, "vrp-wheelfitment_w_rl"),
        w_rr = DecorGetFloat(vehicle, "vrp-wheelfitment_w_rr")
    }
  end
  return RPC.execute('showroom:saveTestDrivePreset', netId, fitment)
end

RegisterUICallback("vrp-ui:showroomTestDrive", function(data, cb)
  if data.location == 'tuner' then
    local hasPerms = exports['vrp-business']:HasPermission('tuner', 'hire')
...
RegisterUICallback("vrp-ui:showroomTestDriveReturn", function(data, cb)
  returnCurrentVehicle()
  cb({ data = {}, meta = { ok = true, message = 'done' } })
end)
RegisterUICallback("vrp-ui:showroomTestDriveSavePreset", function(data, cb)
  local success = savePreset()
  cb({ data = {}, meta = { ok = success, message = success and 'done' or 'Could not save.'} })
end)

-- RegisterNetEvent('vrp-showrooms:getCars', function()
--   -- local cars = RPC.execute('vrp-showrooms:fetchCars')
...
--       Wait(100)
--       local class = exports['vrp-vehicles']:GetVehicleRatingClass(tempCar)
--       if not class then
...
--   TriggerServerEvent('vrp-showrooms:saveCars', new)
-- end)
