...
function Login.SelectedChar(data)
	...
	local returnData = RPC.execute('vrp-base:selectCharacter', data.actionData)
    ...
    local clothingData = RPC.execute('vrp-clothing:fetchClothingData', { data.actionData })
    ...
    if not hasClothing then
        ...
        TriggerEvent("vrp-base:firstSpawn")
    end
end
...
function Login.setClothingForChar()
    ...
    exports['vrp-clothing']:SetModel(genderModel)

    TriggerEvent('vrp-clothing:openClothing', true, true)
...
RegisterNetEvent("vrp-spawn:finishedClothing")
AddEventHandler("vrp-spawn:finishedClothing", function(endType)
