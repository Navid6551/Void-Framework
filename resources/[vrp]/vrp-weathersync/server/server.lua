...
local function nextWeatherStage()
    ...
    TriggerEvent("vrp-weathersync:server:RequestStateSync")
end
...
-- EVENTS
RegisterNetEvent('vrp-weathersync:server:RequestStateSync', function()
    TriggerClientEvent('vrp-weathersync:client:SyncWeather', -1, CurrentWeather, blackout)
    TriggerClientEvent('vrp-weathersync:client:SyncTime', -1, baseTime, timeOffset, freezeTime)
end)

RegisterNetEvent('vrp-weathersync:server:setWeather', function(weather)
...
RegisterNetEvent('vrp-weathersync:server:setTime', function(hour, minute)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('vrp-weathersync:server:toggleBlackout', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('vrp-weathersync:server:toggleFreezeTime', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('vrp-weathersync:server:toggleDynamicWeather', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
