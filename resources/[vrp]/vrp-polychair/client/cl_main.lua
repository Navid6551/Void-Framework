Citizen.CreateThread(function()
    ...
    for idx,polyTarget in ipairs(ChairZones) do
        exports['vrp-interact']:AddPeekEntryByPolyTarget('vrp-polychair:chair_' .. tostring(idx) .. "-".. tostring(polyTarget[4].name), {{
            event = "vrp-polychair:chairSit",
...
function chairSit(chairPosition, cancel)
    ...
    if isSitting then
        ...
        isSitting = false
        TriggerEvent('vrp-polychair:stopSitting')
    else
        ...
        TriggerEvent('vrp-polychair:startSitting')
    end
    exports["vrp-flags"]:SetPedFlag(ped, 'isSittingOnChair', isSitting)
end

AddEventHandler('vrp-polychair:chairSit', function(pParameters, pEntity, pContext)
