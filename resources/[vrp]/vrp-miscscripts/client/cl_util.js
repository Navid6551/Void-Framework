    ...
    start(pTask) {
        ...
        this.tickId = setTick(async () => {
            if (this.animation && !IsEntityPlayingAnim(this.ped, this.dictionary, this.animation, 3)) {
                await VPX.Streaming.loadAnim(this.dictionary);
...
function Taskbar(pLength, pName, pRunCheck = false) {
    return new Promise((resolve) => {
        if (pName) {
            exports['vrp-taskbar'].taskBar(pLength, pName, pRunCheck, true, null, false, resolve);
        }
        else {
...
function TaskbarMiniGame(pDifficulty, pGap) {
    return new Promise((resolve) => {
        exports['vrp-ui'].taskBarSkill(pDifficulty, pGap, resolve);
    });
}