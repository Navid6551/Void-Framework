...
function SetEngineSound(pVehicle, pIsTemporary)
    ...
    if not sound then
        sound = exports['vrp-vehicles']:GetVehicleAfterMarket(pVehicle, 'engineSound')
    end
...
function RequestSyncStateUpdate(pVehicle)
    ...
    Citizen.SetTimeout(50, function()
        ...
        TriggerServerEvent('vrp-vehicleSync:updateSyncState', netId, syncState)
    end)
end
...
function UpdateSyncState(pVehicle, pSyncState)
    ...
    if flags['wheelFitment'] ~= previous['wheelFitment'] then
        local wheelFitment = exports['vrp-vehicles']:GetVehicleWheelFitment(pVehicle)
        if wheelFitment and wheelFitment.width then
            if not DecorExistOn(pVehicle, "vrp-wheelfitment_applied") then
                DecorSetBool(pVehicle, "vrp-wheelfitment_applied", true)
            end

            DecorSetFloat(pVehicle, "vrp-wheelfitment_w_width", wheelFitment.width)
            DecorSetFloat(pVehicle, "vrp-wheelfitment_w_fl", wheelFitment.fl)
            DecorSetFloat(pVehicle, "vrp-wheelfitment_w_fr", wheelFitment.fr)
            DecorSetFloat(pVehicle, "vrp-wheelfitment_w_rl", wheelFitment.rl)
            DecorSetFloat(pVehicle, "vrp-wheelfitment_w_rr", wheelFitment.rr)
        end
    end
...
RegisterNetEvent('vrp-vehicleSync:updateSyncState')
AddEventHandler('vrp-vehicleSync:updateSyncState', function(pNetId, pSyncState)
...
RegisterNetEvent('vrp-vehicles:spawnedVehicle', function(pVehicle)
    Citizen.Wait(1000)
    ...
    local afterMarkets = exports['vrp-vehicles']:GetVehicleAfterMarket(vehicle)
    ...
    local wheelFitment = exports['vrp-vehicles']:GetVehicleWheelFitment(vehicle)
...
RegisterNetEvent('vrp-vehicles:updateAfterMarkets', function(pNetId)
    Citizen.Wait(1000)
    local vehicle = NetworkGetEntityFromNetworkId(pNetId)
...
Citizen.CreateThread(function()
    exports["vrp-keybinds"]:registerKeyMapping("", "Vehicle", "Toggle Neon Lights", "+toggleNeonLights", "-toggleNeonLights")
    exports["vrp-keybinds"]:registerKeyMapping("", "Vehicle", "Toggle Light Indicator (Right)", "+toggleRightIndicator", "-toggleRightIndicator")
    exports["vrp-keybinds"]:registerKeyMapping("", "Vehicle", "Toggle Light Indicator (Left)", "+toggleLeftIndicator", "-toggleLeftIndicator")
    exports["vrp-keybinds"]:registerKeyMapping("", "Vehicle", "Toggle Light Indicator (Hazard)", "+toggleHazardLights", "-toggleHazardLights")
end)
