...
Citizen.CreateThread(function()
  -- dispatchersOnDuty = RPC.execute("vrp-dispatch:getDispatchersOnDuty")
end)

RegisterNetEvent("vrp-dispatch:dispatchersOnDuty")
AddEventHandler("vrp-dispatch:dispatchersOnDuty", function(pOnDuty)
	dispatchersOnDuty = 0 -- pOnDuty
end)

RegisterNetEvent("vrp-dispatch:becomeDispatchSelf")
AddEventHandler("vrp-dispatch:becomeDispatchSelf", function(pOnDuty)
    ...
    if pNotificationData.recipientList then
        for key, value in pairs(pNotificationData.recipientList) do
            if key == currentJob and value and not disableNotifications then
                ...
                elseif currentJob == "news" and exports["vrp-inventory"]:getQuantity("scanner") > 0 then
                    sendNewsBlip(pNotificationData)
                end
                ...
                then
                    TriggerServerEvent('vrp-fx:sound:playDispatch', pNotificationData.soundName);
                end
            end
        end
    end
...
AddEventHandler("vrp-jobmanager:playerBecameJob", function(job)
    ...
    if currentJob == "police" or currentJob == "doc" or currentJob == "ems" or currentJob == "news" then
        ...
        exports["vrp-ui"]:sendAppEvent("dispatch", {
            action = "initLoad",
            data = { active = true },
        })
...
function displayFastDispatch()
    if (currentJob == "police" or currentJob == "ems") and not isDead then
        ...
        TriggerEvent("vrp-vehicles:isDispatchOpen", showDispatchLog)
        exports["vrp-ui"]:SendUIMessage({
            source = "vrp-nui",
            app = "dispatch",
            show = showDispatchLog,
            data = data or {},
        })
        exports["vrp-ui"]:SetUIFocusCustom(showDispatchLog, showDispatchLog)
    end
end
...
Citizen.CreateThread(function()
    exports["vrp-keybinds"]:registerKeyMapping("","Gov", "View Dispatch", "+showFastDispatch", "-showFastDispatch")
end)
...
AddEventHandler("vrp-ui:application-closed", function(name)
    if name == "dispatch" and showDispatchLog then
        showDispatchLog = false
        TriggerEvent("vrp-vehicles:isDispatchOpen", true)
        mapOpen = false
        exports["vrp-ui"]:SetUIFocusCustom(false, false)
        TriggerServerEvent("vrp-dispatch:mapIsOpen", mapOpen)
    end
end)

RegisterNetEvent("vrp-dispatch:openFull")
AddEventHandler("vrp-dispatch:openFull", function(pData)
    mapOpen = true
    exports["vrp-ui"]:openApplication("dispatch", {
        showWithMap = true
    })
    TriggerServerEvent("vrp-dispatch:mapIsOpen", mapOpen)
end)
...
RegisterNetEvent("vrp-dispatch:updateUnits")
AddEventHandler("vrp-dispatch:updateUnits", function(units)
...
--         })
--     end
--     if currentJob == "news" and exports["vrp-inventory"]:getQuantity("scanner") > 0 then
...
RegisterNetEvent("vrp-dispatch:updateUnitCoords")
AddEventHandler("vrp-dispatch:updateUnitCoords", function(pCoords)
...
RegisterNetEvent("vrp-dispatch:updateDispatch")
AddEventHandler("vrp-dispatch:updateDispatch", function(pData)
  exports["vrp-ui"]:sendAppEvent("dispatch", pData)
end)
...
--         exports["vrp-ui"]:openApplication("dispatch")
--     else
--         exports["vrp-ui"]:closeApplication("dispatch")
--     end
-- end)

RegisterUICallback("vrp-ui:dispatchAction", function(data, cb)
    if data.action == "createCall" then
        RPC.execute("vrp-dispatch:createCall", data.ctxId)
    end
    if data.action == "dismissPing" then
        RPC.execute("vrp-dispatch:dismissPing", data.ctxId)
    end
    if data.action == "dismissCall" then
        RPC.execute("vrp-dispatch:dismissCall", data.ctxId)
    end
    if data.action == "clearUnits" then
        RPC.execute("vrp-dispatch:clearUnits", data.ctxId)
    end
    if data.action == "toggleUnit" then
        RPC.execute("vrp-dispatch:toggleUnitAssignment", data.ctxId, data.unit)
    end
    if data.action == "setGPSLocation" then
        SetNewWaypoint(data.ping.origin.x, data.ping.origin.y)
    end
    if data.action == "toggleUnitActive" then
        RPC.execute("vrp-dispatch:toggleUnitActive", data.ctxId, data.unit)
    end
    if data.action == "setRadio" then
      setRadioChannelForCall(data.ctxId)
    end
    if data.action == "setUnitVehicle" then
        RPC.execute("vrp-dispatch:setUnitVehicle", data.data)
    end
    if data.action == "setUnitStatus" then
        RPC.execute("vrp-dispatch:setUnitStatus", data.data)
    end
    if data.action == "setUnitRidingWith" then
        RPC.execute("vrp-dispatch:setUnitRidingWith", data.data)
    end
    cb({ data = {}, meta = { ok = true, message = "done" } })
end)
RegisterUICallback("vrp-ui:getDispatchData", function(data, cb)
  local result = RPC.execute("vrp-dispatch:getDispatchData")
  print('vrp-dispatch:getDispatchData', result)
  cb({ data = result, meta = { ok = true, message = 'ok' } })
end)
...
function setRadioChannelForCall(callId)
  exports["vrp-ui"]:closeApplication("dispatch")
  Wait(1000)
  local input = exports['vrp-ui']:OpenInputMenu({
  ...
  RPC.execute("vrp-dispatch:setRadioChannelForCall", callId, input.channel)
end
      ...
      if (key ~= nil and item ~= nil) then
        ...
        if #(vector2(pos.x, pos.y) - vector2(item.pos.x, item.pos.y)) < 50.0 then
          if item.jobType == "ems" then
            if exports["vrp-base"]:getModule("LocalPlayer"):getVar("job") == "ems" then
          ...
          elseif item.jobType == "news" then 
            if exports["vrp-base"]:getModule("LocalPlayer"):getVar("job") == "news" then
              TriggerServerEvent('phone:assistRemove', item.id, item.jobType) -- Send message of clear to others
              clearBlip(item, false)
              if GetTimeDifference(curTime, item.timestamp) > 2000 then
                if canPay then
