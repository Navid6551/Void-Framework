AddEventHandler("vrp-vineyard:npcManager", function(p1)
  ...
  if p1.id == "start_harvest" then
    RPC.execute("vrp-vineyard:startHarvest")
    return
  end
  if p1.id == "rent_atv" then
    local finished = exports["vrp-taskbar"]:taskBar(math.random(5000, 10000), "Grabbing ATV")
    if finished ~= 100 then return end
    RPC.execute("vrp-business:herowine:chargeCompany", 5000)
...
AddEventHandler("vrp-vineyard:pickupBasket", function(p1, p2, p3)
  if p3 and p3.meta and p3.meta.data and p3.meta.data.metadata and p3.meta.data.metadata.type == "vineyard_grape_basket" then
    ...
    RPC.execute("vrp-vineyard:pickupBasket", objectId, data)
  end
end)

Citizen.CreateThread(function()
  exports["vrp-interact"]:AddPeekEntryByModel({ `prop_fruit_basket` }, {
    {
      id = "peek_prop_fruit_basket",
      event = "vrp-vineyard:pickupBasket",
