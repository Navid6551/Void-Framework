...
function HasPermission(pBusiness, pPermission)
    ...
    if not PermissionCache[pBusiness][pPermission] or (GetGameTimer() - PermissionCache[pBusiness][pPermission].UpdatedAt) > 15 * 60000 then
        ...
        local success = RPC.execute("vrp-business:hasPermission", pBusiness, pPermission, cid)
...
function HasRole(pBusiness, pRole)
    ...
    if not RoleCache[pBusiness][pRole] or (GetGameTimer() - RoleCache[pBusiness][pRole].UpdatedAt) > 15 * 60000 then
        local cid = exports["isPed"]:isPed("cid")
        local success = RPC.execute("vrp-business:hasRole", pBusiness, pRole, cid)
...
AddEventHandler('vrp-spawn:characterSpawned', RefreshEmploymentList)

RegisterNetEvent('vrp-business:employmentStatus')
AddEventHandler('vrp-business:employmentStatus', function(pBusiness, pEmployed)
