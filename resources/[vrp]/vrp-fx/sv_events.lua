RegisterNetEvent("vrp-fx:smoke:grenade", function(pCoords)
    local players = exports["vrp-infinity"]:GetNerbyPlayers(pCoords, 100)
...
RegisterNetEvent("fx:waterPour", function(pNetId)
    ...
    local players = exports["vrp-infinity"]:GetNerbyPlayers(pCoords, 10)
...
RegisterNetEvent("vrp-fx:graffiti:spray", function(pNetId, coords)
    local src = source
...
RegisterNetEvent("vrp-fx:graffiti:stopSpray", function(pNetId)
    local pEntity = NetworkGetEntityFromNetworkId(pNetId)
    local pCoords = GetEntityCoords(pEntity)
...
RegisterNetEvent("vrp-fx:chain:blingDiamonds", function(pCoords, pType, pSize, pStrengh, pScale)
...
local function doFx(fx, i, offset)
    Citizen.CreateThread(function()
        ...
        TriggerEvent("vrp-scenes:addSceneToClient", {
...
RegisterServerEvent("vrp-fx:sound:playDispatch")
AddEventHandler("vrp-fx:sound:playDispatch", function(sound)
    local src = source
    local coords = GetEntityCoords(GetPlayerPed(src))
    local players = exports["vrp-infinity"]:GetNerbyPlayers(coords, 5)
    
    -- triggerAudio(src, 1, 2.0, sound, 'DLC_NIKEZ_GENERAL', 'vrp-fx:sound:PlaySound')
    triggerAudio(src, 1, 2.0, sound, 'DLC_NIKEZ_RINGTONES', 'vrp-fx:sound:PlaySound')
end)
...
RegisterServerEvent("vrp-fx:sound:vehicles")
AddEventHandler("vrp-fx:sound:vehicles", function(sound)
    local src = source
    local coords = GetEntityCoords(GetPlayerPed(src))
    local players = exports["vrp-infinity"]:GetNerbyPlayers(coords, 3)

    triggerAudio(src, 1, 3.0, sound, 'DLC_NIKEZ_GENERAL', 'vrp-fx:sound:PlaySound')
end)

RegisterServerEvent("vrp-fx:sound:playGateLocking")
AddEventHandler("vrp-fx:sound:playGateLocking", function(isLocked, isSpecial)
    local src = source
    local coords = GetEntityCoords(GetPlayerPed(src))
    local players = exports["vrp-infinity"]:GetNerbyPlayers(coords, 3)
    local sound = ""
...
function triggerAudio(pPlayerId, pType, pRadius, pSound, pRef, pEvent)
    ...
    local Event = {
        ...
        outEvent = 'vrp-fx:sound:StopSound'
    }
