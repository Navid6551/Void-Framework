...
CreateThread(function ()
    exports["vrp-polyzone"]:AddBoxZone("reed_pickup", vector3(1001.51, -2116.59, 30.48), 3.5, 3.5, {
        heading=357,
        minZ=29.48,
        maxZ=35.00,
        data = {
    ...
    exports['vrp-interact']:AddPeekEntryByEntityType(
        { 1 },
        {
          {
            id = 'reed_pickup',
            label = "Attach",
            icon = 'dizzy',
            event = 'vrp-miscscripts:reedHook',
            parameters = {},
          },
        },
        {
...
AddEventHandler("vrp-polyzone:enter", function(pName, pData)
    if pName ~= "reed_pickup" then return end
    isInsideHookZone = true
end)

AddEventHandler("vrp-polyzone:exit", function(pName, pData)
    if pName ~= "reed_pickup" then return end
    isInsideHookZone = false
end)

AddEventHandler("vrp-miscscripts:reedHook", function(pParameters, pEntity, pContext)
    local hookedTargetServerId = GetPlayerServerId(NetworkGetPlayerIndexFromPed(pEntity))
    if hookedTargetServerId ~= 0 then
        VPX.Events.emitNet('vrp-miscscripts:reedSetHook', hookedTargetServerId)
    end
end)
...
VPX.Events.onNet('vrp-miscscripts:reedSetHooked', function(pTarget, pState)
    print(pTarget, pState)
    if pState and not isHooked then
        VPX.Streaming.loadAnim(animDict)
        isHooked = true
...
RegisterCommand("-unhook", function()
    if isHooked or isInsideHookZone then
        VPX.Events.emitNet('vrp-miscscripts:reedSetHook')
    end
end, false)