...
function rebuildCurrent(name)
    ...
    if CURRENT_OBJECTS and CURRENT_OBJECTS[name] then
        ...
        for k,v in pairs(CURRENT_OBJECTS[name]) do
            ...
            if not IsModelValid(modelHash) or (CURRENT_ISSHOP and v.realName == 'prop_ped_gib_01') then
                goto continue
            end
            local coords = applyOffsets(v.coords)
            local quat = v.quat
            objectsToCreate[grouped][#objectsToCreate[grouped]+1] = {
                id = k,
                modelHash = modelHash,
                coords = coords,
                quat = quat
            ...
            if count % 100 == 0 then
                grouped = grouped + 1
                objectsToCreate[grouped] = {}
            end
            count = count + 1
            ::continue::
        end
        for groupId,groupedObjects in pairs(objectsToCreate) do
            Citizen.CreateThreadNow(function()
                for k,v in pairs(groupedObjects) do
                    LoadModel(v.modelHash)
                    local obj = CreateObject(v.modelHash, v.coords.x, v.coords.y, v.coords.z, false, false, false)
                    if obj then
                        FreezeEntityPosition(obj, true)
                        SetEntityQuaternion(obj, v.quat)
                        CURRENT_ENTITY[name][v.id] = obj
                    end
                end
            end)
        end
...
local buildingObjects = false
RegisterNetEvent("vrp-editor:buildName")
AddEventHandler("vrp-editor:buildName", function(name,objects, offsets, isShop)
    buildingObjects = true
    if objects then
    ...
    loadObjects(name)
    buildingObjects = false
end)

RegisterNetEvent("vrp-editor:destroyName")
AddEventHandler("vrp-editor:destroyName", function(name)
    while buildingObjects do
        Wait(1000)
    end
    cleanUpArea(name)
        ...
        if distance < 50.0 and ObjectFound ~= playerped then
            ...
            if not ignore then
                if IsEntityAPed(ObjectFound) then
                    ...
                    if not IsEntityAVehicle(ObjectFound) and not IsEntityAttached(ObjectFound) and not Entity(ObjectFound).state.DontClear then
                        DeleteObject(ObjectFound)
                    end
                end
            end           
...
function ObjectToNui(name,pass)
    ...
    for k,v in pairs(CURRENT_OBJECTS[name]) do
        sortedObject[k] = {CURRENT_ENTITY[name][k],v.realName,v.id, DISPLAY_NAMES[v.realName] }
    end
...
function getCurrentEntityIndex(entityId)
    local index = -1

    if CURRENT_ENTITY ~= nil and CURRENT_ENTITY[CURRENT_NAME] ~= nil then
        for k,v in pairs(CURRENT_ENTITY[CURRENT_NAME]) do
            if v == entityId then
                index = k
                break;
...
function buildNPC(coords, heading)
    ...
    local currentPropertyBuisness = exports['vrp-housing']:getPropertyBuisness()

    if currentPropertyBuisness ~= nil and CUSTOM_SHOPS[currentPropertyBuisness] ~= nil then
        CUSTOM_SHOPS[currentPropertyBuisness](coords,heading)   
        return
    end
    ...
    for o,i in pairs(npc.peek) do
        exports["vrp-interact"]:AddPeekEntryByFlag({ "isNPC" }, {
            {
            id = i.id..o,
            label = i.label,
            icon = i.icon,
...
function removeNPC()
    if CURRENT_NPC then
        for k,v in pairs(CURRENT_NPC) do
            exports["vrp-npcs"]:DisableNPC(k)
            exports["vrp-npcs"]:RemoveNPC(k)
        end
...
function pedToModel(name)
    if CURRENT_OBJECTS and CURRENT_ISSHOP and CURRENT_OBJECTS[name] then
        for k,v in pairs(CURRENT_OBJECTS[name]) do
            if v.realName == 'prop_ped_gib_01' then
